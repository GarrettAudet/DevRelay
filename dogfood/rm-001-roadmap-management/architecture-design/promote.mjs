...
export async function promoteSimplificationArchitecture(context) {
  const {
    root,
    dir,
    read,
  ...
  const ownerApprovalBytes = read(
    "dogfood/rm-001-roadmap-management/architecture-design/architecture-gate-owner-approval.json",
  );
  ...
  const gateReviewRef = fileRef(
    "architecture-gate-review-rm-001-roadmap-management-v1",
  ...
  const executionProofRef = fileRef(
    "architecture-dogfood-execution-proof-rm-001-roadmap-management-v1",
  ...
  const promotedBaseline = {
    ...
    baselineId: "architecture-baseline-devrelay-v1-rm-001-roadmap-management-001",
  ...
  const promotedProjectState = {
    ...
    stateId: "project-architecture-state-devrelay-rm-001-roadmap-management-approved-v1",
    state: "baselined",
  ...
  const expectedRequiredInterfaces = [
    ...new Set([
      ...
      "IF-RM-GATE",
      "IF-RM-BASELINE",
      "IF-RM-SESSION-SNAPSHOT",
      "IF-RM-SESSION-RECEIPT",
      "IF-RM-REFRESH",
      "IF-RM-TRACEABILITY",
    ]),
  ].sort();
  ...
  const promotionId = "architecture-promotion-rm-001-roadmap-management-v1";
