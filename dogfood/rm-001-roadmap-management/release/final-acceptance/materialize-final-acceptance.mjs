...
const fullRef = (
  ...
  schema,
  mediaType,
  uri: `memory://devrelay/rm-001/${encodeURIComponent(artifactId)}/${digest.slice(7)}.json`,
});
...
function restoreGraph() {
  const graphPath =
    "dogfood/rm-001-roadmap-management/release/traceability-graph-snapshot.json";
...
const contract = loadedRaw(
  ...
  readJson("project/contract-baseline.json").baselineId,
...
const verificationSummary = readJson(
  "dogfood/rm-001-roadmap-management/release/verification-summary.json",
);
const canonicalReceipt = readJson(
  "dogfood/rm-001-roadmap-management/release/canonical-verification-receipt.json",
);
const lifecycleCompletion = readJson(
  "dogfood/rm-001-roadmap-management/release/lifecycle-completion-summary.json",
);
const roadmapPromotion = readJson(
  "dogfood/rm-001-roadmap-management/release/roadmap-promotion-record.json",
);
const releaseEvidenceValue = seal(
  {
    apiVersion: API,
    kind: "RM001ReleaseCandidateEvidenceSet",
    evidenceSetId: "RELEASE-EVIDENCE-DEVRELAY-RM001-ROADMAP-MANAGEMENT-001",
    implementationCommit,
    verification: {
      command: "npm.cmd run verify",
      total: canonicalReceipt.testSummary.tests,
      passed: canonicalReceipt.testSummary.passed,
      failed: canonicalReceipt.testSummary.failed,
      skipped: canonicalReceipt.testSummary.skipped,
      receipt: ref(canonicalReceipt.receiptId, canonicalReceipt.receiptDigest),
      summary: ref("RMVerificationSummary", verificationSummary.contentDigest),
    },
    roadmapDogfood: {
      baseline: lifecycleCompletion.roadmapDogfood.baseline,
      projection: lifecycleCompletion.roadmapDogfood.projection,
      contextRefresh: lifecycleCompletion.roadmapDogfood.contextRefresh,
      resultingGraph: lifecycleCompletion.roadmapDogfood.resultingGraph,
      promotion: ref("RM-ROADMAP-PROMOTION-RECORD-001", roadmapPromotion.contentDigest),
      lifecycle: ref("RM-LIFECYCLE-COMPLETION", lifecycleCompletion.contentDigest),
    },
    releaseBoundary:
...
const workItemIds = lifecycleCompletion.completedWorkItems
  .map(({ workItemId }) => workItemId)
  .sort((left, right) => left.localeCompare(right, "en"));
const sourceIntegration = readJson(
  "dogfood/rm-001-roadmap-management/release/integration-record.json",
);
const aggregateIntegrationRecord = seal(
  {
    apiVersion: API,
    kind: "IntegratedChangeRecord",
    recordId: "ICR-DEVRELAY-RM001-ROADMAP-MANAGEMENT-001",
    sourceIntegration: ref(sourceIntegration.integrationId, sourceIntegration.contentDigest),
    implementationCommit,
    outcome: "integrated",
    workItemIds,
  },
...
const records = [aggregateIntegrationRecord];
const completionFacts = workItemIds.map((workItemId) => ({
  workItemId,
  authority: "approved",
  integrationRef: ref(aggregateIntegrationRecord.recordId, aggregateIntegrationRecord.recordDigest),
  evidence: [ref("RMVerificationSummary", verificationSummary.contentDigest)],
}));
const completionFactSet = seal(
  {
    apiVersion: API,
    kind: "IntegratedCompletionFactSet",
    factSetId: "ICFS-DEVRELAY-RM001-ROADMAP-MANAGEMENT-001",
...
const systemPolicy = seal(
  {
    ...
    policyId: "SV-POLICY-DEVRELAY-RM001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const integratedSubject = seal(
  {
    ...
    subjectId: `DEVRELAY-RM001-DESKTOP-WINDOWS-${releaseEvidence.value.evidenceSetDigest.slice(7, 19).toUpperCase()}`,
...
const reviewObligations = obligations.obligations
  ...
  .map(({ obligationId }) => obligationId);
const invocations = [
  invocationFor("SV-DEVRELAY-RM001-TEST-001", TEST_SYSTEM_VERIFIER, testObligations),
  invocationFor("SV-DEVRELAY-RM001-REVIEW-001", REVIEW_SYSTEM_VERIFIER, reviewObligations),
];
...
const verifier = async (invocation) => {
  ...
  if (invocation.verifier.id === TEST_SYSTEM_VERIFIER.id) {
    ...
    return adaptSystemTestResult({
      invocation,
      nativeBytes: bytes,
      nativeArtifact: ref("NATIVE-DEVRELAY-RM001-TEST-001", sha256Digest(bytes)),
    });
  }
  const native = {
  ...
  return adaptSystemReviewResult({
    invocation,
    nativeBytes: bytes,
    nativeArtifact: ref("NATIVE-DEVRELAY-RM001-REVIEW-001", sha256Digest(bytes)),
  });
};
...
const systemInvocation = {
  invocationId: "SYSTEM-VERIFICATION-DEVRELAY-RM001-001",
...
const businessPolicy = seal(
  {
    ...
    policyId: "BA-POLICY-DEVRELAY-RM001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const businessEvidence = seal(
  {
    ...
    evidenceSetId: "BA-EVIDENCE-DEVRELAY-RM001-DESKTOP-WINDOWS-001",
    requirementsBaseline: ref(requirements.ref.artifactId, requirements.ref.digest),
    projectOverviewBaseline: ref(overview.ref.artifactId, overview.ref.digest),
...
const requirementsInvocation = {
  invocationId: "REQUIREMENTS-BASELINE-SCOPE-RECONCILIATION-RM001-001",
...
assert.equal(
  requirementsMerged.snapshot.nodes.filter(
    ({ kind, state, scope }) => kind === "business-scope" && state === "active" && scope === "requirements/baseline",
  ).length,
  scopeIds.length,
);
...
const businessInput = {
  checkpointId: "BA-DEVRELAY-RM001-DESKTOP-WINDOWS-001",
...
const ownerApproval = seal(
  {
    ...
    approvalId: "BA-APPROVAL-DEVRELAY-RM001-DESKTOP-WINDOWS-001",
...
let ownerCalls = 0;
const gateInput = {
  checkpointId: "BA-GATE-DEVRELAY-RM001-DESKTOP-WINDOWS-001",
...
const businessInvocation = {
  invocationId: "BUSINESS-ACCEPTANCE-GATE-DEVRELAY-RM001-001",
  module: {
...
const approvalRequest = seal(
  {
    ...
    requestId: "BA-APPROVAL-REQUEST-DEVRELAY-RM001-DESKTOP-WINDOWS-001",
    candidate: ref(business.candidate.candidateId, business.candidate.candidateDigest),
    candidateRawDigest: sha256Digest(candidateBytes),
    technicalCoverage: ref(
      business.technicalCoverage.coverageId,
...
const finalProof = seal(
  {
    apiVersion: API,
    kind: "DevRelayRM001SourceReleaseAcceptanceProof",
    proofId: "DEVRELAY-RM001-SOURCE-RELEASE-ACCEPTANCE-001",
    sourceBaseCommit,
...
const summary = {
  apiVersion: API,
  kind: "DevRelayRM001FinalAcceptanceSummary",
  releaseEvidence: releaseEvidenceRef,
...
writeText(
  "FINAL_ACCEPTANCE.md",
  `# DevRelay RM-001 final acceptance\n\n` +
