...
const dogfoodRelative = "dogfood/rm-001-roadmap-management/work-breakdown";
...
const repositoryBytes = read(
  "dogfood/rm-001-roadmap-management/repository-snapshot.json",
);
...
const priorGraphBytes = read(
  "dogfood/sim-001-simplification/work-breakdown/traceability-graph-snapshot.json",
);
const priorGraphReceipt = load(
  "dogfood/sim-001-simplification/work-breakdown/module-execution-record.json",
);
const requirementsPromotionBytes = read(
  "dogfood/rm-001-roadmap-management/requirements-gate-promotion-proof.json",
);
const architecturePromotionBytes = read(
  "dogfood/rm-001-roadmap-management/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofBytes = read(
  "dogfood/sim-001-simplification/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractPromotionBytes = read(
  "dogfood/rm-001-roadmap-management/contract-generation/replay-v1/contract-gate-promotion.json",
);
...
const requirementsPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/rm-001-roadmap-management/requirements-gate-promotion-proof.json",
);
const architecturePromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/rm-001-roadmap-management/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/sim-001-simplification/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/rm-001-roadmap-management/contract-generation/replay-v1/contract-gate-promotion.json",
);

const priorRequirements = load("project/history/2.0.0/requirements-baseline.json");
...
const targetContractIds = contractBaseline.contracts
  .filter(({ interfaceIntentId }) => interfaceIntentId.startsWith("IF-RM-"))
  .map(({ id }) => id)
  .sort();
...
const priorGraphRef = clone(priorGraphReceipt.mergeReceipt.snapshotRef);
assert.equal(
...
assert.equal(traceabilityRefs.length, 12);
...
const packageReviewRef = artifactRef(
  "approved-change-package-review-rm-001-v1",
  {
...
const workItemSpecs = load("dogfood/rm-001-roadmap-management/work-breakdown/work-items.json");
...
const priorAcceptanceBytes = read("dogfood/sim-001-simplification/final-acceptance/26-business-acceptance-record.json");
const priorAcceptanceValue = JSON.parse(priorAcceptanceBytes);
const priorAcceptanceRef = artifactRef(priorAcceptanceValue.recordId, { schema: "https://devrelay.dev/artifacts/business-acceptance-record/v1", mediaType: "application/json" }, priorAcceptanceBytes, "file:///C:/repos/DevRelay/dogfood/sim-001-simplification/final-acceptance/26-business-acceptance-record.json");
...
const coverageDispositions = [
  ...currentBaseline.coverageDispositions.map(({ scopeKind, scopeRef }) => {
    ...
    return {
      ...
      rationale:
        "The prior SIM-001 simplification work is present in the pinned repository snapshot and has an exact approved BusinessAcceptance evidence; it is historical completion evidence, not new RM-001 work.",
...
const changes = [
  ...currentBaseline.workItems.map((item) => ({
    ...
    rationale:
      "The prior SIM-001 simplification deliverable is present in the pinned repository revision and has exact BusinessAcceptance evidence; retain its scope as already satisfied instead of carrying completed work into the RM-001 plan.",
  })),
...
const resultFor = (invocationId) => ({
  ...
  evidence: [
    ...
    },
    {
      ...
      artifact: workBreakdownChangeSetRef,
      summary:
        "Twelve prior accepted SIM-001 items are retired and their approved scope dispositions are preserved as already satisfied with exact repository and BusinessAcceptance evidence.",
    },
  ],
  diagnostics: [],
});
...
const routeRef = artifactRef(
  "module-route-decision-rm-001-v1",
  ROUTE_CONTRACT,
  routeBytes,
...
const invocation = {
  ...
  invocationId: "work-breakdown-decompose-rm-001-v1",
  runId: "rm-001-roadmap-management-dogfood-run-v1",
  ...
  config: {
    ...
    planningOutputRoot:
      "C:/repos/DevRelay/dogfood/rm-001-roadmap-management/work-breakdown",
    toolName: "OpenSpec",
    toolVersion: "0.1.0-fixture",
    changeName: "rm-001-roadmap-management",
...
const checkpointReplayRef = artifactRef(
  "work-breakdown-checkpoint-replay-rm-001-v1",
  {
...
const retiredSimWorkItems = snapshot.nodes.filter(
  ({ kind, state, stableId }) =>
    kind === "work-item" && state === "retired" && stableId.startsWith("WI-SIM-"),
);
assert.equal(activeRmWorkItems.length, workItems.length);
assert.equal(retiredSimWorkItems.length, currentBaseline.workItems.length);
const objectivePath = queryTraceabilityGraph(snapshot, {
  start: {
    kind: "business-objective",
    stableId: "BO-DEV-ROADMAP-001",
    authority: "approved",
...
const runtimeProofRef = artifactRef(
  "work-breakdown-runtime-proof-rm-001-v1",
  {
...
const gateCandidate = {
  ...
  candidateId: "work-breakdown-gate-candidate-rm-001-v1",
...
assert.equal(
  gateCandidateRef.digest,
  "sha256:54fdbe0de9facb6aed00176230ffe9032707735d7b836c2644f947fdd0e40e4c",
);
...
const ownerApprovalSourceRef = artifactRef(
  "work-breakdown-gate-owner-approval-source-rm-001-v1",
  {
...
const ownerApproval = {
  ...
  approvalId: "work-breakdown-gate-owner-approval-rm-001-v1",
  decision: "approved",
...
const promotedProjectState = {
  ...clone(projectState),
  stateId: "PWBS-RM-001-PROMOTED-001",
...
const promotionProof = {
  ...
  proofId: "work-breakdown-gate-promotion-rm-001-v1",
  status: "promoted",
...
const pending = {
  ...
  transactionId: "work-breakdown-promotion-rm-001-v1",
  status: "prepared",
  candidate: workBreakdownChangeSetRef,
  gateCandidate: gateCandidateRef,
  ownerApproval: ownerApprovalRef,
...
writeJson("work-breakdown-dogfood-proof.json", {
  ...
  limitations: [
    ...
    "WorkBreakdown did not assign, schedule, execute, build, verify, integrate, or claim completion of any RM-001 item.",
  ],
});
