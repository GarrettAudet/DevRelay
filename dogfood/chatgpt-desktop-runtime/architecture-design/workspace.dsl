workspace "DevRelay ChatGPT Desktop Windows Runtime" "ChatGPT Desktop Windows release-host target architecture" {
  !identifiers flat
  ...
  model {
    e1 = softwareSystem "DevRelay" "Deterministic orchestration runtime for composable software-engineering modules." {
      ...
      e96 = container "ChatGPT Desktop Windows Host" "Local host boundary that packages DevRelay for ChatGPT Desktop and supervises bounded Codex work-item tasks." "Provider-neutral DevRelay desktop runtime" {
        properties {
          "devrelay.id" "EL-DESKTOP-HOST"
        }
        e97 = component "DevRelay Desktop Plugin" "Repository-owned plugin package with workflow skill and local MCP declaration." "Provider-neutral DevRelay desktop runtime" {
          properties {
            "devrelay.id" "EL-DESKTOP-PLUGIN"
          }
        }
        e98 = component "Typed STDIO MCP Bridge" "Bounded local protocol surface over deterministic Core operations." "Provider-neutral DevRelay desktop runtime" {
          properties {
            "devrelay.id" "EL-DESKTOP-MCP-BRIDGE"
          }
        }
        e99 = component "Codex Task Supervisor" "Host adapter that maps each approved ready work item to one Codex app-server task and immutable attempt." "Provider-neutral DevRelay desktop runtime" {
          properties {
            "devrelay.id" "EL-DESKTOP-TASK-SUPERVISOR"
          }
        }
        e100 = component "Codex App Server Client" "Version-pinned JSON-RPC client for thread, turn, approval, and event lifecycles." "Provider-neutral DevRelay desktop runtime" {
          properties {
            "devrelay.id" "EL-DESKTOP-APP-SERVER-CLIENT"
          }
        }
        e101 = component "Durable Desktop Run Store" "Local content-addressed state for runs, task attempts, checkpoints, raw handoffs, and installation identity." "Provider-neutral DevRelay desktop runtime" {
          properties {
            "devrelay.id" "EL-DESKTOP-RUN-STORE"
          }
        }
        e102 = component "Desktop Capability Resolver" "Resolves the release circuit to evidence-backed live bindings and labels optional alternatives honestly." "Provider-neutral DevRelay desktop runtime" {
          properties {
            "devrelay.id" "EL-DESKTOP-CAPABILITY-RESOLVER"
          }
        }
        e103 = component "Local Marketplace Install Controller" "Validates and manages repository marketplace installation lifecycle on Windows." "Provider-neutral DevRelay desktop runtime" {
          properties {
            "devrelay.id" "EL-DESKTOP-INSTALL-CONTROLLER"
          }
        }
      }
    }
    ...
    e97 -> e98 "Invokes typed local DevRelay tools from the workflow skill." {
      properties {
        "devrelay.id" "REL-DESKTOP-PLUGIN-MCP"
      }
    }
    e98 -> e2 "Submits validated commands while Core selects routes, Gates, and progression." {
      properties {
        "devrelay.id" "REL-DESKTOP-MCP-CORE"
      }
    }
    e25 -> e99 "Provides an exact Core-derived ready frontier and approved assignments." {
      properties {
        "devrelay.id" "REL-DESKTOP-CORE-SUPERVISOR"
      }
    }
    e99 -> e100 "Creates or resumes one isolated Codex task per work-item attempt." {
      properties {
        "devrelay.id" "REL-DESKTOP-SUPERVISOR-APP-SERVER"
      }
    }
    e99 -> e56 "Returns raw attempt observations and handoff bytes to WorkExecution." {
      properties {
        "devrelay.id" "REL-DESKTOP-SUPERVISOR-WE"
      }
    }
    e98 -> e101 "Persists exact command, checkpoint, approval, and response bytes." {
      properties {
        "devrelay.id" "REL-DESKTOP-MCP-STORE"
      }
    }
    e99 -> e101 "Persists thread, turn, attempt, event, and raw handoff identities." {
      properties {
        "devrelay.id" "REL-DESKTOP-SUPERVISOR-STORE"
      }
    }
    e102 -> e2 "Supplies an evidence-backed configured binding set before run creation." {
      properties {
        "devrelay.id" "REL-DESKTOP-CAPABILITY-CORE"
      }
    }
    e103 -> e97 "Installs, upgrades, rolls back, or removes one validated plugin revision." {
      properties {
        "devrelay.id" "REL-DESKTOP-INSTALL-PLUGIN"
      }
    }
    e97 -> e33 "Reads the current sourced human-readable lifecycle report." {
      properties {
        "devrelay.id" "REL-DESKTOP-PLUGIN-REPORT"
      }
    }
  }
  views {
    ...
    container e1 "VIEW-DESKTOP-CONTAINERS" {
      include e2
      include e3
      include e4
      include e11
    ...
    component e96 "VIEW-DESKTOP-HOST-COMPONENTS" {
      include e97
      include e98
      include e99
      include e100
