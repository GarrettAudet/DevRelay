...
export async function promoteDesktopRuntimeArchitecture(context) {
  const {
    root,
    dir,
    read,
  ...
  const ownerApprovalBytes = read(
    "dogfood/chatgpt-desktop-runtime/architecture-design/architecture-gate-owner-approval.json",
  );
  ...
  const gateReviewRef = fileRef(
    "architecture-gate-review-chatgpt-desktop-runtime-v1",
  ...
  const executionProofRef = fileRef(
    "architecture-dogfood-execution-proof-chatgpt-desktop-runtime-v1",
  ...
  const promotedBaseline = {
    ...
    baselineId: "architecture-baseline-devrelay-v1-chatgpt-desktop-runtime-001",
  ...
  const promotedProjectState = {
    ...
    stateId: "project-architecture-state-devrelay-chatgpt-desktop-runtime-approved-v1",
    state: "baselined",
  ...
  const expectedRequiredInterfaces = [
    ...JSON.parse(read("project/architecture-baseline.json")).sections.interfaceIntent.content.interfaces
      .filter(({ contractGeneration }) => contractGeneration?.required === true)
      .map(({ id }) => id),
  ...
  const promotionId = "architecture-promotion-chatgpt-desktop-runtime-v1";
  ...
  fs.writeFileSync(
    path.join(projectDir, "architecture-gate-owner-approval-chatgpt-desktop-runtime-v1.json"),
    ownerApprovalBytes,
  );
