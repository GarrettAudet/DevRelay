...
import { buildDesktopRuntimeArchitecture } from "./desktop-runtime-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/chatgpt-desktop-runtime/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/chatgpt-desktop-runtime/architecture-design";
...
const projectContext = load("dogfood/chatgpt-desktop-runtime/project-context.json");
const repositorySnapshot = load("dogfood/chatgpt-desktop-runtime/repository-snapshot.json");
const architectureBaseline = load("project/history/architecture/architecture-baseline-devrelay-v1-architecture-discovery-001/architecture-baseline.json");
...
const projectContextRef = artifactRef(
  "project-context-devrelay-chatgpt-desktop-runtime-v1",
  "https://devrelay.dev/artifacts/project-context/v1",
  "application/vnd.devrelay.project-context+json",
  sha256(read("dogfood/chatgpt-desktop-runtime/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/chatgpt-desktop-runtime/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  "repository-snapshot-devrelay-6e65cc0",
  "https://devrelay.dev/artifacts/repository-snapshot/v1",
  "application/vnd.devrelay.repository-snapshot+json",
  sha256(read("dogfood/chatgpt-desktop-runtime/repository-snapshot.json")),
...
const architectureBaselineRef = artifactRef(
  ...
  sha256(read("project/history/architecture/architecture-baseline-devrelay-v1-architecture-discovery-001/architecture-baseline.json")),
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-chatgpt-desktop-runtime-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-chatgpt-desktop-runtime-architecture-v1",
...
const {
  ...
  risks,
  uniq,
} = buildDesktopRuntimeArchitecture({
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay ChatGPT Desktop Windows Runtime")} ${dslQuote("ChatGPT Desktop Windows release-host target architecture")} {`,
    "  !identifiers flat",
...
const designRef = fileRef(
  "openspec-design-chatgpt-desktop-runtime-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-chatgpt-desktop-runtime-v1",
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/chatgpt-desktop-runtime/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-DESKTOP-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
  ...
  nativeEntry("NA-DESKTOP-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-DESKTOP-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-DESKTOP-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-DESKTOP-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-DESKTOP-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-DESKTOP-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-DESKTOP-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by the ChatGPT Desktop Windows runtime.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the ChatGPT Desktop Windows target architecture.`,
      compatibilityImpact: "backward-compatible",
      sourceRequirementIds: uniq(
        operation === "remove"
          ? [DESKTOP.runtime, DESKTOP.deterministic]
          : kind === "view"
            ? [DESKTOP.runtime, DESKTOP.deterministic]
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-DESKTOP-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the ChatGPT Desktop Windows architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-chatgpt-desktop-runtime-v1",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-chatgpt-desktop-runtime-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DESKTOP-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-chatgpt-desktop-runtime-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DESKTOP-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/chatgpt-desktop-runtime/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/chatgpt-desktop-runtime/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/chatgpt-desktop-runtime",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-chatgpt-desktop-runtime-v1",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-chatgpt-desktop-runtime-v1",
  runId: "chatgpt-desktop-runtime-architecture-run-v1",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "chatgpt-desktop-runtime", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/chatgpt-desktop-runtime/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/chatgpt-desktop-runtime/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/chatgpt-desktop-runtime/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(projectContextRef, read("dogfood/chatgpt-desktop-runtime/project-context.json"));
register(repositorySnapshotRef, read("dogfood/chatgpt-desktop-runtime/repository-snapshot.json"));
register(architectureBaselineRef, read("project/history/architecture/architecture-baseline-devrelay-v1-architecture-discovery-001/architecture-baseline.json"));
...
if (process.argv.includes("--promote")) {
  const { promoteDesktopRuntimeArchitecture } = await import("./promote.mjs");
  const promotion = await promoteDesktopRuntimeArchitecture({
    root,
    dir,
    read,
    fileRef,
