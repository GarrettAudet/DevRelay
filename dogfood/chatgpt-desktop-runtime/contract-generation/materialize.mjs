...
const RUN_ID = "chatgpt-desktop-runtime-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-chatgpt-desktop-runtime-v1";
const GRAPH_ID = "devrelay/chatgpt-desktop-runtime-contract-change";
...
const architecture = await loadedFile(
  "dogfood/chatgpt-desktop-runtime/architecture-design/architecture-baseline.json",
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== 54) {
  throw new Error(`expected 54 required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-CHATGPT-DESKTOP-RUNTIME-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-CHATGPT-DESKTOP-RUNTIME-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-chatgpt-desktop-runtime-v1",
);
...
const checkpointFile = await exactJsonFile(
  ...
  "application/json",
  "CG-CHECKPOINT-CHATGPT-DESKTOP-RUNTIME-001",
);
  ...
  evidence: [
    {
      ...
      summary: "Core independently validated all 54 native JSON Schema draft 2020-12 artifacts.",
    },
    {
      ...
      summary: "Core computed a deterministic six-add contract diff against the exact 48-contract baseline.",
    },
  ],
  diagnostics: [],
};
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-CHATGPT-DESKTOP-RUNTIME-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-CHATGPT-DESKTOP-RUNTIME-CONTRACT-CHANGE-001",
  ...
  "application/json",
  "CGR-CHATGPT-DESKTOP-RUNTIME-001",
);
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-CHATGPT-DESKTOP-RUNTIME-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
  ...
  "application/json",
  "CGC-CHATGPT-DESKTOP-RUNTIME-001",
);
await exactJsonFile(
  ...
  "application/json",
  "CG-RUNTIME-PROOF-CHATGPT-DESKTOP-RUNTIME-001",
);
