...
if (!workItemId || !expectedCommit || !sourceCommit) {
  throw new Error(
    "usage: node finalize-frontier-integration.mjs <work-item-id> <expected-commit> <source-commit>",
  );
}
...
const integrationRoot = resolve(executionRoot, "integration", workItemId);
...
const subject = readJson(
  `dogfood/chatgpt-desktop-runtime/execution/integration/${workItemId}/verified-subject.json`,
);
const binding = readJson(
  `dogfood/chatgpt-desktop-runtime/execution/integration/${workItemId}/input-binding.json`,
);
const plan = readJson(
  `dogfood/chatgpt-desktop-runtime/execution/integration/${workItemId}/integration-plan.json`,
);
const invocation = readJson(
  `dogfood/chatgpt-desktop-runtime/execution/integration/${workItemId}/adapter-invocation.json`,
);
const authorization = readJson(
  `dogfood/chatgpt-desktop-runtime/execution/integration/${workItemId}/integration-authorization.json`,
);
...
const preRepositorySnapshot = {
  ...
  treeDigest: treeDigest(expectedCommit),
};
...
const observationStored = persistExact(observation, {
  artifactId: `TARGET-OBSERVATION-${slug}-001`,
  schema:
...
const postStored = persistExact(postRepositorySnapshot, {
  artifactId: `REPOSITORY-SNAPSHOT-${slug}-001`,
...
const proofStored = persistExact(incorporationProof, {
  artifactId: `INCORPORATION-PROOF-${slug}-001`,
  schema:
...
).workItems.find(({ id }) => id === workItemId);
const changeSet = readJson(
  `dogfood/chatgpt-desktop-runtime/execution/verification/${workItemId}/change-set-draft.json`,
);
...
const context = {
  ...
  loadedAttachments: {
    ...
    trace: loaded(traceabilityInput, `TRACE-${slug}-001`),
...
const integrationDirectory = resolve(executionRoot, "integration");
const graphCandidates = [];
for (const entry of readdirSync(integrationDirectory, { withFileTypes: true })) {
  if (!entry.isDirectory() || entry.name === workItemId) continue;
  const path = resolve(
    integrationDirectory,
    entry.name,
    "traceability-graph-snapshot.json",
  );
  if (existsSync(path)) graphCandidates.push(readJson(path));
}
const currentGraph =
  graphCandidates.sort((left, right) => right.revision - left.revision)[0] ??
  readJson(
...
const currentGraphRef = ref(
  ...
  api.sha256Digest(currentGraphBytes),
  {
    ...
    uri: `memory://devrelay/traceability/desktop-runtime/approved-r${currentGraph.revision}.json`,
  },
);
...
const graph = api.createTraceabilityGraphService({
  ...
  store,
  contributors: [api.changeIntegrationTraceabilityContributor],
});
...
const gateApproval = readJson(
  `dogfood/chatgpt-desktop-runtime/execution/verification/${workItemId}/gate-approval.json`,
);
const receipt = seal(
  {
    apiVersion: "devrelay.dev/v1alpha1",
    kind: "DesktopWorkItemIntegrationReceipt",
    receiptId: `CI-RECEIPT-${slug}-001`,
    workItemId: workItem.id,
    executionAttemptId: `ATT-DESKTOP-${slug}-001`,
...
const completionBody = {
  completionId: `ICF-DESKTOP-${slug}-001`,
...
const completionFactSetPath = resolve(
  executionRoot,
  "integrated-completion-facts.json",
);
const priorFactSet = existsSync(completionFactSetPath)
  ? JSON.parse(readFileSync(completionFactSetPath, "utf8"))
  : { facts: [] };
const newWorkExecutionFact = {
  workItemId: workItem.id,
  authority: "approved",
...
const workExecutionFacts = [
  ...priorFactSet.facts.filter(({ workItemId: id }) => id !== workItem.id),
  newWorkExecutionFact,
...
for (const entry of readdirSync(integrationDirectory, { withFileTypes: true })) {
  ...
  const path = resolve(
    integrationDirectory,
    entry.name,
    "integrated-completion-fact.json",
  );
  if (existsSync(path)) completionFacts.push(JSON.parse(readFileSync(path, "utf8")));
}
const dependencyBaseline = readJson("project/work-dependency-baseline.json");
const completedWorkItemIds = completionFacts.map(
  ({ workItem }) => workItem.artifactId,
);
...
const readyFrontier = deriveReadyFrontier({
  baseline: dependencyBaseline,
  completionFacts,
  frontierId: `FRONTIER-DESKTOP-AFTER-${slug}-001`,
});
write(executionRoot, "ready-frontier-current.json", readyFrontier);
write(
  executionRoot,
  `ready-frontier-after-${slug.toLowerCase()}.json`,
  readyFrontier,
);
console.log(
  JSON.stringify(
