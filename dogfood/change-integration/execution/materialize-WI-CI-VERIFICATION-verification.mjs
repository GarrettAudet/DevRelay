...
const outDir = resolve(root, "dogfood/change-integration/execution/verification/WI-CI-VERIFICATION");
...
const taskPath = "dogfood/change-integration/execution/task-contracts/WI-CI-VERIFICATION.attempt-009.task.json";
const handoffPath = "dogfood/change-integration/execution/task-contracts/attempts/WI-CI-VERIFICATION.attempt-009.handoff.raw.json";
...
const workItem = breakdown.workItems.find(({ id }) => id === "WI-CI-VERIFICATION");
if (!workItem) throw new Error("approved WI-CI-VERIFICATION is missing");

const integratedFiles = [
  "src/index.mjs",
  "test/change-integration-release.test.mjs",
...
const handoffRef = { artifactId:"WI-CI-VERIFICATION.attempt-009.handoff", digest:api.sha256Digest(handoffBytes), mediaType:"application/json", uri:`file:${handoffPath}` };
const testReport = { apiVersion:"devrelay.dev/v1alpha1", kind:"MachineTestReport", reportId:"TEST-CI-VERIFICATION-009", command:"npm.cmd run verify", outcome:"pass", tests:{ total:612, passed:612, failed:0 }, checkedFiles:integratedFiles, repositoryWideGate:{ outcome:"pass", tests:{ total:612, passed:612, failed:0 }, staticVerification:{ jsonFiles:1479, javascriptModules:286, lfOnlyTextFiles:1973, downstreamOperationsWithProjectOverview:7 } } };
const testReportRef = write("test-report.json", testReport);
const attempt = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"ExecutionAttempt", attemptId:"ATT-CI-VERIFICATION-009", workItemId:workItem.id, invocationFingerprint:api.canonicalJsonDigest(task), bindingDigest:api.canonicalJsonDigest(readJson("dogfood/change-integration/assignment/specialist-assignment-baseline.json")), result:handoffRef, status:"proposed" }, "attemptDigest");
...
const policy = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"VerificationPolicy", policyId:"POL-CI-VERIFICATION-001", version:"1.0.0", evaluationSemantics:"devrelay.work-item-verification/v1", rules:{ allObligationsMandatory:true, requiredEvidenceBinding:"explicit-obligation-bound", verifierIndependence:"required", nativeArtifacts:"provenance-only", unknownEvidence:"reject", outcomePrecedence:["failed","needs-evidence","verified"] } }, "policyDigest");
const { apiVersion:ignoredApi, kind:ignoredKind, policyDigest:ignoredDigest, ...policyBindingArtifact } = policy;
const candidateWorkspace = { apiVersion:"devrelay.dev/v1alpha1", kind:"CandidateWorkspace", version:"1.0.0", workspaceId:"CWS-CI-VERIFICATION-009", files:integratedFiles, workspaceDigest:api.canonicalJsonDigest(integratedFiles) };
const artifacts = {
  workItem,
...
const subject = api.bindWorkItemVerificationSubject({ subjectId:"SUB-CI-VERIFICATION-009", workItemId:workItem.id, bindings });
const obligationSet = api.expandWorkItemVerificationObligations({ subject, workItem, policyDutyRefs:["POL-CI-VERIFICATION-ALL-OBLIGATIONS"] });
...
const binding = api.validateVerifierBindingSet({
  bindingId:"BIND-CI-VERIFICATION-TEST-009", subject, obligationSet,
  ...
  verificationPolicy:{ independenceRequired:true, evidenceKinds:{ "change-integration/release-gate":{ requiredCapabilities:["CAP-TEST-ENGINEERING"], requiredTools:["TOOL-NODE"] } } },
  verifierRegistry:{ entries:[{ verifier, configurationDigest, capabilities:["CAP-TEST-ENGINEERING"], tools:["TOOL-NODE"], supportedEvidenceKinds:["change-integration/release-gate"], permissionDemand:[{ kind:"process.spawn", values:["node"] }], identityAliases:["parent.integration-owner"] }] },
...
const invocation = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"VerifierInvocation", verificationAttemptId:"VAT-CI-VERIFICATION-009", subject:ref(subject.subjectId,subject.subjectDigest), obligationSet:ref(obligationSet.obligationSetId,obligationSet.obligationSetDigest), binding:ref(binding.bindingId,binding.bindingDigest), verifier, obligationIds:binding.partitions[0].obligationIds, assignedObligations:obligationSet.obligations, candidateWorkspace:subject.candidateWorkspace, grants:[{ kind:"process.spawn", values:["node"] }] }, "invocationFingerprint");
const nativeResult = { terminalState:"observed", tests:invocation.assignedObligations.map((obligation) => ({ obligationId:obligation.obligationId, status:"pass", summary:"The canonical DevRelay package release gate passed for the exact candidate: static verification succeeded and all 612 tests passed with zero failures.", evidence:obligation.requiredEvidenceKinds.map((kind) => ({ kind, artifact:testReportRef })) })) };
let adapterCalls = 0;
const checkpoints = checkpointStore();
const controller = api.createWorkItemVerificationCheckpointController({ verifier:async (exactInvocation) => { adapterCalls += 1; const nativeBytes=Buffer.from(api.canonicalJson(nativeResult)); const raw=api.adaptTestVerifierResult({ invocation:exactInvocation, binding, nativeBytes, nativeResult, nativeArtifact:ref("NATIVE-CI-VERIFICATION-TEST-009",api.sha256Digest(nativeBytes)) }); return Buffer.from(api.canonicalJson(raw)); } });
...
const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({ subject, obligationSet, invocation, binding, replay, collectionTime:{ disposition:"not-applicable", rationale:"Deterministic repository verification uses content-addressed evidence." }, normalizedEvidenceId:"NVE-CI-VERIFICATION-009" });
const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId:"WIVC-CI-VERIFICATION-009", policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation:evaluation });
