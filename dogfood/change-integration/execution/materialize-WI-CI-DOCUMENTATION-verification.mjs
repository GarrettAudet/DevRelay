...
const outDir = resolve(root, "dogfood/change-integration/execution/verification/WI-CI-DOCUMENTATION");
...
const taskPath = "dogfood/change-integration/execution/task-contracts/WI-CI-DOCUMENTATION.attempt-001.task.json";
const handoffPath = "dogfood/change-integration/execution/task-contracts/attempts/WI-CI-DOCUMENTATION.attempt-001.handoff.raw.json";
...
const handoffRef = { artifactId:"WI-CI-DOCUMENTATION.attempt-001.handoff", digest:api.sha256Digest(handoffBytes), mediaType:"application/json", uri:`file:${handoffPath}` };
const testReport = { apiVersion:"devrelay.dev/v1alpha1", kind:"MachineTestReport", reportId:"TEST-CI-DOCUMENTATION-001", command:"node --test test/change-integration-documentation.test.mjs", outcome:"pass", tests:{ total:4, passed:4, failed:0 }, checkedFiles:integratedFiles };
const testReportRef = write("test-report.json", testReport);
const attempt = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"ExecutionAttempt", attemptId:"ATT-CI-DOCUMENTATION-001", workItemId:workItem.id, invocationFingerprint:api.canonicalJsonDigest(task), bindingDigest:api.canonicalJsonDigest(readJson("dogfood/change-integration/assignment/specialist-assignment-baseline.json")), result:handoffRef, status:"proposed" }, "attemptDigest");
...
const policy = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"VerificationPolicy", policyId:"POL-CI-DOCUMENTATION-001", version:"1.0.0", evaluationSemantics:"devrelay.work-item-verification/v1", rules:{ allObligationsMandatory:true, requiredEvidenceBinding:"explicit-obligation-bound", verifierIndependence:"required", nativeArtifacts:"provenance-only", unknownEvidence:"reject", outcomePrecedence:["failed","needs-evidence","verified"] } }, "policyDigest");
const { apiVersion:ignoredApi, kind:ignoredKind, policyDigest:ignoredDigest, ...policyBindingArtifact } = policy;
const candidateWorkspace = { apiVersion:"devrelay.dev/v1alpha1", kind:"CandidateWorkspace", version:"1.0.0", workspaceId:"CWS-CI-DOCUMENTATION-001", files:integratedFiles, workspaceDigest:api.canonicalJsonDigest(integratedFiles) };
const artifacts = {
  workItem,
...
const subject = api.bindWorkItemVerificationSubject({ subjectId:"SUB-CI-DOCUMENTATION-001", workItemId:workItem.id, bindings });
...
const binding = api.validateVerifierBindingSet({
  bindingId:"BIND-CI-DOCUMENTATION-TEST-001", subject, obligationSet,
...
const invocation = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"VerifierInvocation", verificationAttemptId:"VAT-CI-DOCUMENTATION-001", subject:ref(subject.subjectId,subject.subjectDigest), obligationSet:ref(obligationSet.obligationSetId,obligationSet.obligationSetDigest), binding:ref(binding.bindingId,binding.bindingDigest), verifier, obligationIds:binding.partitions[0].obligationIds, assignedObligations:obligationSet.obligations, candidateWorkspace:subject.candidateWorkspace, grants:[{ kind:"process.spawn", values:["node"] }] }, "invocationFingerprint");
...
const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({ subject, obligationSet, invocation, binding, replay, collectionTime:{ disposition:"not-applicable", rationale:"Deterministic repository verification uses content-addressed evidence." }, normalizedEvidenceId:"NVE-CI-DOCUMENTATION-001" });
const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId:"WIVC-CI-DOCUMENTATION-001", policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation:evaluation });
