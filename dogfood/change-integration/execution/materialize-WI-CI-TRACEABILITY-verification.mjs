...
const outDir = resolve(root, "dogfood/change-integration/execution/verification/WI-CI-TRACEABILITY");
...
const taskPath = "dogfood/change-integration/execution/task-contracts/WI-CI-TRACEABILITY.attempt-003.task.json";
const handoffPath = "dogfood/change-integration/execution/task-contracts/attempts/WI-CI-TRACEABILITY.attempt-003.handoff.raw.json";
...
const workItem = breakdown.workItems.find(({ id }) => id === "WI-CI-TRACEABILITY");
if (!workItem) throw new Error("approved WI-CI-TRACEABILITY is missing");

const integratedFiles = [
  "src/change-integration-traceability-contributor.mjs",
  "src/index.mjs",
  "src/traceability-artifact-validator.mjs",
  "test/change-integration-traceability.test.mjs",
  "test/traceability-vocabulary-migration.test.mjs",
  "test/work-breakdown-traceability.test.mjs",
...
const handoffRef = { artifactId:"WI-CI-TRACEABILITY.attempt-003.handoff", digest:api.sha256Digest(handoffBytes), mediaType:"application/json", uri:`file:${handoffPath}` };
const testReport = { apiVersion:"devrelay.dev/v1alpha1", kind:"MachineTestReport", reportId:"TEST-CI-TRACEABILITY-003", command:"node --experimental-loader file:///C:/Users/garre/OneDrive/Documents/Portable%20Best%20Practices/.tmp-devrelay-dependency-loader.mjs --test test/change-integration-traceability.test.mjs test/traceability-vocabulary-migration.test.mjs test/traceability-runtime-integration.test.mjs test/traceability-merge.test.mjs test/traceability-diagnostics.test.mjs test/work-breakdown-traceability.test.mjs test/work-dependency-traceability.test.mjs test/specialist-assignment-traceability.test.mjs test/work-item-verification-traceability.test.mjs test/change-integration-result-validator.test.mjs test/change-integration-contracts.test.mjs", outcome:"pass", tests:{ total:79, passed:79, failed:0 }, checkedFiles:integratedFiles };
const testReportRef = write("test-report.json", testReport);
const attempt = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"ExecutionAttempt", attemptId:"ATT-CI-TRACEABILITY-003", workItemId:workItem.id, invocationFingerprint:api.canonicalJsonDigest(task), bindingDigest:api.canonicalJsonDigest(readJson("dogfood/change-integration/assignment/specialist-assignment-baseline.json")), result:handoffRef, status:"proposed" }, "attemptDigest");
...
const policy = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"VerificationPolicy", policyId:"POL-CI-TRACEABILITY-001", version:"1.0.0", evaluationSemantics:"devrelay.work-item-verification/v1", rules:{ allObligationsMandatory:true, requiredEvidenceBinding:"explicit-obligation-bound", verifierIndependence:"required", nativeArtifacts:"provenance-only", unknownEvidence:"reject", outcomePrecedence:["failed","needs-evidence","verified"] } }, "policyDigest");
const { apiVersion:ignoredApi, kind:ignoredKind, policyDigest:ignoredDigest, ...policyBindingArtifact } = policy;
const candidateWorkspace = { apiVersion:"devrelay.dev/v1alpha1", kind:"CandidateWorkspace", version:"1.0.0", workspaceId:"CWS-CI-TRACEABILITY-003", files:integratedFiles, workspaceDigest:api.canonicalJsonDigest(integratedFiles) };
const artifacts = {
  workItem,
...
const subject = api.bindWorkItemVerificationSubject({ subjectId:"SUB-CI-TRACEABILITY-003", workItemId:workItem.id, bindings });
const obligationSet = api.expandWorkItemVerificationObligations({ subject, workItem, policyDutyRefs:["POL-CI-TRACEABILITY-ALL-OBLIGATIONS"] });
...
const binding = api.validateVerifierBindingSet({
  bindingId:"BIND-CI-TRACEABILITY-TEST-003", subject, obligationSet,
  executorIdentity:"executor.codex-task.019fe2bc-233e-70a1-829f-ad41b117fac0",
  changeProducerIdentities:["producer.codex-task.019fe2bc-233e-70a1-829f-ad41b117fac0"],
  verificationPolicy:{ independenceRequired:true, evidenceKinds:{ "change-integration/traceability-tests":{ requiredCapabilities:["CAP-TEST-ENGINEERING"], requiredTools:["TOOL-NODE"] } } },
  verifierRegistry:{ entries:[{ verifier, configurationDigest, capabilities:["CAP-TEST-ENGINEERING"], tools:["TOOL-NODE"], supportedEvidenceKinds:["change-integration/traceability-tests"], permissionDemand:[{ kind:"process.spawn", values:["node"] }], identityAliases:["parent.integration-owner"] }] },
...
const invocation = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"VerifierInvocation", verificationAttemptId:"VAT-CI-TRACEABILITY-003", subject:ref(subject.subjectId,subject.subjectDigest), obligationSet:ref(obligationSet.obligationSetId,obligationSet.obligationSetDigest), binding:ref(binding.bindingId,binding.bindingDigest), verifier, obligationIds:binding.partitions[0].obligationIds, assignedObligations:obligationSet.obligations, candidateWorkspace:subject.candidateWorkspace, grants:[{ kind:"process.spawn", values:["node"] }] }, "invocationFingerprint");
...
const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({ subject, obligationSet, invocation, binding, replay, collectionTime:{ disposition:"not-applicable", rationale:"Deterministic repository verification uses content-addressed evidence." }, normalizedEvidenceId:"NVE-CI-TRACEABILITY-003" });
const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId:"WIVC-CI-TRACEABILITY-003", policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation:evaluation });
