...
const outDir = resolve(root, "dogfood/change-integration/execution/verification/WI-CI-LOCAL-GIT-ADAPTER");
...
const taskPath = "dogfood/change-integration/execution/task-contracts/WI-CI-LOCAL-GIT-ADAPTER.attempt-003.task.json";
const handoffPath = "dogfood/change-integration/execution/task-contracts/attempts/WI-CI-LOCAL-GIT-ADAPTER.attempt-003.handoff.raw.json";
...
const workItem = breakdown.workItems.find(({ id }) => id === "WI-CI-LOCAL-GIT-ADAPTER");
if (!workItem) throw new Error("approved WI-CI-LOCAL-GIT-ADAPTER is missing");

const integratedFiles = [
  "src/change-integration-local-git-adapter.mjs",
  "examples/plugins/local-git-integration.plugin.json",
  "test/change-integration-local-git-adapter.test.mjs",
...
const handoffRef = { artifactId:"WI-CI-LOCAL-GIT-ADAPTER.attempt-003.handoff", digest:api.sha256Digest(handoffBytes), mediaType:"application/json", uri:`file:${handoffPath}` };
const testReport = { apiVersion:"devrelay.dev/v1alpha1", kind:"MachineTestReport", reportId:"TEST-CI-LOCAL-GIT-ADAPTER-003", command:"node --experimental-loader file:///C:/Users/garre/OneDrive/Documents/Portable%20Best%20Practices/.tmp-devrelay-dependency-loader.mjs --test test/change-integration-local-git-adapter.test.mjs test/change-integration-target-cas.test.mjs test/change-integration-input-plan.test.mjs test/change-integration-contracts.test.mjs", outcome:"pass", tests:{ total:40, passed:40, failed:0 }, checkedFiles:integratedFiles };
const testReportRef = write("test-report.json", testReport);
const attempt = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"ExecutionAttempt", attemptId:"ATT-CI-LOCAL-GIT-ADAPTER-003", workItemId:workItem.id, invocationFingerprint:api.canonicalJsonDigest(task), bindingDigest:api.canonicalJsonDigest(readJson("dogfood/change-integration/assignment/specialist-assignment-baseline.json")), result:handoffRef, status:"proposed" }, "attemptDigest");
...
const subject = api.bindWorkItemVerificationSubject({ subjectId:"SUB-CI-LOCAL-GIT-ADAPTER-003", workItemId:workItem.id, bindings });
const obligationSet = api.expandWorkItemVerificationObligations({ subject, workItem, policyDutyRefs:["POL-CI-LOCAL-GIT-ADAPTER-ALL-OBLIGATIONS"] });
...
const binding = api.validateVerifierBindingSet({
  bindingId:"BIND-CI-LOCAL-GIT-ADAPTER-TEST-003", subject, obligationSet,
  executorIdentity:"executor.codex-task.019fde3a-7a99-78f3-8326-e077e319b3da",
  changeProducerIdentities:["producer.codex-task.019fde3a-7a99-78f3-8326-e077e319b3da"],
  verificationPolicy:{ independenceRequired:true, evidenceKinds:{ "change-integration/local-git-adapter-conformance":{ requiredCapabilities:["CAP-TEST-ENGINEERING"], requiredTools:["TOOL-NODE"] } } },
  verifierRegistry:{ entries:[{ verifier, configurationDigest, capabilities:["CAP-TEST-ENGINEERING"], tools:["TOOL-NODE"], supportedEvidenceKinds:["change-integration/local-git-adapter-conformance"], permissionDemand:[{ kind:"process.spawn", values:["node"] }], identityAliases:["parent.integration-owner"] }] },
...
const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({ subject, obligationSet, invocation, binding, replay, collectionTime:{ disposition:"not-applicable", rationale:"Deterministic repository verification uses content-addressed evidence." }, normalizedEvidenceId:"NVE-CI-LOCAL-GIT-ADAPTER-003" });
const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId:"WIVC-CI-LOCAL-GIT-ADAPTER-003", policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation:evaluation });
