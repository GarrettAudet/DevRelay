...
const outDir = resolve(root, "dogfood/change-integration/execution/verification/WI-CI-CONTRACTS/attempt-005");
...
const taskPath = "dogfood/change-integration/execution/task-contracts/WI-CI-CONTRACTS.attempt-005.task.json";
const handoffPath = "dogfood/change-integration/execution/task-contracts/attempts/WI-CI-CONTRACTS.attempt-005.handoff.raw.json";
...
if (mutations.some(({ beforeDigest }) => !beforeDigest)) throw new Error("attempt-005 must bind every prior integrated file digest");
const handoffRef = { artifactId:"WI-CI-CONTRACTS.attempt-005.handoff", digest:api.sha256Digest(handoffBytes), mediaType:"application/json", uri:`file:${handoffPath}` };
const testReport = { apiVersion:"devrelay.dev/v1alpha1", kind:"MachineTestReport", reportId:"TEST-CI-CONTRACTS-005", command:"node --experimental-loader file:///C:/Users/garre/OneDrive/Documents/Portable%20Best%20Practices/.tmp-devrelay-dependency-loader.mjs --test test/change-integration-contracts.test.mjs", outcome:"pass", tests:{ total:13, passed:13, failed:0 }, checkedFiles:integratedFiles };
const testReportRef = write("test-report.json", testReport);
const attempt = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"ExecutionAttempt", attemptId:"ATT-CI-CONTRACTS-005", workItemId:workItem.id, invocationFingerprint:api.canonicalJsonDigest(task), bindingDigest:api.canonicalJsonDigest(readJson("dogfood/change-integration/assignment/specialist-assignment-baseline.json")), result:handoffRef, status:"proposed" }, "attemptDigest");
...
const candidateWorkspace = { apiVersion:"devrelay.dev/v1alpha1", kind:"CandidateWorkspace", version:"1.0.0", workspaceId:"CWS-CI-CONTRACTS-005", files:integratedFiles, workspaceDigest:api.canonicalJsonDigest(integratedFiles) };
const artifacts = {
  workItem,
...
const subject = api.bindWorkItemVerificationSubject({ subjectId:"SUB-CI-CONTRACTS-005", workItemId:workItem.id, bindings });
...
const binding = api.validateVerifierBindingSet({
  ...
  executorIdentity:"executor.codex-task.019fe35d-f963-7c62-b2a8-979c8a78c335",
  changeProducerIdentities:["producer.codex-task.019fe35d-f963-7c62-b2a8-979c8a78c335"],
...
const invocation = seal({ apiVersion:"devrelay.dev/v1alpha1", kind:"VerifierInvocation", verificationAttemptId:"VAT-CI-CONTRACTS-005", subject:ref(subject.subjectId,subject.subjectDigest), obligationSet:ref(obligationSet.obligationSetId,obligationSet.obligationSetDigest), binding:ref(binding.bindingId,binding.bindingDigest), verifier, obligationIds:binding.partitions[0].obligationIds, assignedObligations:obligationSet.obligations, candidateWorkspace:subject.candidateWorkspace, grants:[{ kind:"process.spawn", values:["node"] }] }, "invocationFingerprint");
...
const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({ subject, obligationSet, invocation, binding, replay, collectionTime:{ disposition:"not-applicable", rationale:"Deterministic repository verification uses content-addressed evidence." }, normalizedEvidenceId:"NVE-CI-CONTRACTS-005" });
const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId:"WIVC-CI-CONTRACTS-005", policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation:evaluation });
