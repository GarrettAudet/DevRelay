...
import { buildChangeIntegrationArchitecture } from "./change-integration-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/change-integration/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/change-integration/architecture-design";
...
const projectContext = load("dogfood/change-integration/project-context.json");
const repositorySnapshot = load("dogfood/change-integration/repository-snapshot.json");
// ChangeIntegration is designed against the exact current WorkItemVerification architecture baseline.
const architectureBaselineSourcePath = "project/architecture-baseline.json";
...
const projectContextRef = artifactRef(
  "project-context-devrelay-change-integration-v1",
  "https://devrelay.dev/artifacts/project-context/v1",
  "application/vnd.devrelay.project-context+json",
  sha256(read("dogfood/change-integration/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/change-integration/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  ...
  sha256(read("dogfood/change-integration/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/change-integration/repository-snapshot.json",
);
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-change-integration-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-change-integration-architecture-v1",
...
const {
  ...
  risks,
  uniq,
} = buildChangeIntegrationArchitecture({
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay ChangeIntegration")} ${dslQuote("Evidence-bound verification and Gate target architecture")} {`,
    "  !identifiers flat",
...
const designRef = fileRef(
  "openspec-design-change-integration-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-change-integration-v1",
...
const diagrams = {
  diagramSetId: "DIAGRAMS-DEVRELAY-CI-005",
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/change-integration/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-CI-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
    { section: "technicalDesign", entityIds: [technicalDesign.technicalDesignId], jsonPointers: ["/sections/technicalDesign"] },
    { section: "interfaceIntent", entityIds: interfaces.filter(({ id }) => id.startsWith("IF-CI-")).map(({ id }) => id), jsonPointers: ["/sections/interfaceIntent"] },
    { section: "architectureConstraints", entityIds: constraints.filter(({ id }) => id.startsWith("CON-CI-")).map(({ id }) => id), jsonPointers: ["/sections/architectureConstraints"] },
  ]),
  nativeEntry("NA-CI-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-CI-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-CI-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-CI-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-CI-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-CI-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-CI-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by bounded change integration.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the bounded change-integration target architecture.`,
      compatibilityImpact: "backward-compatible",
      sourceRequirementIds: uniq(
        operation === "remove"
          ? [CG.integrate, CG.auditability]
          : kind === "view"
            ? [CG.integrate, CG.auditability]
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-CI-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the bounded change-integration architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-change-integration-v1",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-change-integration-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-CI-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-change-integration-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-CI-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/change-integration/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/change-integration/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/change-integration",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-change-integration-v1",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-change-integration-v1",
  runId: "change-integration-architecture-run-v1",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "change-integration", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/change-integration/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/change-integration/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/change-integration/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(projectContextRef, read("dogfood/change-integration/project-context.json"));
register(repositorySnapshotRef, read("dogfood/change-integration/repository-snapshot.json"));
register(architectureBaselineRef, read(architectureBaselineSourcePath));
register(architectureBaselineProjectContextRef, read("dogfood/work-item-verification/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/work-item-verification/repository-snapshot.json"));
...
if (process.argv.includes("--promote")) {
  const { promoteChangeIntegrationArchitecture } = await import("./promote.mjs");
  const promotion = await promoteChangeIntegrationArchitecture({
    root,
    dir,
    read,
    fileRef,
