...
const RUN_ID = "change-integration-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-change-integration-v1";
const GRAPH_ID = "devrelay/change-integration-contract-change";
...
architecture.ref = structuredClone(projectArchitectureState.architectureBaseline);
...
const persistedContractBaseline = await loadedFile(
...
const persistedContractState = JSON.parse(
  await readFile(new URL("project/project-contract-state.json", ROOT), "utf8"),
);
let currentContractBaseline = persistedContractBaseline;
if (persistedContractBaseline.value.baselineId === "CB-DEVRELAY-005") {
  currentContractBaseline = await loadedFile(
    "project/history/contracts/CB-DEVRELAY-004/contract-baseline.json",
    "https://devrelay.dev/artifacts/contract-baseline/v1",
    "application/vnd.devrelay.contract-baseline+json",
    "baselineId",
  );
  currentContractBaseline.ref = structuredClone(
    persistedContractBaseline.value.supersedes,
  );
} else {
  currentContractBaseline.ref = structuredClone(
    persistedContractState.contractBaseline,
  );
}
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== 41) {
  throw new Error(`expected 41 required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-CHANGE-INTEGRATION-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-CHANGE-INTEGRATION-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-change-integration-v1",
);
...
const checkpointFile = await exactJsonFile(
  ...
  "application/json",
  "CG-CHECKPOINT-CHANGE-INTEGRATION-001",
);
  ...
  evidence: [
    {
      ...
      summary: "Core independently validated all 41 native JSON Schema draft 2020-12 artifacts.",
    },
    {
      ...
      summary: "Core computed a deterministic nine-add contract diff against the exact 25-contract baseline.",
    },
  ],
  diagnostics: [],
};
...
const upstreamPrepared = await graph.prepare({
  graphId: GRAPH_ID,
  projectId: PROJECT_ID,
...
await graph.mergePrepared(upstreamPrepared);
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-CHANGE-INTEGRATION-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-CHANGE-INTEGRATION-CONTRACT-CHANGE-001",
...
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-CHANGE-INTEGRATION-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
...
await exactJsonFile(
  ...
  "application/json",
  "CG-RUNTIME-PROOF-CHANGE-INTEGRATION-001",
);
