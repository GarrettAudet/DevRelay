...
const integrationSummary = readJson(
  "dogfood/ep-001-environment-preparation/integration-frontier-4/integration-summary.json",
);
...
const completionsValue = readJson(
  "dogfood/ep-001-environment-preparation/frontier-5/integrated-completion-fact-set.json",
);
const completions = canonicalLoaded(
  completionsValue,
  "ICFS-EP-001-FRONTIER-5",
...
const jobs = [
  { workItemId: "WI-EP-DESKTOP-INTEGRATION", attemptId: "ATT-EP-DESKTOP-INTEGRATION-001", files: ["README.md", "docs/environment-preparation.md", "examples/modules/environment-preparation.module.json", "examples/plugins/native-windows-environment-inventory.plugin.json", "package.json", "src/environment-preparation-desktop.mjs", "src/index.mjs", "test/environment-preparation-desktop.test.mjs", "test/environment-preparation-module-surface.test.mjs"], command: "node --test test/environment-preparation-desktop.test.mjs test/environment-preparation-module-surface.test.mjs", tests: 7 },
  { workItemId: "WI-EP-TRACEABILITY", attemptId: "ATT-EP-TRACEABILITY-001", files: ["src/environment-preparation-traceability-contributor.mjs", "src/traceability-artifact-validator.mjs", "src/traceability-graph.mjs", "test/environment-preparation-traceability.test.mjs"], command: "node --test test/environment-preparation-traceability.test.mjs", tests: 5 },
];
const permissions = [
  { kind: "filesystem.write", scope: { values: ["README.md", "docs/**", "examples/**", "package.json", "src/**", "test/**"] } },
  { kind: "process.spawn", scope: { values: ["node"] } },
];

const graphHeadPath =
  "dogfood/ep-001-environment-preparation/integration-frontier-4/WI-EP-OPTIONAL-ADAPTERS/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "Ep001FrontierFiveExecutionSummary",
