...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-frontier-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-ARTIFACT-CONTRACTS": Object.freeze({
    argv: ["--test", "test/environment-preparation-artifacts.test.mjs"],
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-frontier-1/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} EP-001 first-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001FirstFrontierVerificationSummary", results };
