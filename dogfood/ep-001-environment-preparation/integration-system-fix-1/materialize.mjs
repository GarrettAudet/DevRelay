...
const outRoot = resolve(root, "dogfood/ep-001-environment-preparation/integration-system-fix-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/ep-001-integration-system-fix-1-retry-2";
const sourceRef = "refs/heads/codex/ep-001-source-system-fix-1-retry-2";
...
const baseCommit = json("dogfood/ep-001-environment-preparation/integration-frontier-7/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/ep-001-environment-preparation/execution-system-fix-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/ep-001-environment-preparation/execution-system-fix-1/execution-summary.json");
let graphSnapshot = json("dogfood/ep-001-environment-preparation/execution-system-fix-1/WI-EP-TRACEABILITY/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/ep-001-environment-preparation/verification-system-fix-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/ep-001-environment-preparation/execution-system-fix-1/${workItemId}/change-set-draft.json`);
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-EP001-${workItemId}-REPAIR-001`,
  });
  ...
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-${workItemId}-REPAIR-001`,
...
const summary = {
  apiVersion: API,
  kind: "Ep001SystemVerificationRepairIntegrationSummary",
  targetRef,
  baseCommit,
