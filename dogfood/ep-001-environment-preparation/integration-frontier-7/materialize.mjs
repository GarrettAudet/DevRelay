...
const outRoot = resolve(root, "dogfood/ep-001-environment-preparation/integration-frontier-7");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/ep-001-integration-frontier-7";
const sourceRef = "refs/heads/codex/ep-001-source-frontier-7";
...
const baseCommit = json("dogfood/ep-001-environment-preparation/integration-frontier-6/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/ep-001-environment-preparation/execution-frontier-7/${workItemId}/change-set-draft.json`);
    ...
    git(sourceRepo, "commit", "-m", `EP-001 ${workItemId}`);
...
const graphSummary = json("dogfood/ep-001-environment-preparation/execution-frontier-7/execution-summary.json");
let graphSnapshot = json("dogfood/ep-001-environment-preparation/execution-frontier-7/WI-EP-WINDOWS-E2E/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/ep-001-environment-preparation/verification-frontier-7/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/ep-001-environment-preparation/execution-frontier-7/${workItemId}/change-set-draft.json`);
  ...
  const targetSnapshot = {
    ...
    includedPaths: ["README.md", "docs/**", "dogfood/ep-001-environment-preparation/windows-e2e/**", "examples/**", "package.json", "src/**", "test/**"],
    excludedPaths: [".git/**", "node_modules/**"],
  };
  const targetReference = ref(`REPO-EP001-INTEGRATION-${currentCommit.slice(0, 12).toUpperCase()}`, api.canonicalJsonDigest(targetSnapshot));
  const integrationPolicy = { kind: "IntegrationPolicy", version: "1.0.0", mode: "verified-fast-forward", targetRef };
  const integrationPolicyBytes = Buffer.from(`DevRelay EP-001 verified fast-forward policy for ${targetRef}\n`, "utf8");
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-EP001-${workItemId}`,
  });
...
const summary = {
  apiVersion: API,
  kind: "Ep001SeventhFrontierIntegrationSummary",
  targetRef,
  baseCommit,
