...
const repositoryRevision = execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
...
writeJson("repository-snapshot.json", repository.value);
...
const jobs = [
  { workItemId: "WI-EP-REGRESSION-PERFORMANCE", attemptId: "ATT-EP-REGRESSION-PERFORMANCE-004", files: ["test/v0.11-final-acceptance.test.mjs"], command: "node --test test/v0.11-final-acceptance.test.mjs", tests: 4 },
];
const permissions = [
  { kind: "filesystem.write", scope: { values: ["README.md", "docs/**", "dogfood/**", "examples/**", "package.json", "src/**", "test/**"] } },
...
const summary = {
  apiVersion: API,
  kind: "Ep001ReleaseProvenanceRepairExecutionSummary",
...
for (const job of jobs) {
  ...
  const moduleInvocation = {
    ...
    invocationId: `WE-EP-001-${job.attemptId}`,
    module: {
      id: "work-execution",
      version: "0.1.0",
