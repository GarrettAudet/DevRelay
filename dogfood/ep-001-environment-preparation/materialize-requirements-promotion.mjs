...
if (
  ...
  throw new Error("EP-001 RequirementsGate candidate bindings or pre-approval status changed.");
}
if (
  ...
  throw new Error("EP-001 requirements closure is not promotable.");
}
...
for (const source of nativeBundleDocument.value.sources) {
  const relativePath = source.path.replace("dogfood/ep-001-environment-preparation/", "");
...
const approval = Object.freeze({
  ...
  approvalId: "requirements-gate-approval-ep-001-environment-preparation-v1",
  authority: "project-owner",
  decision: "approve",
  source: {
    channel: "chat",
    statement: "I see, I approve all; standing approval applies to routine in-scope lifecycle gates after exact validation.",
  },
...
const requirementsBaseline = Object.freeze({
  ...
  baselineId: "requirements-baseline-devrelay-v1-ep-001-environment-preparation-001",
  version: "2.3.0",
...
const projectOverviewBaseline = Object.freeze({
  ...
  baselineId: "project-overview-baseline-devrelay-v1-ep-001-environment-preparation-001",
  version: "2.3.0",
...
await mkdir(new URL("history/2.2.0/", project), { recursive: true });
await Promise.all([
  writeFile(new URL("history/2.2.0/requirements-baseline.json", project), previousRequirementsDocument.bytes),
  writeFile(new URL("history/2.2.0/project-overview-baseline.json", project), previousOverviewDocument.bytes),
  writeFile(new URL("history/2.2.0/ProjectOverview.md", project), previousOverviewMarkdownBytes),
...
const pendingPromotion = Object.freeze({
  ...
  promotionId: "requirements-promotion-ep-001-environment-preparation-v1",
  state: "prepared",
...
await writeFile(new URL("requirements-gate-ep-001-environment-preparation-v1.md", project), Buffer.from(approvalText.normalize("NFC"), "utf8"));

const promotionProof = Object.freeze({
  apiVersion: "devrelay.dev/v1alpha1",
  kind: "RequirementsGatePromotionProof",
  proofId: "requirements-promotion-ep-001-environment-preparation-v1",
  status: "pass",
