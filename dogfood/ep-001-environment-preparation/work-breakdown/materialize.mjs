...
const dogfoodRelative = "dogfood/ep-001-environment-preparation/work-breakdown";
...
const repositoryBytes = read(
  "dogfood/ep-001-environment-preparation/repository-snapshot.json",
);
...
const priorGraphBytes = read(
  "dogfood/ep-001-environment-preparation/contract-generation/replay-v1/approved-traceability-graph-snapshot.json",
);
...
const priorWorkBreakdownProofBytes = read(
  "dogfood/pm-001-project-memory/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractInvocation = load(
  "dogfood/ep-001-environment-preparation/contract-generation/contract-generation.invocation.json",
);
const contractResult = load(
  "dogfood/ep-001-environment-preparation/contract-generation/contract-generation.result.json",
);
const contractStateBytes = read(
  "dogfood/ep-001-environment-preparation/contract-generation/project-contract-state.json",
);
...
const requirementsPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/ep-001-environment-preparation/requirements-gate-promotion-proof.json",
);
const architecturePromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/ep-001-environment-preparation/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/ep-001-environment-preparation/contract-generation/replay-v1/contract-gate-promotion.json",
);

const priorRequirements = load("project/history/2.2.0/requirements-baseline.json");
...
const targetContractIds = contractBaseline.contracts
  .filter(({ interfaceIntentId }) => interfaceIntentId.startsWith("IF-EP-"))
  .map(({ id }) => id)
  .sort();
...
assert.equal(authorizedContractIds.length, priorContractIds.length + 7);
...
const packageReviewRef = artifactRef(
  "approved-change-package-review-ep-001-v1",
  {
...
const workItemSpecs = load("dogfood/ep-001-environment-preparation/work-breakdown/work-items.json");
...
const priorAcceptanceBytes = read("dogfood/pm-001-project-memory/final-acceptance/26-business-acceptance-record.json");
const priorAcceptanceValue = JSON.parse(priorAcceptanceBytes);
const priorAcceptanceRef = artifactRef(priorAcceptanceValue.recordId, { schema: "https://devrelay.dev/artifacts/business-acceptance-record/v1", mediaType: "application/json" }, priorAcceptanceBytes, "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/final-acceptance/26-business-acceptance-record.json");
...
const coverageDispositions = [
  ...currentBaseline.coverageDispositions.map(({ scopeKind, scopeRef }) => {
    ...
    return {
      ...
      rationale:
        "The prior PM-001 project-memory work is present in the pinned repository snapshot and has an exact approved BusinessAcceptance evidence; it is historical completion evidence, not new EP-001 work.",
...
const changes = [
  ...currentBaseline.workItems.map((item) => ({
    ...
    rationale:
      "The prior PM-001 project-memory deliverable is present in the pinned repository revision and has exact BusinessAcceptance evidence; retain its scope as already satisfied instead of carrying completed work into the EP-001 plan.",
  })),
...
const resultFor = (invocationId) => ({
  ...
  evidence: [
    ...
    },
    {
      ...
      artifact: workBreakdownChangeSetRef,
      summary:
        "Nine prior accepted PM-001 items are retired and their approved scope dispositions are preserved as already satisfied with exact repository and BusinessAcceptance evidence.",
    },
  ],
  diagnostics: [],
});
...
const routeRef = artifactRef(
  "module-route-decision-ep-001-v1",
  ROUTE_CONTRACT,
  routeBytes,
...
const invocation = {
  ...
  invocationId: "work-breakdown-decompose-ep-001-v1",
  runId: "ep-001-environment-preparation-dogfood-run-v1",
  ...
  config: {
    ...
    planningOutputRoot:
      "C:/repos/DevRelay/dogfood/ep-001-environment-preparation/work-breakdown",
    toolName: "OpenSpec",
    toolVersion: "0.1.0-fixture",
    changeName: "ep-001-environment-preparation",
...
const checkpointReplayRef = artifactRef(
  "work-breakdown-checkpoint-replay-ep-001-v1",
  {
...
const objectivePath = queryTraceabilityGraph(snapshot, {
  start: {
    ...
    stableId: "BO-DEV-ENVIRONMENT-READINESS-001",
    authority: "approved",
...
const runtimeProofRef = artifactRef(
  "work-breakdown-runtime-proof-ep-001-v1",
  {
...
const gateCandidate = {
  ...
  candidateId: "work-breakdown-gate-candidate-ep-001-v1",
...
const ownerApprovalSourceRef = artifactRef(
  "work-breakdown-gate-owner-approval-source-ep-001-v1",
  {
...
const ownerApproval = {
  ...
  approvalId: "work-breakdown-gate-owner-approval-ep-001-v1",
  decision: "approved",
...
const promotedProjectState = {
  ...clone(projectState),
  stateId: "PWBS-EP-001-PROMOTED-001",
...
const promotionProof = {
  ...
  proofId: "work-breakdown-gate-promotion-ep-001-v1",
  status: "promoted",
...
const pending = {
  ...
  transactionId: "work-breakdown-promotion-ep-001-v1",
  status: "prepared",
  candidate: workBreakdownChangeSetRef,
  gateCandidate: gateCandidateRef,
  ownerApproval: ownerApprovalRef,
...
writeJson("work-breakdown-dogfood-proof.json", {
  ...
  limitations: [
    ...
    "WorkBreakdown did not assign, schedule, execute, build, verify, integrate, or claim completion of any EP-001 item.",
  ],
});
