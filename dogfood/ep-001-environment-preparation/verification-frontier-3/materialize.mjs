...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-frontier-3");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-PREPARATION-EFFECTS": Object.freeze({ argv: ["--test", "test/environment-preparation-effects.test.mjs"], command: "node --test test/environment-preparation-effects.test.mjs", tests: 9 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-frontier-2/WI-EP-PROFILE-INVENTORY/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-frontier-3/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} EP-001 third-frontier focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001ThirdFrontierVerificationSummary", results };
