...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-frontier-6");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-REGRESSION-PERFORMANCE": Object.freeze({ argv: ["--test", "test/environment-preparation-regression-performance.test.mjs"], command: "node --test test/environment-preparation-regression-performance.test.mjs", tests: 6 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-frontier-5/WI-EP-TRACEABILITY/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-frontier-6/${workItemId}`;
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001SixthFrontierVerificationSummary", results };
