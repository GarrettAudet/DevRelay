...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-release-security-fix-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-WINDOWS-E2E": Object.freeze({ argv: ["--test", "test/environment-preparation-windows-e2e.test.mjs"], command: "node --test test/environment-preparation-windows-e2e.test.mjs", tests: 5 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-system-fix-2/WI-EP-REGRESSION-PERFORMANCE/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-release-security-fix-1/${workItemId}`;
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001ReleaseSecurityRepairVerificationSummary", results };
