...
const priorGraph = read(
  "../dependency-analysis/promotion/traceability-graph-snapshot.json",
);
const priorProof = read(
  "../dependency-analysis/promotion/work-dependency-gate-promotion-proof.json",
);
...
function sortedJsonFiles(rootUrl) {
  const files = [];
  function visit(currentUrl) {
    for (const entry of readdirSync(currentUrl, { withFileTypes: true }).sort(
...
const wantedDigests = new Set(
  priorGraph.appliedUpdates.map(({ digest }) => digest),
);
for (const searchRoot of [
  new URL("../../../dogfood/", import.meta.url),
  new URL("../../../project/history/", import.meta.url),
]) {
  for (const sourceUrl of sortedJsonFiles(searchRoot)) {
    let update;
    try {
      update = JSON.parse(readFileSync(sourceUrl, "utf8"));
    } catch {
      continue;
    }
    if (update.kind !== "TraceabilityUpdate") continue;
    const bytes = Buffer.from(canonicalJson(update), "utf8");
    const digest = sha256Digest(bytes);
    if (wantedDigests.has(digest) && !historicalByDigest.has(digest)) {
      historicalByDigest.set(digest, { bytes, value: update });
    }
  }
}
const historicalUpdates = priorGraph.appliedUpdates.map((ref) => {
  const source = historicalByDigest.get(ref.digest);
  if (!source) {
    throw new Error(`historical update ${ref.digest} has no source locator`);
  }
  if (sha256Digest(source.bytes) !== ref.digest) {
    throw new Error(`historical update ${ref.digest} bytes are unavailable`);
  }
  return { ref, bytes: source.bytes, value: source.value };
});store.restore(
  priorGraph.graphId,
...
const service = createTraceabilityGraphService({
  ...
  vocabulary: priorGraph.vocabulary,
});
...
async function merge({ invocation, outcome, name, artifact }) {
  ...
  if (canonicalJsonDigest(merged.snapshot.vocabulary) !== canonicalJsonDigest(priorGraph.vocabulary)) {
    throw new Error("assignment promotion must preserve the predecessor graph vocabulary");
  }
...
const draft = loaded(
  ...
  draftValue.draftId,
  "devrelay-repo:///dogfood/ep-001-environment-preparation/assignment/specialist-assignment-draft.json",
);
const candidate = await merge({
  invocation: {
    invocationId: "SA-EP-001-001",
    module: {
...
const approved = await merge({
  invocation: {
    invocationId: "SA-GATE-EP-001-001",
    module: {
