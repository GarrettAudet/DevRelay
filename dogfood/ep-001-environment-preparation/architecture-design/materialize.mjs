...
import { buildEnvironmentArchitecture } from "./environment-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/ep-001-environment-preparation/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/ep-001-environment-preparation/architecture-design";
...
const projectContext = load("dogfood/ep-001-environment-preparation/project-context.json");
const repositorySnapshot = load("dogfood/ep-001-environment-preparation/repository-snapshot.json");
const architectureBaseline = load(
  "project/architecture-baseline.json",
);
const ownerDecisionPath = "docs/specs/ep-001-environment-preparation/owner-architecture-decisions.json";
...
const projectContextRef = artifactRef(
  "project-context-devrelay-ep-001-environment-preparation-v1",
  "https://devrelay.dev/artifacts/project-context/v1",
  "application/vnd.devrelay.project-context+json",
  sha256(read("dogfood/ep-001-environment-preparation/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/ep-001-environment-preparation/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  "repository-snapshot-devrelay-a18e6fb-ep-001-environment-preparation",
  "https://devrelay.dev/artifacts/repository-snapshot/v1",
  "application/vnd.devrelay.repository-snapshot+json",
  sha256(read("dogfood/ep-001-environment-preparation/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/ep-001-environment-preparation/repository-snapshot.json",
);
...
const ownerDecisionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/docs/specs/ep-001-environment-preparation/owner-architecture-decisions.json",
);
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-ep-001-environment-preparation-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-ep-001-environment-preparation-architecture-v1",
...
const discoveryDecision = {
  ...
  decisionId: "architecture-discovery-bypass-ep-001-environment-preparation-v1",
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay EP-001 EnvironmentPreparation/Verification")} ${dslQuote("Profile-bound environment preparation, verification, readiness, and execution authorization")} {`,
    "  !identifiers flat",
...
const designRef = fileRef(
  "openspec-design-ep-001-environment-preparation-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-ep-001-environment-preparation-v2",
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/ep-001-environment-preparation/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-EP-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
    { section: "technicalDesign", entityIds: [technicalDesign.technicalDesignId], jsonPointers: ["/sections/technicalDesign"] },
    { section: "interfaceIntent", entityIds: interfaces.filter(({ id }) => id.startsWith("IF-EP-")).map(({ id }) => id), jsonPointers: ["/sections/interfaceIntent"] },
    { section: "architectureConstraints", entityIds: constraints.filter(({ id }) => id.startsWith("CON-EP-")).map(({ id }) => id), jsonPointers: ["/sections/architectureConstraints"] },
  ]),
  nativeEntry("NA-EP-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-EP-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-EP-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-EP-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-EP-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-EP-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-EP-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by the approved EP-001 EnvironmentPreparation/Verification design.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the approved EP-001 environment preparation and verification architecture.`,
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-EP-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the approved EP-001 EnvironmentPreparation/Verification architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-ep-001-environment-preparation-v2",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-ep-001-environment-preparation-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-EP-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-ep-001-environment-preparation-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-EP-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/ep-001-environment-preparation/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/ep-001-environment-preparation/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/ep-001-environment-preparation",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-ep-001-environment-preparation-v2",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-ep-001-environment-preparation-v2",
  runId: "ep-001-environment-preparation-architecture-run-v2",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "ep-001-environment-preparation", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/ep-001-environment-preparation/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/ep-001-environment-preparation/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/ep-001-environment-preparation/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(projectContextRef, read("dogfood/ep-001-environment-preparation/project-context.json"));
register(repositorySnapshotRef, read("dogfood/ep-001-environment-preparation/repository-snapshot.json"));
register(architectureBaselineRef, read("project/architecture-baseline.json"));
register(ownerDecisionRef, ownerDecisionBytes);
register(architectureBaselineProjectContextRef, read("dogfood/pm-001-project-memory/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/pm-001-project-memory/repository-snapshot.json"));
...
const traceabilityGraph = createTraceabilityGraphService({
  graphId: "traceability-graph-devrelay-ep-001-environment-preparation",
  projectId: "devrelay",
...
if (process.argv.includes("--promote")) {
  const { promoteEnvironmentArchitecture } = await import("./promote.mjs");
  const promotion = await promoteEnvironmentArchitecture({
    root,
    dir,
    read,
    fileRef,
