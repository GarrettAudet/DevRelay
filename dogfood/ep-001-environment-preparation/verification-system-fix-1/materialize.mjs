...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-system-fix-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-TRACEABILITY": Object.freeze({ argv: ["--test", "test/business-acceptance-traceability.test.mjs", "test/traceability-vocabulary-migration.test.mjs"], command: "node --test test/business-acceptance-traceability.test.mjs test/traceability-vocabulary-migration.test.mjs", tests: 10 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-frontier-7/WI-EP-WINDOWS-E2E/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-system-fix-1/${workItemId}`;
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001SystemVerificationRepairVerificationSummary", results };
