...
const outRoot = resolve(root, "dogfood/ep-001-environment-preparation/integration-release-provenance-fix-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/ep-001-integration-release-provenance-fix-1";
const sourceRef = "refs/heads/codex/ep-001-source-release-provenance-fix-1";
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/ep-001-environment-preparation/execution-release-provenance-fix-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/ep-001-environment-preparation/execution-release-provenance-fix-1/execution-summary.json");
let graphSnapshot = json("dogfood/ep-001-environment-preparation/execution-release-provenance-fix-1/WI-EP-REGRESSION-PERFORMANCE/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/ep-001-environment-preparation/verification-release-provenance-fix-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/ep-001-environment-preparation/execution-release-provenance-fix-1/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Ep001ReleaseProvenanceRepairIntegrationSummary",
  targetRef,
  baseCommit,
