...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-frontier-5");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-DESKTOP-INTEGRATION": Object.freeze({ argv: ["--test", "test/environment-preparation-desktop.test.mjs", "test/environment-preparation-module-surface.test.mjs"], command: "node --test test/environment-preparation-desktop.test.mjs test/environment-preparation-module-surface.test.mjs", tests: 7 }),
  "WI-EP-TRACEABILITY": Object.freeze({ argv: ["--test", "test/environment-preparation-traceability.test.mjs"], command: "node --test test/environment-preparation-traceability.test.mjs", tests: 5 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-frontier-4/WI-EP-OPTIONAL-ADAPTERS/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-frontier-5/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-f7cfb2e",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} EP-001 focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001FifthFrontierVerificationSummary", results };
