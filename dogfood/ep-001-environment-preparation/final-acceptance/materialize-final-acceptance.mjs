...
import { TRACEABILITY_VOCABULARY_V1_7 } from "../../../src/traceability-artifact-validator.mjs";
import {
...
const outputRoot = path.join(
  repositoryRoot,
  "dogfood",
  "ep-001-environment-preparation",
  "final-acceptance",
);
...
const fullRef = (
  ...
  schema,
  mediaType,
  uri: `memory://devrelay/ep-001/${encodeURIComponent(artifactId)}/${digest.slice(7)}.json`,
});
...
const windowsDogfoodBytes = readBytes(
  "dogfood/ep-001-environment-preparation/windows-e2e/installed-package-verification-receipt.json",
);
const windowsDogfood = JSON.parse(windowsDogfoodBytes);
assert.equal(windowsDogfood.outcome, "pass");
const scenarioBytes = readBytes(
  "dogfood/ep-001-environment-preparation/windows-e2e/installed-scenario-receipt.json",
);
...
assert.match(scenario.recovery.restoredFingerprint, /^sha256:[0-9a-f]{64}$/u);
assert.equal(scenario.resumedExecution.outcome, "recorded");
const regressionApprovalBytes = readBytes(
  "dogfood/ep-001-environment-preparation/verification-frontier-6/WI-EP-REGRESSION-PERFORMANCE/gate-approval.json",
);
...
const terminalFactsBytes = readBytes(
  "dogfood/ep-001-environment-preparation/frontier-8/integrated-completion-fact-set.json",
);
...
const repairApprovalBytes = readBytes(
  "dogfood/ep-001-environment-preparation/verification-system-fix-1/WI-EP-TRACEABILITY/gate-approval.json",
);
const repairApproval = JSON.parse(repairApprovalBytes);
assert.equal(repairApproval.decision, "approved");
const repairIntegrationBytes = readBytes(
  "dogfood/ep-001-environment-preparation/integration-system-fix-1/integration-summary.json",
);
const repairIntegration = JSON.parse(repairIntegrationBytes);
assert.equal(repairIntegration.finalCommit, "e67323a7d0f90bb1e379ce1bc4ada58356361d1d");
assert.equal(repairIntegration.results[0].outcome, "integrated");
assert.equal(repairIntegration.results[0].replayAdapterCalls, 0);
const baselineRepairApprovalBytes = readBytes(
  "dogfood/ep-001-environment-preparation/verification-system-fix-2/WI-EP-REGRESSION-PERFORMANCE/gate-approval.json",
);
const baselineRepairApproval = JSON.parse(baselineRepairApprovalBytes);
assert.equal(baselineRepairApproval.decision, "approved");
const baselineRepairIntegrationBytes = readBytes(
...
const releaseEvidenceValue = seal(
  {
    apiVersion: API,
    kind: "EP001ReleaseCandidateEvidenceSet",
    evidenceSetId: "RELEASE-EVIDENCE-DEVRELAY-EP001-ENVIRONMENT-PREPARATION-001",
    ...
    verification: {
      ...
      installedPackage: ref(
        "EP001-WINDOWS-INSTALLED-PACKAGE-VERIFICATION",
      ...
      integratedCompletionFacts: ref(
        "ICFS-DEVRELAY-EP001-ENVIRONMENT-PREPARATION-001",
        sha256Digest(terminalFactsBytes),
      ),
      systemVerificationRepairApproval: ref(
        repairApproval.approvalId,
        repairApproval.approvalDigest,
      ),
      systemVerificationRepairIntegration: ref(
        repairIntegration.kind,
        sha256Digest(repairIntegrationBytes),
      ),
      baselineAssertionRepairApproval: ref(
        baselineRepairApproval.approvalId,
        baselineRepairApproval.approvalDigest,
      ),
    ...
    environmentPreparation: {
      ...
      initialGap: scenario.initialGap,
      establishment: scenario.establishment,
      drift: scenario.drift,
      recovery: scenario.recovery,
      resumedExecution: scenario.resumedExecution,
      traceability: scenario.traceability,
    },
    releaseBoundary:
      "Deterministic GitHub source/library and installable package with EnvironmentPreparation verified end-to-end through ChatGPT Desktop on Windows.",
    exclusions: [
...
const integrationSources = [
  { integration: "dogfood/ep-001-environment-preparation/integration-frontier-1", verification: "dogfood/ep-001-environment-preparation/verification-frontier-1" },
  { integration: "dogfood/ep-001-environment-preparation/integration-frontier-2", verification: "dogfood/ep-001-environment-preparation/verification-frontier-2" },
  { integration: "dogfood/ep-001-environment-preparation/integration-frontier-3", verification: "dogfood/ep-001-environment-preparation/verification-frontier-3" },
  { integration: "dogfood/ep-001-environment-preparation/integration-frontier-4", verification: "dogfood/ep-001-environment-preparation/verification-frontier-4" },
  { integration: "dogfood/ep-001-environment-preparation/integration-frontier-5", verification: "dogfood/ep-001-environment-preparation/verification-frontier-5" },
  { integration: "dogfood/ep-001-environment-preparation/integration-frontier-6", verification: "dogfood/ep-001-environment-preparation/verification-frontier-6" },
...
const completionFactSet = seal(
  {
    ...
    factSetId: "ICFS-DEVRELAY-EP001-PROJECT-MEMORY-001",
...
const systemPolicy = seal(
  {
    ...
    policyId: "SV-POLICY-DEVRELAY-EP001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const integratedSubject = seal(
  {
    ...
    subjectId: `DEVRELAY-EP001-DESKTOP-WINDOWS-${releaseEvidence.value.evidenceSetDigest.slice(7, 19).toUpperCase()}`,
...
const reviewObligations = obligations.obligations
  ...
  .map(({ obligationId }) => obligationId);
const invocations = [
  invocationFor("SV-DEVRELAY-EP001-TEST-001", TEST_SYSTEM_VERIFIER, testObligations),
  invocationFor("SV-DEVRELAY-EP001-REVIEW-001", REVIEW_SYSTEM_VERIFIER, reviewObligations),
];
...
const verifier = async (invocation) => {
  ...
  if (invocation.verifier.id === TEST_SYSTEM_VERIFIER.id) {
    ...
    return adaptSystemTestResult({
      invocation,
      nativeBytes: bytes,
      nativeArtifact: ref("NATIVE-DEVRELAY-EP001-TEST-001", sha256Digest(bytes)),
    });
  }
  const native = {
  ...
  return adaptSystemReviewResult({
    invocation,
    nativeBytes: bytes,
    nativeArtifact: ref("NATIVE-DEVRELAY-EP001-REVIEW-001", sha256Digest(bytes)),
  });
};
...
const systemInvocation = {
  invocationId: "SYSTEM-VERIFICATION-DEVRELAY-EP001-001",
...
const businessPolicy = seal(
  {
    ...
    policyId: "BA-POLICY-DEVRELAY-EP001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const businessEvidence = seal(
  {
    ...
    evidenceSetId: "BA-EVIDENCE-DEVRELAY-EP001-DESKTOP-WINDOWS-001",
    requirementsBaseline: ref(requirements.ref.artifactId, requirements.ref.digest),
    projectOverviewBaseline: ref(overview.ref.artifactId, overview.ref.digest),
...
const businessInput = {
  checkpointId: "BA-DEVRELAY-EP001-DESKTOP-WINDOWS-001",
...
const ownerApproval = seal(
  {
    ...
    approvalId: "BA-APPROVAL-DEVRELAY-EP001-DESKTOP-WINDOWS-001",
...
let ownerCalls = 0;
const gateInput = {
  checkpointId: "BA-GATE-DEVRELAY-EP001-DESKTOP-WINDOWS-001",
...
const businessInvocation = {
  invocationId: "BUSINESS-ACCEPTANCE-GATE-DEVRELAY-EP001-001",
  module: {
...
const approvalRequest = seal(
  {
    ...
    requestId: "BA-APPROVAL-REQUEST-DEVRELAY-EP001-DESKTOP-WINDOWS-001",
    candidate: ref(business.candidate.candidateId, business.candidate.candidateDigest),
    candidateRawDigest: sha256Digest(candidateBytes),
    technicalCoverage: ref(
      business.technicalCoverage.coverageId,
...
const finalProof = seal(
  {
    apiVersion: API,
    kind: "DevRelayEP001SourceReleaseAcceptanceProof",
    proofId: "DEVRELAY-EP001-SOURCE-RELEASE-ACCEPTANCE-001",
    sourceBaseCommit,
...
const summary = {
  apiVersion: API,
  kind: "DevRelayEP001FinalAcceptanceSummary",
  releaseEvidence: releaseEvidenceRef,
...
writeText(
  "FINAL_ACCEPTANCE.md",
  `# DevRelay ep-001 final acceptance\n\n` +
