...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-frontier-4");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-GATE-READINESS": Object.freeze({ argv: ["--test", "test/environment-preparation-gate.test.mjs"], command: "node --test test/environment-preparation-gate.test.mjs", tests: 9 }),
  "WI-EP-OPTIONAL-ADAPTERS": Object.freeze({ argv: ["--test", "test/environment-preparation-adapters.test.mjs"], command: "node --test test/environment-preparation-adapters.test.mjs", tests: 8 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-frontier-3/WI-EP-PREPARATION-EFFECTS/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-frontier-4/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} EP-001 Gate/adapter focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001FourthFrontierVerificationSummary", results };
