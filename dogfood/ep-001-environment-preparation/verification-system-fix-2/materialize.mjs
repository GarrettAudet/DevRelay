...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-system-fix-2");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-REGRESSION-PERFORMANCE": Object.freeze({ argv: ["--test", "test/pm001-final-release.test.mjs", "test/project-baseline.test.mjs"], command: "node --test test/pm001-final-release.test.mjs test/project-baseline.test.mjs", tests: 3 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-system-fix-1/WI-EP-TRACEABILITY/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-system-fix-2/${workItemId}`;
