...
const outRoot = resolve(root, "dogfood/ep-001-environment-preparation/integration-frontier-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/ep-001-integration-frontier-1";
const sourceRef = "refs/heads/codex/ep-001-source-frontier-1";
const ids = Object.freeze([
  "WI-EP-ARTIFACT-CONTRACTS",
]);
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/ep-001-environment-preparation/execution-frontier-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/ep-001-environment-preparation/execution-frontier-1/execution-summary.json");
let graphSnapshot = json("dogfood/ep-001-environment-preparation/execution-frontier-1/WI-EP-ARTIFACT-CONTRACTS/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/ep-001-environment-preparation/verification-frontier-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/ep-001-environment-preparation/execution-frontier-1/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Ep001FirstFrontierIntegrationSummary",
  targetRef,
  baseCommit,
