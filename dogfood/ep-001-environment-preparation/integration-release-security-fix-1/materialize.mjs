...
const outRoot = resolve(root, "dogfood/ep-001-environment-preparation/integration-release-security-fix-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/ep-001-integration-release-security-fix-1";
const sourceRef = "refs/heads/codex/ep-001-source-release-security-fix-1";
...
const baseCommit = json("dogfood/ep-001-environment-preparation/integration-system-fix-2/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/ep-001-environment-preparation/execution-release-security-fix-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/ep-001-environment-preparation/execution-release-security-fix-1/execution-summary.json");
let graphSnapshot = json("dogfood/ep-001-environment-preparation/execution-release-security-fix-1/WI-EP-WINDOWS-E2E/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/ep-001-environment-preparation/verification-release-security-fix-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/ep-001-environment-preparation/execution-release-security-fix-1/${workItemId}/change-set-draft.json`);
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-EP001-${workItemId}-REPAIR-003`,
  });
  ...
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-${workItemId}-REPAIR-003`,
...
const summary = {
  apiVersion: API,
  kind: "Ep001ReleaseSecurityRepairIntegrationSummary",
  targetRef,
  baseCommit,
