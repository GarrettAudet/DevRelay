...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-frontier-2");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-PROFILE-INVENTORY": Object.freeze({ argv: ["--test", "test/environment-preparation-profile-inventory.test.mjs"], command: "node --test test/environment-preparation-profile-inventory.test.mjs", tests: 8 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/integration-frontier-1/WI-EP-ARTIFACT-CONTRACTS/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-frontier-2/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} EP-001 profile-inventory frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001SecondFrontierVerificationSummary", results };
