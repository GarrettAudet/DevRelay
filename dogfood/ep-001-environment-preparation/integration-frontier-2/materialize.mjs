...
const outRoot = resolve(root, "dogfood/ep-001-environment-preparation/integration-frontier-2");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/ep-001-integration-frontier-2";
const sourceRef = "refs/heads/codex/ep-001-source-frontier-2";
const ids = Object.freeze([
  "WI-EP-PROFILE-INVENTORY",
]);
...
const baseCommit = json("dogfood/ep-001-environment-preparation/integration-frontier-1/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/ep-001-environment-preparation/execution-frontier-2/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/ep-001-environment-preparation/execution-frontier-2/execution-summary.json");
let graphSnapshot = json("dogfood/ep-001-environment-preparation/execution-frontier-2/WI-EP-PROFILE-INVENTORY/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/ep-001-environment-preparation/verification-frontier-2/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/ep-001-environment-preparation/execution-frontier-2/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Ep001SecondFrontierIntegrationSummary",
  targetRef,
  baseCommit,
