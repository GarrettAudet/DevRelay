...
const outRoot = resolve(root, "dogfood/ep-001-environment-preparation/integration-frontier-3");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/ep-001-integration-frontier-3";
const sourceRef = "refs/heads/codex/ep-001-source-frontier-3";
const ids = Object.freeze([
  "WI-EP-PREPARATION-EFFECTS",
]);
...
const baseCommit = json("dogfood/ep-001-environment-preparation/integration-frontier-2/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/ep-001-environment-preparation/execution-frontier-3/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/ep-001-environment-preparation/execution-frontier-3/execution-summary.json");
let graphSnapshot = json("dogfood/ep-001-environment-preparation/execution-frontier-3/WI-EP-PREPARATION-EFFECTS/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/ep-001-environment-preparation/verification-frontier-3/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/ep-001-environment-preparation/execution-frontier-3/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Ep001ThirdFrontierIntegrationSummary",
  targetRef,
  baseCommit,
