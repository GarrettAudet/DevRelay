...
const verificationRoot = resolve(root, "dogfood/ep-001-environment-preparation/verification-release-provenance-fix-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-EP-REGRESSION-PERFORMANCE": Object.freeze({ argv: ["--test", "test/v0.11-final-acceptance.test.mjs"], command: "node --test test/v0.11-final-acceptance.test.mjs", tests: 4 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/ep-001-environment-preparation/execution-release-provenance-fix-1/repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/ep-001-environment-preparation/execution-release-provenance-fix-1/${workItemId}`;
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Ep001ReleaseProvenanceRepairVerificationSummary", results };
