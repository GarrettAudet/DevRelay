...
const repositoryValue = readJson(
  "dogfood/ep-001-environment-preparation/repository-snapshot.json",
);
const repositoryBytes = readBytes(
  "dogfood/ep-001-environment-preparation/repository-snapshot.json",
);
const repository = loadWorkExecutionInput({
  bytes: repositoryBytes,
  ref: ref(
    `repository-snapshot-devrelay-${repositoryValue.revision.slice(0, 7)}`,
    "https://devrelay.dev/artifacts/repository-snapshot/v1",
    "application/vnd.devrelay.repository-snapshot+json",
    sha256Digest(repositoryBytes),
  ),
...
const jobs = [
  {
    workItemId: "WI-EP-ARTIFACT-CONTRACTS",
    attemptId: "ATT-EP-ARTIFACT-CONTRACTS-001",
    files: [
      "contracts/environment-preparation-artifacts.schema.json",
      "src/environment-preparation-artifact-validator.mjs",
      "test/environment-preparation-artifacts.test.mjs",
      "src/index.mjs",
    ],
    command: "node --test test/environment-preparation-artifacts.test.mjs",
    tests: 6,
  },
];const permissions = [
  { kind: "filesystem.write", scope: { values: ["contracts/**", "src/**", "test/**"] } },
  { kind: "process.spawn", scope: { values: ["node"] } },
];

const graphHeadPath =
  "dogfood/ep-001-environment-preparation/assignment/promotion/traceability-graph-snapshot.json";
const graphHeadValue = readJson(graphHeadPath);
const graphHeadBytes = Buffer.from(canonicalJson(graphHeadValue), "utf8");
const graphHeadRef = assignmentPromotion.resultingGraph;
...
const summary = {
  apiVersion: API,
  kind: "Ep001FrontierOneExecutionSummary",
