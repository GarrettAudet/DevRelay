...
export async function promoteLifecycleRunReportArchitecture(context) {
  const {
    root,
    dir,
    read,
  ...
  const gateReviewRef = fileRef(
    "architecture-gate-review-lifecycle-run-report-v1",
  ...
  const executionProofRef = fileRef(
    "architecture-dogfood-execution-proof-lifecycle-run-report-v1",
  ...
  const promotedBaseline = {
    ...
    baselineId: "architecture-baseline-devrelay-v1-lifecycle-run-report-001",
  ...
  const promotedProjectState = {
    ...
    stateId: "project-architecture-state-devrelay-lifecycle-run-report-approved-v1",
    state: "baselined",
  ...
  const promotionId = "architecture-promotion-lifecycle-run-report-v1";
  ...
  fs.writeFileSync(
    path.join(projectDir, "architecture-gate-owner-approval-lifecycle-run-report-v1.json"),
    ownerApprovalBytes,
  );
