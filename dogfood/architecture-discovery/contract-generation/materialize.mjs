...
const RUN_ID = "architecture-discovery-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-architecture-discovery-v1";
const GRAPH_ID = "devrelay/architecture-discovery-contract-change";
...
const currentContractBaseline = persistedContractBaseline;
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== 48) {
  throw new Error(`expected 48 required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-ARCHITECTURE-DISCOVERY-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-ARCHITECTURE-DISCOVERY-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-architecture-discovery-v1",
);
...
const checkpointFile = await exactJsonFile(
  ...
  "application/json",
  "CG-CHECKPOINT-ARCHITECTURE-DISCOVERY-001",
);
  ...
  evidence: [
    {
      ...
      summary: "Core independently validated all 48 native JSON Schema draft 2020-12 artifacts.",
    },
    {
      ...
      summary: "Core computed a deterministic seven-add contract diff against the exact 41-contract baseline.",
    },
  ],
  diagnostics: [],
};
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-ARCHITECTURE-DISCOVERY-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-ARCHITECTURE-DISCOVERY-CONTRACT-CHANGE-001",
  ...
  "application/json",
  "CGR-ARCHITECTURE-DISCOVERY-001",
);
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-ARCHITECTURE-DISCOVERY-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
  ...
  "application/json",
  "CGC-ARCHITECTURE-DISCOVERY-001",
);
await exactJsonFile(
  ...
  "application/json",
  "CG-RUNTIME-PROOF-ARCHITECTURE-DISCOVERY-001",
);
