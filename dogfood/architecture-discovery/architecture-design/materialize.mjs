...
import { buildArchitectureDiscoveryArchitecture } from "./architecture-discovery-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/architecture-discovery/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/architecture-discovery/architecture-design";
...
const projectContext = load("dogfood/architecture-discovery/project-context.json");
const repositorySnapshot = load("dogfood/architecture-discovery/repository-snapshot.json");
// ArchitectureDiscovery is designed against the exact current WorkItemVerification architecture baseline.
...
const projectContextRef = artifactRef(
  "project-context-devrelay-architecture-discovery-v1",
  "https://devrelay.dev/artifacts/project-context/v1",
  "application/vnd.devrelay.project-context+json",
  sha256(read("dogfood/architecture-discovery/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/architecture-discovery/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  ...
  sha256(read("dogfood/architecture-discovery/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/architecture-discovery/repository-snapshot.json",
);
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-architecture-discovery-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-architecture-discovery-architecture-v1",
...
const {
  ...
  risks,
  uniq,
} = buildArchitectureDiscoveryArchitecture({
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay ArchitectureDiscovery")} ${dslQuote("Evidence-bound verification and Gate target architecture")} {`,
    "  !identifiers flat",
...
const designRef = fileRef(
  "openspec-design-architecture-discovery-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-architecture-discovery-v1",
...
const diagrams = {
  diagramSetId: "DIAGRAMS-DEVRELAY-AD-001",
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/architecture-discovery/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-AD-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
    { section: "technicalDesign", entityIds: [technicalDesign.technicalDesignId], jsonPointers: ["/sections/technicalDesign"] },
    { section: "interfaceIntent", entityIds: interfaces.filter(({ id }) => id.startsWith("IF-AD-")).map(({ id }) => id), jsonPointers: ["/sections/interfaceIntent"] },
    { section: "architectureConstraints", entityIds: constraints.filter(({ id }) => id.startsWith("CON-AD-")).map(({ id }) => id), jsonPointers: ["/sections/architectureConstraints"] },
  ]),
  nativeEntry("NA-AD-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-AD-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-AD-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-AD-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-AD-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-AD-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-AD-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by bounded current-architecture discovery.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the bounded architecture-discovery target architecture.`,
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-AD-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the bounded architecture-discovery architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-architecture-discovery-v1",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-architecture-discovery-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-AD-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-architecture-discovery-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-AD-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/architecture-discovery/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/architecture-discovery/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/architecture-discovery",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-architecture-discovery-v1",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-architecture-discovery-v1",
  runId: "architecture-discovery-architecture-run-v1",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "architecture-discovery", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/architecture-discovery/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/architecture-discovery/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/architecture-discovery/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(projectContextRef, read("dogfood/architecture-discovery/project-context.json"));
register(repositorySnapshotRef, read("dogfood/architecture-discovery/repository-snapshot.json"));
register(architectureBaselineRef, read(architectureBaselineSourcePath));
register(architectureBaselineProjectContextRef, read("dogfood/change-integration/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/change-integration/repository-snapshot.json"));
...
if (process.argv.includes("--promote")) {
  const { promoteArchitectureDiscoveryArchitecture } = await import("./promote.mjs");
  const promotion = await promoteArchitectureDiscoveryArchitecture({
    root,
    dir,
    read,
    fileRef,
