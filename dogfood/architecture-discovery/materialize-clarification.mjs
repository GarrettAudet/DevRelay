...
import {
  ...
  projectContext,
  questions,
} from "./architecture-discovery-clarification-data.mjs";
...
const invocation = Object.freeze({
  ...
  invocationId:
    "invocation-requirements-architecture-discovery-openspec-clarify-v1",
  runId: "dogfood-architecture-discovery-v1",
      ...
      adapter: {
        async invoke(_adapterInvocation, _adapterContext, producer) {
          ...
          const clarificationRequest = {
            ...
            requestSetId: "clarification-request-architecture-discovery-v1",
            baseInputs,
            questions,
          };
          ...
          continuation = {
            ...
            continuationId: "requirements-continuation-architecture-discovery-v1",
            ...
            confirmedFacts: [
              "ArchitectureDiscovery is selected only for existing-undiscovered project state.",
              "ArchitectureDesign already requires a CurrentArchitectureSnapshot before establishing a baseline for an existing system.",
              "Discovery is observational and cannot approve intended architecture or promote an ArchitectureBaseline.",
              "Every finding must be bound to exact repository or native analyzer evidence.",
              "Adapters cannot route, approve, mutate TraceabilityGraph, or choose progression.",
              "No promotable ArchitectureDiscovery requirements change exists until every blocking question is answered.",
            ],
...
const executionProof = {
  ...
  proofId: "requirements-clarification-architecture-discovery-v1",
  ...
  architectureDiscoveryImplementationAllowed: false,
};
await writeFile(
...
await Promise.all([
  ...
  writeFile(
    ...
    Buffer.from(
      "# ArchitectureDiscovery dogfood\n\n" +
        "Current state: RequirementsGathering is checkpointed at `needs_clarification`. The exact request, continuation, approved baseline inputs, and effect checkpoint form the resumable package. No ArchitectureDiscovery requirements change, architecture candidate, work breakdown, or implementation is authorized.\n",
    ),
  ),
]);
