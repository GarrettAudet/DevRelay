...
const RUN_ID = "contract-generation-dogfood-run-v1";
const INVOCATION_ID = "contract-generation-establish-devrelay-v1";
const GRAPH_ID = "devrelay/contract-generation";
...
const projectArchitectureState = JSON.parse(
  await readFile(new URL("project/history/architecture/architecture-baseline-devrelay-v1-contract-generation-001/project-architecture-state.json", ROOT), "utf8"),
);
...
const projectOverview = await loadedFile(
  "project/history/1.3.0/project-overview-baseline.json",
...
const requirements = await loadedFile(
  "project/history/1.3.0/requirements-baseline.json",
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== 14) {
  throw new Error(`expected 14 required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-UNBASELINED-001",
    state: "unbaselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-UNBASELINED-001",
);
...
const artifactNames = {
  ...
  candidate: "contract-draft-set.json",
...
const moduleResult = {
  ...
  status: "completed",
  outcome: "generated",
  outputs: { "contract-draft-set": [execution.candidateRef] },
  evidence: [
    {
      kind: "contract-generation/format-validation",
      subject: execution.candidate.draftSetId,
      status: "pass",
      artifact: checkpoint.artifacts.validationSet.ref,
      summary: "Core independently validated all 14 native JSON Schema draft 2020-12 artifacts.",
    },
    {
      ...
      summary: "Core computed a deterministic 14-add initial contract diff against no prior baseline.",
    },
  ],
  diagnostics: [],
};
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    checks: [
      ...
      { id: "canonical-initial-diff", status: "pass", evidence: checkpoint.artifacts.canonicalDiff.ref },
