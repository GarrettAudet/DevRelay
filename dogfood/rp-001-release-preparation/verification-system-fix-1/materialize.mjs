...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-system-fix-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-RP-TRACEABILITY": Object.freeze({ argv: ["--test", "test/business-acceptance-traceability.test.mjs", "test/release-preparation-traceability.test.mjs"], command: "node --test test/business-acceptance-traceability.test.mjs test/release-preparation-traceability.test.mjs", tests: 11 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-frontier-8/WI-RP-WINDOWS-E2E/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-system-fix-1/${workItemId}`;
