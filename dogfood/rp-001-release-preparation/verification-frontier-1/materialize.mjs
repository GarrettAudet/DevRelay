...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-frontier-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-RP-ARTIFACT-CONTRACTS": Object.freeze({
    argv: ["--test", "test/release-preparation-artifacts.test.mjs"],
    command: "node --test test/release-preparation-artifacts.test.mjs",
    tests: 6,
  }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-frontier-1/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} RP-001 first-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Rp001FirstFrontierVerificationSummary", results };
