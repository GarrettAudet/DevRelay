...
const fullRef = (
  ...
  schema,
  mediaType,
  uri: `memory://devrelay/rp-001/${encodeURIComponent(artifactId)}/${digest.slice(7)}.json`,
});
...
function restoreGraph() {
  const graphPath =
    "dogfood/rp-001-release-preparation/integration-system-fix-4/WI-RP-WINDOWS-E2E/traceability-graph-snapshot.json";
  const graphSummary = readJson(
    "dogfood/rp-001-release-preparation/integration-system-fix-4/integration-summary.json",
  );
  const value = readJson(graphPath);
  const bytes = canonicalBytes(value);
  const graphRef = structuredClone(graphSummary.resultGraph);
  assert.equal(sha256Digest(bytes), graphRef.digest);
...
assert.equal(implementationCommit, "ea2e3d691de0fd69446912c717293c4f5ee1a21d");
const previousAcceptanceBytes = readBytes(
  "dogfood/pm-001-project-memory/final-acceptance/business-acceptance-record.json",
);
const previousAcceptance = JSON.parse(previousAcceptanceBytes);
assert.equal(previousAcceptance.outcome, "accepted");
const windowsDogfoodBytes = readBytes(
  "dogfood/rp-001-release-preparation/windows-e2e/installed-package-verification-receipt.json",
);
const windowsDogfood = JSON.parse(windowsDogfoodBytes);
assert.equal(windowsDogfood.outcome, "pass");
const scenarioBytes = readBytes(
  "dogfood/rp-001-release-preparation/windows-e2e/installed-scenario-receipt.json",
);
const scenario = JSON.parse(scenarioBytes);
assert.equal(scenario.outcome, "pass");
assert.equal(scenario.recovery.drift.driftDetected, true);
assert.match(scenario.recovery.recoveredFingerprint, /^sha256:[0-9a-f]{64}$/u);
assert.equal(scenario.recovery.replayEffectCalls, 0);
assert.equal(scenario.noPublication.publicRegistryCalls, 0);
const regressionApprovalBytes = readBytes(
  "dogfood/rp-001-release-preparation/verification-frontier-7/WI-RP-REGRESSION/gate-approval.json",
);
const regressionApproval = JSON.parse(regressionApprovalBytes);
assert.equal(regressionApproval.decision, "approved");
const terminalFactsBytes = readBytes(
  "dogfood/rp-001-release-preparation/frontier-9/integrated-completion-fact-set.json",
);
const terminalFacts = JSON.parse(terminalFactsBytes);
assert.equal(terminalFacts.facts.length, 9);
assert.equal(terminalFacts.facts.every(({ authority }) => authority === "approved"), true);
const windowsApprovalBytes = readBytes(
  "dogfood/rp-001-release-preparation/verification-frontier-8/WI-RP-WINDOWS-E2E/gate-approval.json",
);
const windowsApproval = JSON.parse(windowsApprovalBytes);
assert.equal(windowsApproval.decision, "approved");
const traceabilityRepairApprovalBytes = readBytes(
  "dogfood/rp-001-release-preparation/verification-system-fix-1/WI-RP-TRACEABILITY/gate-approval.json",
);
const traceabilityRepairApproval = JSON.parse(traceabilityRepairApprovalBytes);
assert.equal(traceabilityRepairApproval.decision, "approved");
const catalogRepairApprovalBytes = readBytes(
  "dogfood/rp-001-release-preparation/verification-system-fix-2/WI-RP-WINDOWS-E2E/gate-approval.json",
);
const catalogRepairApproval = JSON.parse(catalogRepairApprovalBytes);
assert.equal(catalogRepairApproval.decision, "approved");
const baselineClosureRepairApprovalBytes = readBytes(
  "dogfood/rp-001-release-preparation/verification-system-fix-3/WI-RP-WINDOWS-E2E/gate-approval.json",
);
const baselineClosureRepairApproval = JSON.parse(baselineClosureRepairApprovalBytes);
assert.equal(baselineClosureRepairApproval.decision, "approved");
const finalCatalogRepairApprovalBytes = readBytes(
  "dogfood/rp-001-release-preparation/verification-system-fix-4/WI-RP-WINDOWS-E2E/gate-approval.json",
);
const finalCatalogRepairApproval = JSON.parse(finalCatalogRepairApprovalBytes);
assert.equal(finalCatalogRepairApproval.decision, "approved");
const releaseEvidenceValue = seal(
  {
    apiVersion: API,
    kind: "RP001ReleaseCandidateEvidenceSet",
    evidenceSetId: "RELEASE-EVIDENCE-DEVRELAY-RP001-RELEASE-PREPARATION-001",
    implementationCommit,
    priorAcceptedBaseline: ref(
      previousAcceptance.recordId,
      previousAcceptance.recordDigest,
    ),
    verification: {
      workItemRegressionApproval: ref(
        regressionApproval.approvalId,
        regressionApproval.approvalDigest,
      ),
      installedPackage: ref(
        "RP001-WINDOWS-INSTALLED-PACKAGE-VERIFICATION",
        sha256Digest(windowsDogfoodBytes),
      ),
      installedScenario: ref(
        scenario.kind,
        sha256Digest(scenarioBytes),
      ),
      integratedCompletionFacts: ref(
        "ICFS-DEVRELAY-RP001-RELEASE-PREPARATION-001",
        sha256Digest(terminalFactsBytes),
      ),
      windowsE2EApproval: ref(
        windowsApproval.approvalId,
        windowsApproval.approvalDigest,
      ),
      traceabilityRepairApproval: ref(
        traceabilityRepairApproval.approvalId,
        traceabilityRepairApproval.approvalDigest,
      ),
      catalogRepairApproval: ref(
        catalogRepairApproval.approvalId,
        catalogRepairApproval.approvalDigest,
      ),
      baselineClosureRepairApproval: ref(
        baselineClosureRepairApproval.approvalId,
        baselineClosureRepairApproval.approvalDigest,
      ),
      finalCatalogRepairApproval: ref(
        finalCatalogRepairApproval.approvalId,
        finalCatalogRepairApproval.approvalDigest,
      ),
    },
    package: {
      name: windowsDogfood.package.name,
      version: windowsDogfood.package.version,
      digest: windowsDogfood.package.digest,
      files: windowsDogfood.package.files,
      unpackedSize: windowsDogfood.package.unpackedSize,
    },
    releasePreparation: {
      host: scenario.host,
      platform: scenario.platform,
      nodeVersion: scenario.nodeVersion,
      attempt: scenario.attempt,
      materialization: scenario.materialization,
      verification: scenario.verification,
      gate: scenario.gate,
      drift: scenario.recovery.drift,
      recovery: scenario.recovery,
      traceability: scenario.traceability,
    },
    releaseBoundary:
      "Deterministic GitHub source/library and installable package with ReleasePreparation verified end-to-end through ChatGPT Desktop on Windows.",
    exclusions: [
...
const workItemIds = workBreakdown.value.workItems
  .map(({ id }) => id)
  .sort((left, right) => left.localeCompare(right, "en"));
const integrationSources = [
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-1", verification: "dogfood/rp-001-release-preparation/verification-frontier-1" },
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-2", verification: "dogfood/rp-001-release-preparation/verification-frontier-2" },
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-3", verification: "dogfood/rp-001-release-preparation/verification-frontier-3" },
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-4", verification: "dogfood/rp-001-release-preparation/verification-frontier-4" },
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-5", verification: "dogfood/rp-001-release-preparation/verification-frontier-5" },
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-6", verification: "dogfood/rp-001-release-preparation/verification-frontier-6" },
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-7", verification: "dogfood/rp-001-release-preparation/verification-frontier-7" },
  { integration: "dogfood/rp-001-release-preparation/integration-frontier-8", verification: "dogfood/rp-001-release-preparation/verification-frontier-8" },
  { integration: "dogfood/rp-001-release-preparation/integration-system-fix-1", verification: "dogfood/rp-001-release-preparation/verification-system-fix-1" },
  { integration: "dogfood/rp-001-release-preparation/integration-system-fix-2", verification: "dogfood/rp-001-release-preparation/verification-system-fix-2" },
  { integration: "dogfood/rp-001-release-preparation/integration-system-fix-3", verification: "dogfood/rp-001-release-preparation/verification-system-fix-3" },
  { integration: "dogfood/rp-001-release-preparation/integration-system-fix-4", verification: "dogfood/rp-001-release-preparation/verification-system-fix-4" },
];
...
const completionFactSet = seal(
  {
    ...
    factSetId: "ICFS-DEVRELAY-RP001-RELEASE-PREPARATION-001",
...
const systemPolicy = seal(
  {
    ...
    policyId: "SV-POLICY-DEVRELAY-RP001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const repositoryReleaseSnapshot = seal(
  {
    ...
    branch: "codex/rp-001-release-preparation",
    sourceBaseCommit,
...
const integratedSubject = seal(
  {
    ...
    subjectId: `DEVRELAY-RP001-DESKTOP-WINDOWS-${releaseEvidence.value.evidenceSetDigest.slice(7, 19).toUpperCase()}`,
...
const reviewObligations = obligations.obligations
  ...
  .map(({ obligationId }) => obligationId);
const invocations = [
  invocationFor("SV-DEVRELAY-RP001-TEST-001", TEST_SYSTEM_VERIFIER, testObligations),
  invocationFor("SV-DEVRELAY-RP001-REVIEW-001", REVIEW_SYSTEM_VERIFIER, reviewObligations),
];
...
const verifier = async (invocation) => {
  ...
  if (invocation.verifier.id === TEST_SYSTEM_VERIFIER.id) {
    ...
    return adaptSystemTestResult({
      invocation,
      nativeBytes: bytes,
      nativeArtifact: ref("NATIVE-DEVRELAY-RP001-TEST-001", sha256Digest(bytes)),
    });
  }
  const native = {
  ...
  return adaptSystemReviewResult({
    invocation,
    nativeBytes: bytes,
    nativeArtifact: ref("NATIVE-DEVRELAY-RP001-REVIEW-001", sha256Digest(bytes)),
  });
};
...
const systemInvocation = {
  invocationId: "SYSTEM-VERIFICATION-DEVRELAY-RP001-001",
...
const businessPolicy = seal(
  {
    ...
    policyId: "BA-POLICY-DEVRELAY-RP001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const businessEvidence = seal(
  {
    ...
    evidenceSetId: "BA-EVIDENCE-DEVRELAY-RP001-DESKTOP-WINDOWS-001",
    requirementsBaseline: ref(requirements.ref.artifactId, requirements.ref.digest),
    projectOverviewBaseline: ref(overview.ref.artifactId, overview.ref.digest),
...
assert.equal(
  graphCheckpoint.snapshot.nodes.filter(
    ({ kind, state, scope }) =>
      kind === "business-scope" &&
      state === "active" &&
...
const businessInput = {
  checkpointId: "BA-DEVRELAY-RP001-DESKTOP-WINDOWS-001",
...
const ownerApproval = seal(
  {
    ...
    approvalId: "BA-APPROVAL-DEVRELAY-RP001-DESKTOP-WINDOWS-001",
...
let ownerCalls = 0;
const gateInput = {
  checkpointId: "BA-GATE-DEVRELAY-RP001-DESKTOP-WINDOWS-001",
...
const businessInvocation = {
  invocationId: "BUSINESS-ACCEPTANCE-GATE-DEVRELAY-RP001-001",
  module: {
...
const approvalRequest = seal(
  {
    ...
    requestId: "BA-APPROVAL-REQUEST-DEVRELAY-RP001-DESKTOP-WINDOWS-001",
    candidate: ref(business.candidate.candidateId, business.candidate.candidateDigest),
    candidateRawDigest: sha256Digest(candidateBytes),
    technicalCoverage: ref(
      business.technicalCoverage.coverageId,
...
const finalProof = seal(
  {
    apiVersion: API,
    kind: "DevRelayRP001SourceReleaseAcceptanceProof",
    proofId: "DEVRELAY-RP001-SOURCE-RELEASE-ACCEPTANCE-001",
    sourceBaseCommit,
...
const summary = {
  apiVersion: API,
  kind: "DevRelayRP001FinalAcceptanceSummary",
  releaseEvidence: releaseEvidenceRef,
...
writeText(
  "FINAL_ACCEPTANCE.md",
  `# DevRelay rp-001 final acceptance\n\n` +
