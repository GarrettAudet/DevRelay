...
import { buildReleasePreparationRequirements } from "./rp-001-approved-data.mjs";

const root = new URL("../../", import.meta.url);
const output = new URL("./", import.meta.url);
const canonicalRoot = "C:/repos/DevRelay";
const artifactRoot = "file:///C:/repos/DevRelay/dogfood/rp-001-release-preparation";
...
const projectContextRef = store.addExisting({
  artifactId: "project-context-devrelay-rp-001-release-preparation-v1",
  bytes: contextBytes,
  type: "projectContext",
...
const repositorySnapshotRef = store.addJson(
  ...
  "repository-snapshot-devrelay-1ff3cc4-rp-001",
  "repositorySnapshot",
  repositorySnapshot,
);
...
const closureRef = store.addExisting({
  artifactId: "requirements-closure-rp-001-release-preparation-v1",
  bytes: closureBytes,
...
const requirementSourceRefs = [
  ...
  { role: "owner-decision", artifact: pointer(decisionsRef), location: "All 24 recommendations approved in wave RQW-BF39D9028F6F9289" },
...
const replacement = buildReleasePreparationRequirements(requirementsBaseline.requirements, requirementSourceRefs);
...
const requirementsChangeSet = {
  ...
  changeSetId: "requirements-change-set-rp-001-release-preparation-v1",
  baseInputs: [
    { role: "goal", artifact: pointer(goalRef) },
    { role: "project-context", artifact: pointer(projectContextRef) },
    { role: "repository-snapshot", artifact: pointer(repositorySnapshotRef) },
  ...
  replacement,
  changedSections,
  reason: "Add deterministic ReleasePreparation after SystemVerification and a separate ReleaseVerificationGate before BusinessAcceptance, with exact candidate bytes, required verification policy, immutable evidence, and no publication authority.",
  compatibilityImpact: "backward-compatible",
  risks: replacement.risks.filter((entry) => /candidate|release|artifact|supply-chain|hosted provider|receipt/i.test(entry)),
  requiredEvidence: replacement.requiredEvidence.filter((entry) => entry.startsWith("release-preparation/") || entry.includes("release-preparation-cycle")),
...
const overviewMarkdownRef = store.addBytes(
  ...
  "project-overview-markdown-rp-001-release-preparation-v1",
...
const overviewChangeSet = {
  ...
  changeSetId: "project-overview-change-set-rp-001-release-preparation-v1",
...
const proposalRef = store.addText(
  ...
  "openspec-proposal-rp-001-release-preparation-v1",
  "# RP-001 ReleasePreparation and ReleaseVerification\n\nAdd deterministic release-candidate preparation after SystemVerification and a separate ReleaseVerificationGate before BusinessAcceptance while preserving Core, owner, publication, permission, traceability, and downstream lifecycle authority.\n",
);
const specificationRef = store.addText(
  "native/openspec/specs/rp-001-release-preparation/spec.md",
  "openspec-spec-rp-001-release-preparation-v1",
...
const nativeBundle = {
  ...
  sources: [
    { role: "requirements-closure", path: "dogfood/rp-001-release-preparation/requirements-closure-assessment-approved.json", artifact: pointer(closureRef) },
    { role: "owner-decision", path: "dogfood/rp-001-release-preparation/owner-decisions.json", artifact: pointer(decisionsRef) },
    { role: "proposal", path: "dogfood/rp-001-release-preparation/native/openspec/proposal.md", artifact: pointer(proposalRef) },
    { role: "specification", path: "dogfood/rp-001-release-preparation/native/openspec/specs/rp-001-release-preparation/spec.md", artifact: pointer(specificationRef) },
  ],
...
const nativeBundleRef = store.addJson(
  ...
  "native-source-bundle-rp-001-release-preparation-v1",
  "nativeSourceBundle",
  nativeBundle,
);
...
const invocation = {
  ...
  invocationId: "invocation-requirements-rp-001-release-preparation-openspec-v1",
  runId: "dogfood-rp-001-release-preparation-v1",
  ...
  config: {
    ...
    changeName: "rp-001-release-preparation-v1",
    schema: "devrelay-requirements",
...
const executionProof = {
  ...
  proofId: "requirements-change-rp-001-release-preparation-v1",
