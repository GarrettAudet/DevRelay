...
const outRoot = resolve(root, "dogfood/rp-001-release-preparation/integration-frontier-4");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/rp-001-integration-frontier-4";
const sourceRef = "refs/heads/codex/rp-001-source-frontier-4";
const ids = Object.freeze([
  "WI-RP-ADAPTER-EFFECTS",
  "WI-RP-VERIFY-POLICY",
]);
...
const baseCommit = json("dogfood/rp-001-release-preparation/integration-frontier-3/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/rp-001-release-preparation/execution-frontier-4/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/rp-001-release-preparation/execution-frontier-4/execution-summary.json");
let graphSnapshot = json("dogfood/rp-001-release-preparation/execution-frontier-4/WI-RP-VERIFY-POLICY/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/rp-001-release-preparation/verification-frontier-4/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/rp-001-release-preparation/execution-frontier-4/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Rp001FourthFrontierIntegrationSummary",
  targetRef,
  baseCommit,
