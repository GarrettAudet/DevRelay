...
const outRoot = resolve(root, "dogfood/rp-001-release-preparation/integration-system-fix-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/rp-001-integration-system-fix-1";
const sourceRef = "refs/heads/codex/rp-001-source-system-fix-1";
...
const baseCommit = json("dogfood/rp-001-release-preparation/integration-frontier-8/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/rp-001-release-preparation/execution-system-fix-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/rp-001-release-preparation/execution-system-fix-1/execution-summary.json");
let graphSnapshot = json("dogfood/rp-001-release-preparation/execution-system-fix-1/WI-RP-TRACEABILITY/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/rp-001-release-preparation/verification-system-fix-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/rp-001-release-preparation/execution-system-fix-1/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Rp001SystemVerificationRepairIntegrationSummary",
  targetRef,
  baseCommit,
