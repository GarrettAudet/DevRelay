...
const integrationSummary = readJson(
  "dogfood/rp-001-release-preparation/integration-frontier-7/integration-summary.json",
);
...
const jobs = [
  { workItemId: "WI-RP-WINDOWS-E2E", attemptId: "ATT-RP-WINDOWS-E2E-001", files: ["ProjectOverview.md", "project/project-baseline-data.mjs", "project/requirements-baseline.json", "project/project-overview-baseline.json", "project/architecture-baseline.json", "project/contract-disposition.json", "project/work-breakdown-baseline.json", "project/work-dependency-baseline.json", "project/specialist-assignment-baseline.json", "release/0.10.0-rc.3.json", "src/release-preparation-gate.mjs", "src/release-preparation-verification.mjs", "src/release-preparation-traceability-contributor.mjs", "test/v0.11-module-quality-work-breakdown-dogfood.test.mjs", ...fs.readdirSync(path.join(root, "dogfood/rp-001-release-preparation/windows-e2e"), { recursive: true, withFileTypes: true }).filter((entry) => entry.isFile()).map((entry) => path.relative(root, path.join(entry.parentPath, entry.name)).replaceAll("\\", "/"))], command: "node dogfood/rp-001-release-preparation/windows-e2e/materialize.mjs", tests: 1154 },
];
const permissions = [
  { kind: "filesystem.write", scope: { values: ["ProjectOverview.md", "project/**", "release/**", "src/**", "test/**", "dogfood/rp-001-release-preparation/windows-e2e/**"] } },
  { kind: "process.spawn", scope: { values: ["node"] } },
];

const graphHeadPath =
  "dogfood/rp-001-release-preparation/integration-frontier-7/WI-RP-REGRESSION/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "Rp001FrontierEightExecutionSummary",
