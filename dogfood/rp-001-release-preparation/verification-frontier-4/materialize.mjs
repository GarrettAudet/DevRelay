...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-frontier-4");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-RP-ADAPTER-EFFECTS": Object.freeze({ argv: ["--test", "test/release-preparation-adapters.test.mjs"], command: "node --test test/release-preparation-adapters.test.mjs", tests: 6 }),
  "WI-RP-VERIFY-POLICY": Object.freeze({ argv: ["--test", "test/release-preparation-verification.test.mjs"], command: "node --test test/release-preparation-verification.test.mjs", tests: 6 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-frontier-3/WI-RP-NATIVE-MATERIALIZATION/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-frontier-4/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} RP-001 third-frontier focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Rp001FourthFrontierVerificationSummary", results };
