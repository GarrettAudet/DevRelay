...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-frontier-3");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-RP-NATIVE-MATERIALIZATION": Object.freeze({ argv: ["--test", "test/release-preparation-native-materializer.test.mjs"], command: "node --test test/release-preparation-native-materializer.test.mjs", tests: 8 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-frontier-2/WI-RP-IDENTITY-ROUTING/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-frontier-3/${workItemId}`;
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Rp001ThirdFrontierVerificationSummary", results };
