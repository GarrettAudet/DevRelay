...
const outRoot = resolve(root, "dogfood/rp-001-release-preparation/integration-frontier-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/rp-001-integration-frontier-1";
const sourceRef = "refs/heads/codex/rp-001-source-frontier-1";
const ids = Object.freeze([
  "WI-RP-ARTIFACT-CONTRACTS",
]);
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/rp-001-release-preparation/execution-frontier-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/rp-001-release-preparation/execution-frontier-1/execution-summary.json");
let graphSnapshot = json("dogfood/rp-001-release-preparation/execution-frontier-1/WI-RP-ARTIFACT-CONTRACTS/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/rp-001-release-preparation/verification-frontier-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/rp-001-release-preparation/execution-frontier-1/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Rp001FirstFrontierIntegrationSummary",
  targetRef,
  baseCommit,
