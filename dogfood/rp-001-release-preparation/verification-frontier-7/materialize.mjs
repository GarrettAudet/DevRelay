...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-frontier-7");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-RP-REGRESSION": Object.freeze({ argv: ["--test", "test/project-baseline.test.mjs", "test/release-preparation-regression.test.mjs", "test/work-execution-traceability.test.mjs"], command: "node --test test/project-baseline.test.mjs test/release-preparation-regression.test.mjs test/work-execution-traceability.test.mjs", tests: 8 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-frontier-6/WI-RP-TRACEABILITY/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-frontier-7/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} RP-001 seventh-frontier focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Rp001SeventhFrontierVerificationSummary", results };
