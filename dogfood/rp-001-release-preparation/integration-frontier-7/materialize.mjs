...
const outRoot = resolve(root, "dogfood/rp-001-release-preparation/integration-frontier-7");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/rp-001-integration-frontier-7";
const sourceRef = "refs/heads/codex/rp-001-source-frontier-7";
...
const baseCommit = json("dogfood/rp-001-release-preparation/integration-frontier-6/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/rp-001-release-preparation/execution-frontier-7/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/rp-001-release-preparation/execution-frontier-7/execution-summary.json");
let graphSnapshot = json("dogfood/rp-001-release-preparation/execution-frontier-7/WI-RP-REGRESSION/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/rp-001-release-preparation/verification-frontier-7/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/rp-001-release-preparation/execution-frontier-7/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Rp001SeventhFrontierIntegrationSummary",
  targetRef,
  baseCommit,
