...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-system-fix-2");
...
const verificationJobs = Object.freeze({
  "WI-RP-WINDOWS-E2E": Object.freeze({ argv: ["scripts/check-release-manifest.mjs"], command: "node scripts/check-release-manifest.mjs (isolated candidate checkout)", tests: 1 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-system-fix-1/WI-RP-TRACEABILITY/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-system-fix-2/${workItemId}`;
  ...
  const candidateRoot = mkdtempSync(join(tmpdir(), "devrelay-rp001-catalog-verification-"));
  const candidateRepository = join(candidateRoot, "repository");
  const testStarted = process.hrtime.bigint();
  let testRun;
  try {
    const clone = spawnSync("git", ["clone", "--quiet", "--shared", "--no-checkout", root, candidateRepository], {
      cwd: candidateRoot,
      encoding: null,
      windowsHide: true,
    });
    ...
    const checkout = spawnSync("git", ["checkout", "--quiet", "--detach", baselines.repositoryBase.revision], {
    ...
    copyFileSync(resolve(root, "release/0.10.0-rc.3.json"), resolve(candidateRepository, "release/0.10.0-rc.3.json"));
