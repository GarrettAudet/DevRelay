...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-system-fix-3");
...
const verificationJobs = Object.freeze({
  "WI-RP-WINDOWS-E2E": Object.freeze({ argv: [".devrelay-verify-project-closure.mjs"], command: "node .devrelay-verify-project-closure.mjs (isolated candidate checkout)", tests: 2 }),
});
...
const artifactRef = (artifactId, artifact) => ({
  ...
  uri: `memory://devrelay/ep001/${encodeURIComponent(artifactId)}`,
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-system-fix-2/WI-RP-WINDOWS-E2E/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-system-fix-3/${workItemId}`;
  ...
  const candidateRoot = mkdtempSync(join(tmpdir(), "dr-vfy-"));
  const candidateRepository = join(candidateRoot, "repository");
  const testStarted = process.hrtime.bigint();
  let testRun;
  try {
    const clone = spawnSync("git", ["-c", "core.longpaths=true", "clone", "--quiet", "--shared", "--no-checkout", root, candidateRepository], {
      cwd: candidateRoot,
      encoding: null,
      windowsHide: true,
    });
    if (clone.error) throw clone.error;
    if (clone.status !== 0) throw new Error(`isolated candidate clone failed: ${Buffer.from(clone.stderr ?? []).toString("utf8")}`);
    const checkout = spawnSync("git", ["-c", "core.longpaths=true", "checkout", "--quiet", "--detach", baselines.repositoryBase.revision], {
      cwd: candidateRepository,
      encoding: null,
      windowsHide: true,
    });
    if (checkout.error) throw checkout.error;
    if (checkout.status !== 0) throw new Error(`isolated candidate checkout failed: ${Buffer.from(checkout.stderr ?? []).toString("utf8")}`);
    for (const mutation of changeSetDraft.mutations) {
      const destination = resolve(candidateRepository, mutation.path);
      mkdirSync(dirname(destination), { recursive: true });
      copyFileSync(resolve(root, mutation.path), destination);
    }
    symlinkSync(resolve(root, "node_modules"), resolve(candidateRepository, "node_modules"), "junction");
    writeFileSync(resolve(candidateRepository, ".devrelay-verify-project-closure.mjs"), `import { spawnSync } from "node:child_process";\nfor (const argv of [["--test", "test/project-baseline.test.mjs"], ["scripts/check-release-manifest.mjs"]]) {\n  const result = spawnSync(process.execPath, argv, { encoding: null, windowsHide: true, env: process.env });\n  process.stdout.write(result.stdout ?? Buffer.alloc(0));\n  process.stderr.write(result.stderr ?? Buffer.alloc(0));\n  if (result.error) throw result.error;\n  if (result.status !== 0) process.exit(result.status ?? 1);\n}\n`, "utf8");
    testRun = spawnSync(process.execPath, job.argv, {
      cwd: candidateRepository,
      encoding: null,
      windowsHide: true,
  ...
  const receiptRecord = api.recordExecutionReceipt({
    effect: {
      ...
      argv: job.argv,
      cwd: "<isolated-candidate-checkout>",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} RP-001 focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Rp001SystemVerificationRepairVerificationSummary", results };
