...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-frontier-8");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-RP-WINDOWS-E2E": Object.freeze({ argv: ["--test", "test/release-preparation-traceability.test.mjs"], command: "node --test test/release-preparation-traceability.test.mjs", tests: 6 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-frontier-7/WI-RP-REGRESSION/post-repository-snapshot.json"),
};

const windowsSummary = readJson("dogfood/rp-001-release-preparation/windows-e2e/windows-e2e-summary.json");
const releaseCheckOutput = readFileSync(resolve(root, "dogfood/rp-001-release-preparation/windows-e2e/raw/release-check.stdout.txt"), "utf8");
assert.equal(windowsSummary.kind, "Rp001InstalledPackageVerificationReceipt");
assert.equal(windowsSummary.outcome, "pass");
assert.equal(windowsSummary.scenarioReceipt.outcome, "pass");
assert.equal(windowsSummary.scenarioReceipt.gate.summary.decision, "ready");
assert.equal(windowsSummary.scenarioReceipt.verification.obligations.length, 14);
assert.equal(windowsSummary.scenarioReceipt.materialization.replayEffectCalls, 0);
assert.equal(windowsSummary.scenarioReceipt.recovery.replayEffectCalls, 0);
assert.equal(windowsSummary.noPublication.publicRegistryCalls, 0);
assert.match(releaseCheckOutput, /ℹ tests 1154/u);
assert.match(releaseCheckOutput, /ℹ pass 1152/u);
assert.match(releaseCheckOutput, /ℹ fail 0/u);
assert.match(releaseCheckOutput, /Package verification passed: 398 exact catalog-bound files and 194 installed export targets\./u);
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-frontier-8/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent verifier confirmed the authoritative Windows receipt and passed all ${job.tests} RP-001 eighth-frontier focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Rp001EighthFrontierVerificationSummary", results };
