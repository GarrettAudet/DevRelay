...
const verificationRoot = resolve(root, "dogfood/rp-001-release-preparation/verification-frontier-6");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-RP-TRACEABILITY": Object.freeze({ argv: ["--test", "test/release-preparation-traceability.test.mjs"], command: "node --test test/release-preparation-traceability.test.mjs", tests: 6 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/rp-001-release-preparation/integration-frontier-5/WI-RP-GATE-SUMMARY/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/rp-001-release-preparation/execution-frontier-6/${workItemId}`;
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Rp001SixthFrontierVerificationSummary", results };
