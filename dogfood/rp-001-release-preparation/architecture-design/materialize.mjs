...
import { buildReleaseArchitecture } from "./release-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/rp-001-release-preparation/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/rp-001-release-preparation/architecture-design";
...
const projectContext = load("dogfood/rp-001-release-preparation/project-context.json");
const repositorySnapshot = load("dogfood/rp-001-release-preparation/repository-snapshot.json");
const architectureBaseline = load(
  "project/architecture-baseline.json",
);
const ownerDecisionPath = "docs/specs/rp-001-release-preparation/owner-architecture-decisions.json";
...
const projectContextRef = artifactRef(
  "project-context-devrelay-rp-001-release-preparation-v1",
  "https://devrelay.dev/artifacts/project-context/v1",
  "application/vnd.devrelay.project-context+json",
  sha256(read("dogfood/rp-001-release-preparation/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/rp-001-release-preparation/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  "repository-snapshot-devrelay-a18e6fb-rp-001-release-preparation",
  "https://devrelay.dev/artifacts/repository-snapshot/v1",
  "application/vnd.devrelay.repository-snapshot+json",
  sha256(read("dogfood/rp-001-release-preparation/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/rp-001-release-preparation/repository-snapshot.json",
);
...
const ownerDecisionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/docs/specs/rp-001-release-preparation/owner-architecture-decisions.json",
);
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-rp-001-release-preparation-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-rp-001-release-preparation-architecture-v1",
...
const discoveryDecision = {
  ...
  decisionId: "architecture-discovery-bypass-rp-001-release-preparation-v1",
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay RP-001 ReleasePreparation and ReleaseVerification")} ${dslQuote("Profile-bound release candidate preparation, stored-byte verification, readiness, and no-publication authority")} {`,
    "  !identifiers flat",
...
const designRef = fileRef(
  "openspec-design-rp-001-release-preparation-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-rp-001-release-preparation-v2",
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/rp-001-release-preparation/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-RP-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
    { section: "technicalDesign", entityIds: [technicalDesign.technicalDesignId], jsonPointers: ["/sections/technicalDesign"] },
    { section: "interfaceIntent", entityIds: interfaces.filter(({ id }) => id.startsWith("IF-RP-")).map(({ id }) => id), jsonPointers: ["/sections/interfaceIntent"] },
    { section: "architectureConstraints", entityIds: constraints.filter(({ id }) => id.startsWith("CON-RP-")).map(({ id }) => id), jsonPointers: ["/sections/architectureConstraints"] },
  ]),
  nativeEntry("NA-RP-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-RP-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-RP-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-RP-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-RP-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-RP-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-RP-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by the approved RP-001 ReleasePreparation and ReleaseVerification design.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the approved RP-001 release preparation and verification architecture.`,
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-RP-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the approved RP-001 ReleasePreparation and ReleaseVerification architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-rp-001-release-preparation-v2",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-rp-001-release-preparation-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-RP-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-rp-001-release-preparation-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-RP-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/rp-001-release-preparation/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/rp-001-release-preparation/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/rp-001-release-preparation",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-rp-001-release-preparation-v2",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-rp-001-release-preparation-v2",
  runId: "rp-001-release-preparation-architecture-run-v2",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "rp-001-release-preparation", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/rp-001-release-preparation/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/rp-001-release-preparation/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/rp-001-release-preparation/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(projectContextRef, read("dogfood/rp-001-release-preparation/project-context.json"));
register(repositorySnapshotRef, read("dogfood/rp-001-release-preparation/repository-snapshot.json"));
register(architectureBaselineRef, read("project/architecture-baseline.json"));
register(ownerDecisionRef, ownerDecisionBytes);
register(architectureBaselineProjectContextRef, read("dogfood/ep-001-environment-preparation/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/ep-001-environment-preparation/repository-snapshot.json"));
...
const traceabilityGraph = createTraceabilityGraphService({
  graphId: "traceability-graph-devrelay-rp-001-release-preparation",
  projectId: "devrelay",
...
if (process.argv.includes("--promote")) {
  const { promoteReleaseArchitecture } = await import("./promote.mjs");
  const promotion = await promoteReleaseArchitecture({
    root,
    dir,
    read,
    fileRef,
