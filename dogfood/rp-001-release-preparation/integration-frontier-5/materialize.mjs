...
const outRoot = resolve(root, "dogfood/rp-001-release-preparation/integration-frontier-5");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/rp-001-integration-frontier-5";
const sourceRef = "refs/heads/codex/rp-001-source-frontier-5";
...
const baseCommit = json("dogfood/rp-001-release-preparation/integration-frontier-4/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/rp-001-release-preparation/execution-frontier-5/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/rp-001-release-preparation/execution-frontier-5/execution-summary.json");
let graphSnapshot = json("dogfood/rp-001-release-preparation/execution-frontier-5/WI-RP-GATE-SUMMARY/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/rp-001-release-preparation/verification-frontier-5/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/rp-001-release-preparation/execution-frontier-5/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Rp001FifthFrontierIntegrationSummary",
  targetRef,
  baseCommit,
