...
const outRoot = resolve(root, "dogfood/rp-001-release-preparation/integration-frontier-3");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/rp-001-integration-frontier-3";
const sourceRef = "refs/heads/codex/rp-001-source-frontier-3";
const ids = Object.freeze([
  "WI-RP-NATIVE-MATERIALIZATION",
]);
...
const fullRef = (artifactId, digest, schema = "https://devrelay.dev/internal/v1", mediaType = "application/json") => ({ artifactId, schema, mediaType, digest, uri: `memory://rp001/${artifactId}` });
...
const baseCommit = json("dogfood/rp-001-release-preparation/integration-frontier-2/integration-summary.json").finalCommit;
const sourceRoot = mkdtempSync(join(tmpdir(), "devrelay-rp001-source-"));
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/rp-001-release-preparation/execution-frontier-3/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/rp-001-release-preparation/execution-frontier-3/execution-summary.json");
let graphSnapshot = json("dogfood/rp-001-release-preparation/execution-frontier-3/WI-RP-NATIVE-MATERIALIZATION/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/rp-001-release-preparation/verification-frontier-3/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/rp-001-release-preparation/execution-frontier-3/${workItemId}/change-set-draft.json`);
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-RP001-${workItemId}`,
  });
...
const summary = {
  apiVersion: API,
  kind: "Rp001ThirdFrontierIntegrationSummary",
  targetRef,
  baseCommit,
