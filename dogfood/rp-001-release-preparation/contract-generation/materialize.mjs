...
const RUN_ID = "rp-001-release-preparation-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-rp-001-release-preparation-v1";
const GRAPH_ID = "devrelay/rp-001-release-preparation-contract-change";
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== currentContractBaseline.value.contracts.length + 5) {
  throw new Error(`expected prior contract count plus five required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-RP-001-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-RP-001-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-pm-001-v1",
);
...
const moduleResult = {
  ...
  evidence: [
    ...
    },
    {
      ...
      summary: "Core computed a deterministic five-add contract diff against the exact current ContractBaseline.",
    },
  ],
  diagnostics: [],
};
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-RP-001-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-RP-001-CONTRACT-CHANGE-001",
...
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-RP-001-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
