...
const outRoot = resolve(root, "dogfood/rp-001-release-preparation/integration-system-fix-4");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/rp-001-integration-system-fix-4";
const sourceRef = "refs/heads/codex/rp-001-source-system-fix-4";
...
const baseCommit = json("dogfood/rp-001-release-preparation/integration-system-fix-3/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/rp-001-release-preparation/execution-system-fix-4/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/rp-001-release-preparation/execution-system-fix-4/execution-summary.json");
let graphSnapshot = json("dogfood/rp-001-release-preparation/execution-system-fix-4/WI-RP-WINDOWS-E2E/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/rp-001-release-preparation/verification-system-fix-4/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/rp-001-release-preparation/execution-system-fix-4/${workItemId}/change-set-draft.json`);
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-EP001-${workItemId}-REPAIR-004`,
  });
  ...
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-${workItemId}-REPAIR-004`,
...
const summary = {
  apiVersion: API,
  kind: "Rp001FinalReleaseCatalogRepairIntegrationSummary",
  targetRef,
  baseCommit,
