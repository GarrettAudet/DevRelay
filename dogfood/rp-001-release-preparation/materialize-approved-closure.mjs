...
const waveBytes = fs.readFileSync(
...
const responses = {
  ...
  responseSetId: "RCRS-RP-001-WAVE-1",
  wave: {
    waveId: wave.waveId,
    digest: sha256Digest(waveBytes),
  },
  ownerStatement: "Approve all",
...
const decisions = {
  ...
  decisionSetId: "owner-decisions-rp-001-release-preparation-v1",
  sourceResponseSet: {
    responseSetId: responses.responseSetId,
    digest: sha256Digest(
      Buffer.from(`${JSON.stringify(responses, null, 2)}\n`, "utf8"),
    ),
  },
...
fs.writeFileSync(
  path.join(directory, "owner-decisions.md"),
  `${decisionMarkdown.trimEnd()}\n`,
  "utf8",
);

const evidenceRef =
  `dogfood/rp-001-release-preparation/owner-decisions.json#${decisions.contentDigest}`;
...
if (
  ...
  assessment.weightedCoverage !== 1 ||
  assessment.blockingUnknowns.length !== 0 ||
  assessment.unresolvedContradictions.length !== 0
) {
  throw new Error("RP-001 requirements did not close after exact owner approval.");
}
