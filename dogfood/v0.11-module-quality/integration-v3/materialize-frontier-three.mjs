...
const outRoot = resolve(root, "dogfood/v0.11-module-quality/integration-v3");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/v0.11-integration-v3";
const sourceRef = "refs/heads/codex/v0.11-source-candidate-v3-frontier3";
const ids = Object.freeze([
  "WI-MQ-DESKTOP-SKILLS",
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/v0.11-module-quality/execution-v3/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/v0.11-module-quality/execution-v3/execution-summary-frontier-three-final.json");
let graphSnapshot = json("dogfood/v0.11-module-quality/execution-v3/WI-MQ-LIFECYCLE-REPORT/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/v0.11-module-quality/verification-v3/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/v0.11-module-quality/execution-v3/${workItemId}/change-set-draft.json`);
  ...
  const integrationPolicyRef = fullRef(`CI-POLICY-G3-${workItemId}`, api.sha256Digest(integrationPolicyBytes));
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    subjectId: `CI-SUBJECT-G3-${workItemId}`,
    bindingId: `CI-BINDING-G3-${workItemId}`,
    workItem,
    workItemRef,
    verificationSubject,
    ...
    idempotencyKey: `CI-V011-G3-${workItemId}`,
  });
  const sourceCommit = sourceCommits.get(workItemId);
  const plan = api.buildChangeIntegrationPlan({ planId: `CI-PLAN-G3-${workItemId}`, validatedInput, verifiedChangeBytes, sourceCommit, strategy: "fast-forward" });
  ...
  const localAdapter = api.createLocalGitIntegrationAdapter({
    ...configuration,
    persistNativeEvidence: async (bytes) => nativeStore.put(bytes, { artifactId: `NATIVE-G3-${workItemId}-${nativeStore.values.size}`, mediaType: "application/vnd.devrelay.local-git-native-evidence+json" }),
  ...
  const controller = api.createChangeIntegrationCheckpointController({
    ...
    persistRawResult: async (bytes) => rawStore.put(bytes, { artifactId: `RAW-G3-${workItemId}`, schema: "https://devrelay.dev/contracts/change-integration-artifacts.schema.json#/$defs/rawEffectResult", mediaType: "application/vnd.devrelay.raw-integration-effect-result+json" }),
    readRawResult: rawStore.readArtifact.bind(rawStore),
  });
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-G3-${workItemId}`,
  ...
  const postRef = resultStore.put(postBytes, { artifactId: `POST-G3-${workItemId}`, schema: "https://devrelay.dev/artifacts/repository-snapshot/v1", mediaType: "application/vnd.devrelay.repository-snapshot+json" });
  ...
  const proofRef = resultStore.put(proofBytes, { artifactId: `INCORPORATION-G3-${workItemId}`, schema: "https://devrelay.dev/internal/change-integration/repository-incorporation-proof/v1", mediaType: "application/vnd.devrelay.repository-incorporation-proof+json" });
  ...
  const resultLoaded = loaded(result, fullRef(`CI-RESULT-G3-${workItemId}`, api.canonicalJsonDigest(result), "https://devrelay.dev/artifacts/module-result/v1", "application/json"));
  const context = {
    ...
    loadedAttachments: {
      result: resultLoaded,
      trace: loaded(trace, fullRef(`CI-TRACE-G3-${workItemId}`, api.canonicalJsonDigest(trace), "https://devrelay.dev/artifacts/change-integration-traceability-input/v1", "application/json")),
...
const summary = {
  apiVersion: API,
  kind: "V011ModuleQualityGeneration3ThirdFrontierIntegrationSummary",
  targetRef,
  baseCommit,
...
writeFileSync(resolve(outRoot, "integration-summary-frontier-three.json"), `${api.canonicalJson(summary)}\n`, "utf8");
