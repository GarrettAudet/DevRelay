...
const repositoryRevision = execFileSync("git", ["rev-parse", "refs/heads/codex/v0.11-integration-v3"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
const repositoryTree = execFileSync("git", ["rev-parse", repositoryRevision + "^{tree}"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
const repositoryValue = { apiVersion: API, kind: "RepositorySnapshot", repository: root.replaceAll("\\", "/"), revision: repositoryRevision, treeDigest: canonicalJsonDigest({ commit: repositoryRevision, tree: repositoryTree }), includedPaths: [".codex/**", "dogfood/**", "scripts/**", "src/**", "test/**"], excludedPaths: [".git/**", "node_modules/**"] };
...
const integrationRoot = path.join(root, "dogfood/v0.11-module-quality/integration-v2");
const completedIds = fs.readdirSync(integrationRoot, { withFileTypes: true })
  .filter((entry) => entry.isDirectory() && fs.existsSync(path.join(integrationRoot, entry.name, "integrated-change-record.json")))
  .map((entry) => entry.name)
  .filter((workItemId) => !["WI-MQ-DESKTOP-SKILLS", "WI-MQ-GODOT-COMPATIBILITY", "WI-MQ-LIFECYCLE-REPORT"].includes(workItemId))
  .sort((left, right) => left.localeCompare(right, "en"));
const completionFacts = completedIds.map((workItemId) => {
  const record = readJson("dogfood/v0.11-module-quality/integration-v2/" + workItemId + "/integrated-change-record.json");
  const approval = readJson("dogfood/v0.11-module-quality/verification-v2/" + workItemId + "/gate-approval.json");
  return {
    workItemId,
    authority: "approved",
    integrationRef: { artifactId: record.recordId, digest: record.recordDigest },
    evidence: [{ artifactId: approval.approvalId, digest: approval.approvalDigest }],
  };
...
const commonPermissions = [
  { kind: "filesystem.write", scope: { values: [".codex/**", "dogfood/**", "scripts/**", "src/**", "test/**"] } },
  { kind: "process.spawn", scope: { values: ["node", "python"] } },
];
const jobs = [
  {
    workItemId: "WI-MQ-DESKTOP-SKILLS",
    attemptId: "ATT-MQ-DESKTOP-SKILLS-003",
    files: [
      ".codex/skills/devrelay-cycle/SKILL.md",
      ".codex/skills/devrelay-cycle/agents/openai.yaml",
      ".codex/skills/devrelay-godot-release/SKILL.md",
      ".codex/skills/devrelay-godot-release/agents/openai.yaml",
      ".codex/skills/devrelay-plugin-conformance/SKILL.md",
      ".codex/skills/devrelay-plugin-conformance/agents/openai.yaml",
      ".codex/skills/devrelay-trace-query/SKILL.md",
      ".codex/skills/devrelay-trace-query/agents/openai.yaml",
    ],
  },
  {
    workItemId: "WI-MQ-GODOT-COMPATIBILITY",
    attemptId: "ATT-MQ-GODOT-COMPATIBILITY-003",
    files: [
      "src/godot-compatibility.mjs",
      "dogfood/v0.11-module-quality/providers/materialize-godot-compatibility.mjs",
      "dogfood/v0.11-module-quality/providers/godot-compatibility-policy.json",
      "dogfood/v0.11-module-quality/providers/godot-compatibility-decision.json",
    ],
  },
  {
    workItemId: "WI-MQ-LIFECYCLE-REPORT",
    attemptId: "ATT-MQ-LIFECYCLE-REPORT-003",
    files: [
      "src/index.mjs",
      "src/module-quality-report.mjs",
      "scripts/materialize-v0.11-module-quality-report.mjs",
      "dogfood/v0.11-module-quality/lifecycle-report/module-quality-report.json",
      "dogfood/v0.11-module-quality/lifecycle-report/module-quality-report.md",
      "test/v0.11-frontier-three.test.mjs",
    ],
  },
].map((job) => ({
  ...job,
  evidence: {
    artifactId: "EV-" + job.workItemId + "-F3-G3",
    command: "node --test test/v0.11-frontier-three.test.mjs",
    tests: 5,
  },
  permissions: commonPermissions,
}));

const graphHeadPath = "dogfood/v0.11-module-quality/integration-v2/WI-MQ-REQUIREMENTS-STRATEGIES/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "V011ModuleQualityGeneration3ThirdFrontierExecutionSummary",
...
writeJson("execution-summary-frontier-three.json", summary);
