...
assert.equal(gateCandidateRef.digest, "sha256:352af3c794c94e857d315169f5b94307a69a7f2e7f259cce6f5dcfe132cc151a");

const ownerApprovalSourceBytes = read(`${dogfoodRelative}/work-breakdown-gate-owner-approval-source.md`);
const ownerApprovalSourceRef = artifactRef(
  "work-breakdown-gate-owner-approval-source-v0.11-module-quality-v2",
  {
...
const ownerApproval = {
  ...
  approvalId: "work-breakdown-gate-owner-approval-v0.11-module-quality-v2",
  decision: "approved",
...
const ownerApprovalBytes = writeJson("work-breakdown-gate-owner-approval.json", ownerApproval);
...
validateWorkBreakdownArtifact(workBreakdownBaseline, { ref: workBreakdownBaselineRef });
...
validateWorkBreakdownArtifact(promotedProjectState, { ref: promotedProjectStateRef });

const promotionProof = {
  apiVersion: API_VERSION,
  kind: "WorkBreakdownGatePromotionProof",
  proofId: "work-breakdown-gate-promotion-v0.11-module-quality-v2",
  status: "promoted",
...
const historyDir = path.join(root, "project", "history", "work-breakdown", currentBaseline.version);
...
const pending = {
  ...
  transactionId: "work-breakdown-promotion-v0.11-module-quality-v2",
  status: "prepared",
  candidate: workBreakdownChangeSetRef,
  gateCandidate: gateCandidateRef,
  ownerApproval: ownerApprovalRef,
...
const pendingPath = path.join(root, "project", "work-breakdown-promotion.pending.json");
...
fs.writeFileSync(pendingPath, `${JSON.stringify(committed, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  status: "WORK_BREAKDOWN_PROMOTED",
  candidate: workBreakdownChangeSetRef.digest,
  gateCandidate: gateCandidateRef.digest,
  ownerApproval: ownerApprovalRef.digest,
  baseline: workBreakdownBaselineRef.digest,
  promotionProof: promotionProofRef.digest,
  workItems: workBreakdownBaseline.workItems.length,
  coverage: workBreakdownBaseline.coverageDispositions.length,
  replayAdapterCalls: adapterCalls.length - callsBeforeCheckpointVerification,
