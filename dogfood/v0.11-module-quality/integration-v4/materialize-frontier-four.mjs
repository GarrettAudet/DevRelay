...
const outRoot = resolve(root, "dogfood/v0.11-module-quality/integration-v4");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/v0.11-integration-v3";
const sourceRef = "refs/heads/codex/v0.11-source-candidate-v4-frontier4";
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/v0.11-module-quality/execution-v4/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/v0.11-module-quality/execution-v4/execution-summary-frontier-four.json");
let graphSnapshot = json("dogfood/v0.11-module-quality/execution-v4/WI-MQ-WINDOWS-E2E/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/v0.11-module-quality/verification-v4/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/v0.11-module-quality/execution-v4/${workItemId}/change-set-draft.json`);
  ...
  const integrationPolicyRef = fullRef(`CI-POLICY-G4-${workItemId}`, api.sha256Digest(integrationPolicyBytes));
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    subjectId: `CI-SUBJECT-G4-${workItemId}`,
    bindingId: `CI-BINDING-G4-${workItemId}`,
    workItem,
    workItemRef,
    verificationSubject,
    ...
    idempotencyKey: `CI-V011-G4-${workItemId}`,
  });
  const sourceCommit = sourceCommits.get(workItemId);
  const plan = api.buildChangeIntegrationPlan({ planId: `CI-PLAN-G4-${workItemId}`, validatedInput, verifiedChangeBytes, sourceCommit, strategy: "fast-forward" });
  ...
  const localAdapter = api.createLocalGitIntegrationAdapter({
    ...configuration,
    persistNativeEvidence: async (bytes) => nativeStore.put(bytes, { artifactId: `NATIVE-G4-${workItemId}-${nativeStore.values.size}`, mediaType: "application/vnd.devrelay.local-git-native-evidence+json" }),
  ...
  const controller = api.createChangeIntegrationCheckpointController({
    ...
    persistRawResult: async (bytes) => rawStore.put(bytes, { artifactId: `RAW-G4-${workItemId}`, schema: "https://devrelay.dev/contracts/change-integration-artifacts.schema.json#/$defs/rawEffectResult", mediaType: "application/vnd.devrelay.raw-integration-effect-result+json" }),
    readRawResult: rawStore.readArtifact.bind(rawStore),
  });
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-G4-${workItemId}`,
  ...
  const postRef = resultStore.put(postBytes, { artifactId: `POST-G4-${workItemId}`, schema: "https://devrelay.dev/artifacts/repository-snapshot/v1", mediaType: "application/vnd.devrelay.repository-snapshot+json" });
  ...
  const proofRef = resultStore.put(proofBytes, { artifactId: `INCORPORATION-G4-${workItemId}`, schema: "https://devrelay.dev/internal/change-integration/repository-incorporation-proof/v1", mediaType: "application/vnd.devrelay.repository-incorporation-proof+json" });
  ...
  const resultLoaded = loaded(result, fullRef(`CI-RESULT-G4-${workItemId}`, api.canonicalJsonDigest(result), "https://devrelay.dev/artifacts/module-result/v1", "application/json"));
  const context = {
    ...
    loadedAttachments: {
      result: resultLoaded,
      trace: loaded(trace, fullRef(`CI-TRACE-G4-${workItemId}`, api.canonicalJsonDigest(trace), "https://devrelay.dev/artifacts/change-integration-traceability-input/v1", "application/json")),
...
const summary = {
  apiVersion: API,
  kind: "V011ModuleQualityGeneration4FourthFrontierIntegrationSummary",
  targetRef,
  baseCommit,
...
writeFileSync(resolve(outRoot, "integration-summary-frontier-four.json"), `${api.canonicalJson(summary)}\n`, "utf8");
