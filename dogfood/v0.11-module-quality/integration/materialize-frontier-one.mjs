...
const outRoot = resolve(root, "dogfood/v0.11-module-quality/integration");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/v0.11-integration";
const sourceRef = "refs/heads/codex/v0.11-source-candidate";
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/v0.11-module-quality/execution/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/v0.11-module-quality/execution/execution-summary.json");
let graphSnapshot = json("dogfood/v0.11-module-quality/execution/WI-MQ-TRACE-QUERY/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/v0.11-module-quality/verification/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/v0.11-module-quality/execution/${workItemId}/change-set-draft.json`);
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-V011-${workItemId}`,
  });
...
const summary = {
  apiVersion: API,
  kind: "V011ModuleQualityFirstFrontierIntegrationSummary",
  targetRef,
  baseCommit,
