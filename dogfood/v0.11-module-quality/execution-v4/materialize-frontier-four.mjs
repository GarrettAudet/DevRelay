...
for (const source of completionSources) {
  const sourceRoot = path.join(root, source.integration);
  for (const entry of fs.readdirSync(sourceRoot, { withFileTypes: true })) {
    if (!entry.isDirectory() || !fs.existsSync(path.join(sourceRoot, entry.name, "integrated-change-record.json"))) continue;
    const record = readJson(source.integration + "/" + entry.name + "/integrated-change-record.json");
    const approval = readJson(source.verification + "/" + entry.name + "/gate-approval.json");
...
const completionFacts = [...completionByWorkItem.values()].sort((left, right) => left.workItemId.localeCompare(right.workItemId, "en"));
...
const collectFiles = (relativeRoot) => {
  const values = [];
  const walk = (relative) => {
    for (const entry of fs.readdirSync(path.join(root, relative), { withFileTypes: true })) {
      if ([".git", ".runtime", "candidate", "node_modules"].includes(entry.name)) continue;
...
const jobs = [
  {
    workItemId: "WI-MQ-WINDOWS-E2E",
    attemptId: "ATT-MQ-WINDOWS-E2E-004",
    files: [
      ...collectFiles("dogfood/v0.11-module-quality/windows-e2e"),
      "dogfood/v0.11-module-quality/execution-v4/materialize-frontier-four.mjs",
      "dogfood/v0.11-module-quality/verification-v4/materialize-frontier-four.mjs",
      "dogfood/v0.11-module-quality/integration-v4/materialize-frontier-four.mjs",
      "test/v0.11-windows-e2e.test.mjs",
    ].sort((left, right) => left.localeCompare(right, "en")),
  },
].map((job) => ({
  ...job,
  evidence: {
    artifactId: "EV-" + job.workItemId + "-F4-G4",
    command: "node --test test/v0.11-windows-e2e.test.mjs",
    tests: 3,
  },
  permissions: commonPermissions,
}));

const graphHeadPath = "dogfood/v0.11-module-quality/integration-v3/WI-MQ-LIFECYCLE-REPORT/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "V011ModuleQualityGeneration4FourthFrontierExecutionSummary",
...
for (const job of jobs) {
  ...
  const invocationId = `WE-V011-${item.id}-004`;
...
writeJson("execution-summary-frontier-four.json", summary);
