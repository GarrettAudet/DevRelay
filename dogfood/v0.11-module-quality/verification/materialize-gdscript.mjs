...
for (const workItemId of workItemIds) {
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all 10 V0.11 first-frontier tests for the exact ${workItemId} candidate bytes; this is contributing item evidence and does not by itself satisfy the shared Godot compatibility or release criteria.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "V011ModuleQualityGdscriptVerificationSummary", results };
summary.summaryDigest = api.canonicalJsonDigest(summary);
writeFileSync(resolve(verificationRoot, "gdscript-verification-summary.json"), `${api.canonicalJson(summary)}\n`, "utf8");
