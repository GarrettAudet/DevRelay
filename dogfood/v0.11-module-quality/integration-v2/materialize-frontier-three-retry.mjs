...
const sourceRef = "refs/heads/codex/v0.11-source-candidate-v2-frontier3-retry";
const ids = Object.freeze([
  "WI-MQ-GODOT-COMPATIBILITY",
  "WI-MQ-LIFECYCLE-REPORT",
]);
...
const graphSummary = { resultGraph: json("dogfood/v0.11-module-quality/integration-v2/WI-MQ-DESKTOP-SKILLS/traceability-merge-receipt.json").resultGraph };
let graphSnapshot = json("dogfood/v0.11-module-quality/integration-v2/WI-MQ-DESKTOP-SKILLS/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const integrationPolicyRef = fullRef(`CI-POLICY-G2-${workItemId}`, api.sha256Digest(integrationPolicyBytes));
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    subjectId: `CI-SUBJECT-G2-${workItemId}`,
    bindingId: `CI-BINDING-G2-${workItemId}`,
    workItem,
    workItemRef,
    verificationSubject,
    ...
    idempotencyKey: `CI-V011-G2-${workItemId}`,
  });
  const sourceCommit = sourceCommits.get(workItemId);
  const plan = api.buildChangeIntegrationPlan({ planId: `CI-PLAN-G2-${workItemId}`, validatedInput, verifiedChangeBytes, sourceCommit, strategy: "fast-forward" });
  ...
  const localAdapter = api.createLocalGitIntegrationAdapter({
    ...configuration,
    persistNativeEvidence: async (bytes) => nativeStore.put(bytes, { artifactId: `NATIVE-G2-${workItemId}-${nativeStore.values.size}`, mediaType: "application/vnd.devrelay.local-git-native-evidence+json" }),
  ...
  const controller = api.createChangeIntegrationCheckpointController({
    ...
    persistRawResult: async (bytes) => rawStore.put(bytes, { artifactId: `RAW-G2-${workItemId}`, schema: "https://devrelay.dev/contracts/change-integration-artifacts.schema.json#/$defs/rawEffectResult", mediaType: "application/vnd.devrelay.raw-integration-effect-result+json" }),
    readRawResult: rawStore.readArtifact.bind(rawStore),
  });
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-G2-${workItemId}`,
  ...
  const postRef = resultStore.put(postBytes, { artifactId: `POST-G2-${workItemId}`, schema: "https://devrelay.dev/artifacts/repository-snapshot/v1", mediaType: "application/vnd.devrelay.repository-snapshot+json" });
  ...
  const proofRef = resultStore.put(proofBytes, { artifactId: `INCORPORATION-G2-${workItemId}`, schema: "https://devrelay.dev/internal/change-integration/repository-incorporation-proof/v1", mediaType: "application/vnd.devrelay.repository-incorporation-proof+json" });
  ...
  const resultLoaded = loaded(result, fullRef(`CI-RESULT-G2-${workItemId}`, api.canonicalJsonDigest(result), "https://devrelay.dev/artifacts/module-result/v1", "application/json"));
  const context = {
    ...
    loadedAttachments: {
      result: resultLoaded,
      trace: loaded(trace, fullRef(`CI-TRACE-G2-${workItemId}`, api.canonicalJsonDigest(trace), "https://devrelay.dev/artifacts/change-integration-traceability-input/v1", "application/json")),
...
const summary = {
  apiVersion: API,
  kind: "V011ModuleQualityGeneration2ThirdFrontierRetryIntegrationSummary",
  targetRef,
  baseCommit,
...
writeFileSync(resolve(outRoot, "integration-summary-frontier-three-retry.json"), `${api.canonicalJson(summary)}\n`, "utf8");
