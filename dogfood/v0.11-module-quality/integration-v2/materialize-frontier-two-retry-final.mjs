...
const outRoot = resolve(root, "dogfood/v0.11-module-quality/integration-v2");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/v0.11-integration-v2";
const sourceRef = "refs/heads/codex/v0.11-source-candidate-v2-frontier2-retry2";
const ids = Object.freeze(["WI-MQ-REQUIREMENTS-STRATEGIES"]);
...
const fullRef = (artifactId, digest, schema = "https://devrelay.dev/internal/v1", mediaType = "application/json") => ({ artifactId, schema, mediaType, digest, uri: `memory://v011/${artifactId}` });
...
const baseCommit = git(root, "rev-parse", "--verify", targetRef);
const sourceRoot = mkdtempSync(join(tmpdir(), "devrelay-v011-source-"));
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/v0.11-module-quality/execution-v2/${workItemId}/change-set-draft.json`);
    ...
    git(sourceRepo, "commit", "--allow-empty", "-m", `V0.11 ${workItemId}`);
...
let graphSnapshot = json("dogfood/v0.11-module-quality/integration-v2/WI-MQ-LOCAL-METRICS/traceability-graph-snapshot.json");
const graphStore = api.createInMemoryTraceabilityStore();
const graphBytes = canonicalBytes(graphSnapshot);
const graphHeadRef = fullRef("traceability-graph-devrelay-work-breakdown-r" + graphSnapshot.revision, api.sha256Digest(graphBytes), "https://devrelay.dev/artifacts/traceability-graph-snapshot/v1", "application/vnd.devrelay.traceability-graph+json");
...
graphStore.restore(graphSnapshot.graphId, [{ ref: graphHeadRef, bytes: graphBytes, value: graphSnapshot }, ...restoredUpdates.values()], graphHeadRef);
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/v0.11-module-quality/verification-v2/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/v0.11-module-quality/execution-v2/${workItemId}/change-set-draft.json`);
  ...
  const targetReference = ref(`REPO-V011-INTEGRATION-${currentCommit.slice(0, 12).toUpperCase()}`, api.canonicalJsonDigest(targetSnapshot));
  const integrationPolicy = { kind: "IntegrationPolicy", version: "1.0.0", mode: "verified-fast-forward", targetRef };
  const integrationPolicyBytes = Buffer.from(`DevRelay V0.11 verified fast-forward policy for ${targetRef}\n`, "utf8");
  const integrationPolicyRef = fullRef(`CI-POLICY-G2R2-${workItemId}`, api.sha256Digest(integrationPolicyBytes));
  const configuration = { repositoryPath: root, gitExecutable: "git", timeoutMs: 30_000, maxOutputBytes: 1024 * 1024 };
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    subjectId: `CI-SUBJECT-G2R2-${workItemId}`,
    bindingId: `CI-BINDING-G2R2-${workItemId}`,
    workItem,
    workItemRef,
    verificationSubject,
    ...
    idempotencyKey: `CI-V011-G2R2-${workItemId}`,
  });
  const sourceCommit = sourceCommits.get(workItemId);
  const plan = api.buildChangeIntegrationPlan({ planId: `CI-PLAN-G2R2-${workItemId}`, validatedInput, verifiedChangeBytes, sourceCommit, strategy: "fast-forward" });
  ...
  const localAdapter = api.createLocalGitIntegrationAdapter({
    ...configuration,
    persistNativeEvidence: async (bytes) => nativeStore.put(bytes, { artifactId: `NATIVE-G2R2-${workItemId}-${nativeStore.values.size}`, mediaType: "application/vnd.devrelay.local-git-native-evidence+json" }),
  ...
  const controller = api.createChangeIntegrationCheckpointController({
    ...
    persistRawResult: async (bytes) => rawStore.put(bytes, { artifactId: `RAW-G2R2-${workItemId}`, schema: "https://devrelay.dev/contracts/change-integration-artifacts.schema.json#/$defs/rawEffectResult", mediaType: "application/vnd.devrelay.raw-integration-effect-result+json" }),
    readRawResult: rawStore.readArtifact.bind(rawStore),
  });
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-G2R2-${workItemId}`,
  ...
  const postRef = resultStore.put(postBytes, { artifactId: `POST-G2R2-${workItemId}`, schema: "https://devrelay.dev/artifacts/repository-snapshot/v1", mediaType: "application/vnd.devrelay.repository-snapshot+json" });
  ...
  const proofRef = resultStore.put(proofBytes, { artifactId: `INCORPORATION-G2R2-${workItemId}`, schema: "https://devrelay.dev/internal/change-integration/repository-incorporation-proof/v1", mediaType: "application/vnd.devrelay.repository-incorporation-proof+json" });
  ...
  const resultLoaded = loaded(result, fullRef(`CI-RESULT-G2R2-${workItemId}`, api.canonicalJsonDigest(result), "https://devrelay.dev/artifacts/module-result/v1", "application/json"));
  const context = {
    ...
    loadedAttachments: {
      result: resultLoaded,
      trace: loaded(trace, fullRef(`CI-TRACE-G2R2-${workItemId}`, api.canonicalJsonDigest(trace), "https://devrelay.dev/artifacts/change-integration-traceability-input/v1", "application/json")),
...
const summary = {
  apiVersion: API,
  kind: "V011ModuleQualityGeneration2SecondFrontierRetryIntegrationSummary",
  targetRef,
  baseCommit,
...
writeFileSync(resolve(outRoot, "integration-summary-frontier-two-retry-final.json"), `${api.canonicalJson(summary)}\n`, "utf8");
