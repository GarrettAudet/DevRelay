...
const fullRef = (
  ...
  schema,
  mediaType,
  uri: `memory://devrelay/v0.11/${encodeURIComponent(artifactId)}/${digest.slice(7)}.json`,
});
...
function restoreGraph() {
  const graphPath =
    "dogfood/v0.11-module-quality/integration-v4/WI-MQ-WINDOWS-E2E/traceability-graph-snapshot.json";
...
const windowsDogfood = readJson(
  "dogfood/v0.11-module-quality/windows-e2e/evidence/windows-godot-dogfood-summary.json",
);
const releaseCatalogBytes = readBytes("release/0.10.0-rc.1.json");
const historyBundleBytes = readBytes(
  "dogfood/v0.11-module-quality/windows-e2e/evidence/greeting-card-history.bundle",
);
const releaseEvidenceValue = seal(
  {
    apiVersion: API,
    kind: "V011ReleaseCandidateEvidenceSet",
    evidenceSetId: "RELEASE-EVIDENCE-DEVRELAY-V011-MODULE-QUALITY-001",
    ...
    windowsDesktopDogfood: ref(
      "WINDOWS-GODOT-DOGFOOD-SUMMARY-V011",
      windowsDogfood.summaryDigest,
    ),
    releaseCatalog: ref("devrelay-release-catalog-0.10.0-rc.1", sha256Digest(releaseCatalogBytes)),
    reconstructableDogfoodHistory: ref(
      "devrelay-v011-greeting-card-history-bundle",
...
const integrationSources = [
  { integration: "dogfood/v0.11-module-quality/integration-v2", verification: "dogfood/v0.11-module-quality/verification-v2" },
  { integration: "dogfood/v0.11-module-quality/integration-v3", verification: "dogfood/v0.11-module-quality/verification-v3" },
  { integration: "dogfood/v0.11-module-quality/integration-v4", verification: "dogfood/v0.11-module-quality/verification-v4" },
];
...
const completionFactSet = seal(
  {
    ...
    factSetId: "ICFS-DEVRELAY-V011-MODULE-QUALITY-001",
...
const systemPolicy = seal(
  {
    ...
    policyId: "SV-POLICY-DEVRELAY-V011-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const integratedSubject = seal(
  {
    ...
    subjectId: `DEVRELAY-V011-DESKTOP-WINDOWS-${releaseEvidence.value.evidenceSetDigest.slice(7, 19).toUpperCase()}`,
...
const reviewObligations = obligations.obligations
  ...
  .map(({ obligationId }) => obligationId);
const invocations = [
  invocationFor("SV-DEVRELAY-V011-TEST-001", TEST_SYSTEM_VERIFIER, testObligations),
  invocationFor("SV-DEVRELAY-V011-REVIEW-001", REVIEW_SYSTEM_VERIFIER, reviewObligations),
];
...
const verifier = async (invocation) => {
  ...
  if (invocation.verifier.id === TEST_SYSTEM_VERIFIER.id) {
    ...
    return adaptSystemTestResult({
      invocation,
      nativeBytes: bytes,
      nativeArtifact: ref("NATIVE-DEVRELAY-V011-TEST-001", sha256Digest(bytes)),
    });
  }
  const native = {
  ...
  return adaptSystemReviewResult({
    invocation,
    nativeBytes: bytes,
    nativeArtifact: ref("NATIVE-DEVRELAY-V011-REVIEW-001", sha256Digest(bytes)),
  });
};
...
const systemInvocation = {
  invocationId: "SYSTEM-VERIFICATION-DEVRELAY-V011-001",
...
const businessPolicy = seal(
  {
    ...
    policyId: "BA-POLICY-DEVRELAY-V011-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const businessEvidence = seal(
  {
    ...
    evidenceSetId: "BA-EVIDENCE-DEVRELAY-V011-DESKTOP-WINDOWS-001",
    requirementsBaseline: ref(requirements.ref.artifactId, requirements.ref.digest),
    projectOverviewBaseline: ref(overview.ref.artifactId, overview.ref.digest),
...
const requirementsInvocation = {
  invocationId: "REQUIREMENTS-BASELINE-SCOPE-RECONCILIATION-V011-001",
...
const businessInput = {
  checkpointId: "BA-DEVRELAY-V011-DESKTOP-WINDOWS-001",
...
const ownerApproval = seal(
  {
    ...
    approvalId: "BA-APPROVAL-DEVRELAY-V011-DESKTOP-WINDOWS-001",
...
let ownerCalls = 0;
const gateInput = {
  checkpointId: "BA-GATE-DEVRELAY-V011-DESKTOP-WINDOWS-001",
...
const businessInvocation = {
  invocationId: "BUSINESS-ACCEPTANCE-GATE-DEVRELAY-V011-001",
  module: {
...
const approvalRequest = seal(
  {
    ...
    requestId: "BA-APPROVAL-REQUEST-DEVRELAY-V011-DESKTOP-WINDOWS-001",
    candidate: ref(business.candidate.candidateId, business.candidate.candidateDigest),
    candidateRawDigest: sha256Digest(candidateBytes),
    technicalCoverage: ref(
      business.technicalCoverage.coverageId,
...
const finalProof = seal(
  {
    apiVersion: API,
    kind: "DevRelayV011SourceReleaseAcceptanceProof",
    proofId: "DEVRELAY-V011-SOURCE-RELEASE-ACCEPTANCE-001",
    sourceBaseCommit,
...
const summary = {
  apiVersion: API,
  kind: "DevRelayV011FinalAcceptanceSummary",
  releaseEvidence: releaseEvidenceRef,
...
writeText(
  "FINAL_ACCEPTANCE.md",
  `# DevRelay v0.11 final acceptance\n\n` +
