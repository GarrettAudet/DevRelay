...
const RUN_ID = "v0.11-module-quality-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-v0.11-module-quality-v1";
const GRAPH_ID = "devrelay/v0.11-module-quality-contract-change";
...
const currentContractBaseline = await loadedFile(
  "project/history/contracts/CB-DEVRELAY-008/contract-baseline.json",
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== currentContractBaseline.value.contracts.length + 8) {
  throw new Error(`expected prior contract count plus eight required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-V011-MODULE-QUALITY-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-V011-MODULE-QUALITY-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-v0.11-module-quality-v1",
);
...
const checkpointFile = await exactJsonFile(
  ...
  "application/json",
  "CG-CHECKPOINT-V011-MODULE-QUALITY-001",
);
...
const moduleResult = {
  ...
  evidence: [
    ...
    },
    {
      ...
      summary: "Core computed a deterministic eight-add contract diff against the exact current ContractBaseline.",
    },
  ],
  diagnostics: [],
};
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-V011-MODULE-QUALITY-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-V011-MODULE-QUALITY-CONTRACT-CHANGE-001",
...
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-V011-MODULE-QUALITY-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
...
await exactJsonFile(
  ...
  "application/json",
  "CG-RUNTIME-PROOF-V011-MODULE-QUALITY-001",
);
