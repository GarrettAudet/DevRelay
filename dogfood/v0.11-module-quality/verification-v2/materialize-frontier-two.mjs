...
for (const workItemId of workItemIds) {
  ...
  const testRun = spawnSync(process.execPath, ["--test", "test/v0.11-frontier-two.test.mjs"], {
    cwd: root,
    encoding: null,
    windowsHide: true,
  });
  ...
  const receiptRecord = api.recordExecutionReceipt({
    effect: {
      ...
      argv: ["--test", "test/v0.11-frontier-two.test.mjs"],
      cwd: ".",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: workItemId === "WI-MQ-GDSCRIPT-DISCOVERY" ? `The independent focused verifier passed all 10 V0.11 second-frontier tests for the exact ${workItemId} candidate bytes; this is contributing item evidence and does not by itself satisfy the shared Godot compatibility or release criteria.` : `The independent focused verifier passed all 10 V0.11 second-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "V011ModuleQualityGeneration2SecondFrontierVerificationSummary", results };
summary.summaryDigest = api.canonicalJsonDigest(summary);
writeFileSync(resolve(verificationRoot, "verification-summary-frontier-two.json"), `${api.canonicalJson(summary)}\n`, "utf8");
