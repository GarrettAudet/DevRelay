...
const executionRoot = resolve(root, "dogfood/v0.11-module-quality/execution-v2");
const verificationRoot = resolve(root, "dogfood/v0.11-module-quality/verification-v2");
mkdirSync(verificationRoot, { recursive: true });

const workItemIds = Object.freeze([
...
const artifactRef = (artifactId, artifact) => ({
  ...
  uri: `memory://devrelay/v011/${encodeURIComponent(artifactId)}`,
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/v0.11-module-quality/repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/v0.11-module-quality/execution-v2/${workItemId}`;
  ...
  const testRun = spawnSync(process.execPath, ["--test", "test/v0.11-frontier-three.test.mjs"], {
    cwd: root,
    encoding: null,
    windowsHide: true,
  });
  ...
  const receiptRecord = api.recordExecutionReceipt({
    effect: {
      ...
      argv: ["--test", "test/v0.11-frontier-three.test.mjs"],
      cwd: ".",
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-a97f1f5-v011-module-quality",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: workItemId === "WI-MQ-GDSCRIPT-DISCOVERY" ? `The independent focused verifier passed all 10 V0.11 third-frontier tests for the exact ${workItemId} candidate bytes; this is contributing item evidence and does not by itself satisfy the shared Godot compatibility or release criteria.` : `The independent focused verifier passed all 10 V0.11 third-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "V011ModuleQualityGeneration2ThirdFrontierVerificationSummary", results };
summary.summaryDigest = api.canonicalJsonDigest(summary);
writeFileSync(resolve(verificationRoot, "verification-summary-frontier-three.json"), `${api.canonicalJson(summary)}\n`, "utf8");
