...
const executionRoot = resolve(root, "dogfood/v0.11-module-quality/execution-v4");
const verificationRoot = resolve(root, "dogfood/v0.11-module-quality/verification-v4");
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/v0.11-module-quality/execution-v4/${workItemId}`;
  ...
  const testRun = spawnSync(process.execPath, ["--test", "test/v0.11-windows-e2e.test.mjs"], {
    cwd: root,
    encoding: null,
    windowsHide: true,
  });
  ...
  const receiptRecord = api.recordExecutionReceipt({
    effect: {
      ...
      argv: ["--test", "test/v0.11-windows-e2e.test.mjs"],
      cwd: ".",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent verifier passed all 3 Windows end-to-end evidence tests for the exact ${workItemId} candidate bytes, including the installed-package lifecycle, live providers, trace graph, lifecycle report, and two-phase Git seal.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "V011ModuleQualityGeneration4FourthFrontierVerificationSummary", results };
summary.summaryDigest = api.canonicalJsonDigest(summary);
writeFileSync(resolve(verificationRoot, "verification-summary-frontier-four.json"), `${api.canonicalJson(summary)}\n`, "utf8");
