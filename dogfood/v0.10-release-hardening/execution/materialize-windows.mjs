...
const completions = canonicalLoaded(
  completionsValue,
  "ICFS-REL-WINDOWS-VERIFY-FRONTIER",
...
const jobs = [{
  workItemId: "WI-REL-WINDOWS-VERIFY",
  attemptId: "ATT-REL-WINDOWS-VERIFY-001",
  files: windowsFiles,
  evidence: {
    artifactId: "EV-REL-WINDOWS-MATRIX-001",
    command: "GitHub Actions run 31673951438: release:check on windows-latest with Node 22 and Node 24",
    tests: 2,
  },
  permissions: [
    { kind: "filesystem.write", scope: { values: [".github/workflows/verify.yml", "test/package-export-verification.test.mjs"] } },
    { kind: "process.spawn", scope: { values: ["node", "npm"] } },
  ],
}];

const graphHeadPath =
  "dogfood/v0.10-release-hardening/traceability/package-traceability-graph-snapshot.json";
...
writeJson("windows-execution-summary.json", summary);
