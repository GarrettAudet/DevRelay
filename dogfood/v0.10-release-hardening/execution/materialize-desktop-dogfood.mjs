...
const desktopDogfoodFiles = collectFiles("dogfood/v0.10-release-hardening/desktop-dogfood");
const jobs = [{
  workItemId: "WI-REL-DESKTOP-DOGFOOD",
  attemptId: "ATT-REL-DESKTOP-DOGFOOD-001",
  files: desktopDogfoodFiles,
  evidence: {
    artifactId: "EV-REL-DESKTOP-DOGFOOD-001",
    command: "Installed candidate tarball through ChatGPT Desktop on Windows; built, tested, integrated, replayed, and accepted a separate authentication project",
    tests: 4,
  },
  permissions: [
    { kind: "filesystem.write", scope: { values: ["dogfood/v0.10-release-hardening/desktop-dogfood/**"] } },
    { kind: "process.spawn", scope: { values: ["git", "node", "npm"] } },
  ],
}];

const graphHeadPath =
  "dogfood/v0.10-release-hardening/traceability/protected-main-traceability-graph-snapshot.json";
...
writeJson("desktop-dogfood-execution-summary.json", summary);
