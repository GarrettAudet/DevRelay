...
const jobs = [{
  ...
  permissions: [
    { kind: "filesystem.write", scope: { values: ["package.json", "package-lock.json", "scripts/**", "test/**"] } },
    { kind: "process.spawn", scope: { values: ["node", "npm"] } },
  ],
}];

const graphHeadPath =
  "dogfood/v0.10-release-hardening/traceability/traceability-graph-snapshot.json";
...
writeJson("package-execution-summary.json", summary);
