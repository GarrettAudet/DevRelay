...
const runtimeArchitectureIds = [
  ...
  "EL-WE-TRACEABILITY-CONTRIBUTOR",
];
...
const workItems = [
  ...
  workItem({
    ...
    included: ["Validate exact baselines, readiness, assignment, binding, policy, and repository identity before executor entry.", "Checkpoint exact raw executor bytes, replay with zero calls, and assemble only ExecutionAttempt, ChangeSetDraft, and ExecutionEvidenceBundle candidate outputs.", "Project only trusted candidate attempt and produces relationships through the WorkExecution traceability contributor.", "Export the runtime and cover drift, permission, substitution, failure, interruption, retry, traceability, and replay paths."],
    ...
    deliverables: [["DEL-REL-WORK-EXECUTION-RUNTIME", "Released WorkExecution input guard, frontier validator, binding validator, checkpoint controller, result assembler, traceability contributor, and orchestration API.", "RuntimeLibrary"]],
    workType: "code-change",
    ...
    verificationMethod: "Run focused positive and negative runtime tests proving exact readiness, binding, checkpoint, replay, output, retry, trusted candidate traceability, and no-downstream-authority behavior.",
    successCriteria: "A host can invoke one exact ready work item through the installed provider-neutral runtime and receive only validated candidate execution artifacts plus trusted forward candidate traceability with byte-identical zero-call replay.",
