...
const fullRef = (
  ...
  schema,
  mediaType,
  uri: `memory://devrelay/v0.10/${encodeURIComponent(artifactId)}/${digest.slice(7)}.json`,
});
...
function restoreGraph() {
  const graphPath =
    "dogfood/v0.10-release-hardening/traceability/acceptance-traceability-graph-snapshot.json";
...
const releaseEvidence = loadedRaw(
...
const completions = workItemIds.map((workItemId) =>
  readJson(
    `dogfood/v0.10-release-hardening/integration/${workItemId}/integrated-completion-fact.json`,
  ),
);
const records = workItemIds.map((workItemId) =>
  readJson(
    `dogfood/v0.10-release-hardening/integration/${workItemId}/integrated-change-record.json`,
  ),
);
assert.ok(completions.every(({ status }) => status === "verified-and-integrated"));

const completionFactSet = seal(
  {
    apiVersion: API,
    kind: "IntegratedCompletionFactSet",
    factSetId: "ICFS-DEVRELAY-V010-RELEASE-001",
    facts: completions
      .map((value) => ref(value.completionId, value.completionDigest))
...
const systemPolicy = seal(
  {
    ...
    policyId: "SV-POLICY-DEVRELAY-V010-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const integratedSubject = seal(
  {
    ...
    subjectId: `DEVRELAY-V010-DESKTOP-WINDOWS-${releaseEvidence.value.evidenceSetDigest.slice(7, 19).toUpperCase()}`,
    ...
    verificationEnvironment: {
      ...
      rationale:
        "The supported product is a source/library workflow run from ChatGPT Desktop on Windows; no deployed runtime environment is claimed.",
    },
...
const reviewObligations = obligations.obligations
  ...
  .map(({ obligationId }) => obligationId);
const invocations = [
  invocationFor("SV-DEVRELAY-V010-TEST-001", TEST_SYSTEM_VERIFIER, testObligations),
  invocationFor("SV-DEVRELAY-V010-REVIEW-001", REVIEW_SYSTEM_VERIFIER, reviewObligations),
];
...
const verifier = async (invocation) => {
  ...
  if (invocation.verifier.id === TEST_SYSTEM_VERIFIER.id) {
    ...
    return adaptSystemTestResult({
      invocation,
      nativeBytes: bytes,
      nativeArtifact: ref("NATIVE-DEVRELAY-V010-TEST-001", sha256Digest(bytes)),
    });
  }
  const native = {
  ...
  return adaptSystemReviewResult({
    invocation,
    nativeBytes: bytes,
    nativeArtifact: ref("NATIVE-DEVRELAY-V010-REVIEW-001", sha256Digest(bytes)),
  });
};
...
const systemInvocation = {
  invocationId: "SYSTEM-VERIFICATION-DEVRELAY-V010-001",
...
const businessPolicy = seal(
  {
    ...
    policyId: "BA-POLICY-DEVRELAY-V010-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const businessEvidence = seal(
  {
    ...
    evidenceSetId: "BA-EVIDENCE-DEVRELAY-V010-DESKTOP-WINDOWS-001",
    requirementsBaseline: ref(requirements.ref.artifactId, requirements.ref.digest),
    projectOverviewBaseline: ref(overview.ref.artifactId, overview.ref.digest),
...
const requirementsInvocation = {
  invocationId: "REQUIREMENTS-BASELINE-SCOPE-RECONCILIATION-V010-001",
...
const businessInput = {
  checkpointId: "BA-DEVRELAY-V010-DESKTOP-WINDOWS-001",
...
const ownerApproval = seal(
  {
    ...
    approvalId: "BA-APPROVAL-DEVRELAY-V010-DESKTOP-WINDOWS-001",
...
let ownerCalls = 0;
const gateInput = {
  checkpointId: "BA-GATE-DEVRELAY-V010-DESKTOP-WINDOWS-001",
...
const businessInvocation = {
  invocationId: "BUSINESS-ACCEPTANCE-GATE-DEVRELAY-V010-001",
  module: {
...
const approvalRequest = seal(
  {
    ...
    requestId: "BA-APPROVAL-REQUEST-DEVRELAY-V010-DESKTOP-WINDOWS-001",
    candidate: ref(business.candidate.candidateId, business.candidate.candidateDigest),
    candidateRawDigest: sha256Digest(candidateBytes),
    technicalCoverage: ref(
      business.technicalCoverage.coverageId,
...
const finalProof = seal(
  {
    apiVersion: API,
    kind: "DevRelayV010SourceReleaseAcceptanceProof",
    proofId: "DEVRELAY-V010-SOURCE-RELEASE-ACCEPTANCE-001",
    sourceBaseCommit,
...
const summary = {
  apiVersion: API,
  kind: "DevRelayV010FinalAcceptanceSummary",
  releaseEvidence: releaseEvidenceRef,
...
writeText(
  "FINAL_ACCEPTANCE.md",
  `# DevRelay v0.10 final acceptance\n\n` +
    ...
    `This acceptance is limited to GitHub source plus the deterministic installable tarball operated through ChatGPT Desktop on Windows. It does not claim public npm publication, a one-click Desktop plug-in, or a hosted backend.\n`,
);
