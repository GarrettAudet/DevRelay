...
export async function promoteReleaseHardeningArchitecture(context) {
  ...
  const ownerApprovalBytes = read(
    "dogfood/v0.10-release-hardening/architecture-design/architecture-gate-owner-approval.json",
  );
  ...
  const gateReviewRef = fileRef(
    "architecture-gate-review-v0.10-release-hardening-v1",
  ...
  const executionProofRef = fileRef(
    "architecture-dogfood-execution-proof-v0.10-release-hardening-v1",
  ...
  const promotedBaseline = {
    ...
    baselineId: "architecture-baseline-devrelay-v1-v0.10-release-hardening-001",
  ...
  const promotedProjectState = {
    ...
    stateId: "project-architecture-state-devrelay-v0.10-release-hardening-approved-v1",
    state: "baselined",
  ...
  const expectedRequiredInterfaces = [
    ...new Set([
      ...architectureBaseline.sections.interfaceIntent.content.interfaces
        .filter(({ contractGeneration }) => contractGeneration?.required === true)
        .map(({ id }) => id),
      "IF-REL-EXPORT-INVENTORY",
      "IF-REL-CANDIDATE-EVIDENCE",
      "IF-REL-GITHUB-PROMOTION",
    ]),
  ].sort();
  if (
  ...
  const promotionId = "architecture-promotion-v0.10-release-hardening-v1";
  ...
  fs.writeFileSync(
    path.join(projectDir, "architecture-gate-owner-approval-v0.10-release-hardening-v1.json"),
    ownerApprovalBytes,
  );
