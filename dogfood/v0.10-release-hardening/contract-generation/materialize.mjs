...
const RUN_ID = "v0.10-release-hardening-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-v0.10-release-hardening-v1";
const GRAPH_ID = "devrelay/v0.10-release-hardening-contract-change";
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== currentContractBaseline.value.contracts.length + 3) {
  throw new Error(`expected prior contract count plus three required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-V010-RELEASE-HARDENING-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-V010-RELEASE-HARDENING-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-v0.10-release-hardening-v1",
);
...
const checkpointFile = await exactJsonFile(
  ...
  "application/json",
  "CG-CHECKPOINT-V010-RELEASE-HARDENING-001",
);
...
const moduleResult = {
  ...
  evidence: [
    ...
    },
    {
      ...
      summary: "Core computed a deterministic three-add contract diff against the exact current ContractBaseline.",
    },
  ],
  diagnostics: [],
};
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-V010-RELEASE-HARDENING-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-V010-RELEASE-HARDENING-CONTRACT-CHANGE-001",
  ...
  "application/json",
  "CGR-V010-RELEASE-HARDENING-001",
);
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-V010-RELEASE-HARDENING-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
  ...
  "application/json",
  "CGC-V010-RELEASE-HARDENING-001",
);
await exactJsonFile(
  ...
  "application/json",
  "CG-RUNTIME-PROOF-V010-RELEASE-HARDENING-001",
);
