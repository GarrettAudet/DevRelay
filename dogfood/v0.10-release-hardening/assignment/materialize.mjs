...
const agentCard = {
  protocolVersion: "1.0",
  name: "DevRelay OSS Release Governance Specialist",
  description: "Produces bounded OSS release governance and technical documentation.",
  version: "1.0.0",
  supportedInterfaces: [{ url: "https://invalid.example/a2a", protocolBinding: "JSONRPC", protocolVersion: "1.0" }],
  capabilities: {},
  defaultInputModes: ["text/plain"],
  defaultOutputModes: ["text/markdown"],
  skills: [{ id: "oss-release-governance", name: "OSS release governance", description: "Document and review approved OSS release governance.", tags: ["documentation", "release", "governance"] }],
};
const a2aSnapshot = {
  source: {
    version: "1.0",
    digest: canonicalJsonDigest(agentCard),
    uri: "fixture://devrelay/a2a/v0.10-release-hardening-documentation-specialist-card.json",
  },
  agentCard,
  toolIds: ["TOOL-NPM"],
...
const a2aProfile = normalizeA2AAgentCard({
  ...
  mappings: [{ skillId: "oss-release-governance", capabilityIds: ["CAP-RELEASE-ENGINEERING", "CAP-TECHNICAL-WRITING"] }],
});
const specialistCatalog = {
  apiVersion: "devrelay.dev/v1alpha1",
  kind: "SpecialistCatalog",
  catalogId: "SC-REL-DOGFOOD-001",
  version: "1.0.0",
  profiles: [
    {
      ...
      toolIds: ["TOOL-GITHUB", "TOOL-NODE", "TOOL-NPM"],
...
const documentationWorkItem = workBreakdown.workItems.find(({ id }) => id === "WI-REL-GOVERNANCE");
const assignmentPolicy = {
  apiVersion: "devrelay.dev/v1alpha1",
  kind: "AssignmentPolicy",
  policyId: "AP-REL-DOGFOOD-001",
  version: "1.0.0",
  workItemRules: [{ workItemId: "WI-REL-PROTECTED-MAIN", requiredToolIds: ["TOOL-GITHUB"], requiredGrantIds: ["GRANT-GITHUB-ADMIN"] }],
  profilePriorities: [
    { profileId: a2aProfile.id, priority: 100 },
    { profileId: "PROFILE-DEVRELAY-ENGINEERING", priority: 10 },
  ],
  rationale: `Prefer the A2A OSS governance profile only when it is eligible for ${documentationWorkItem.id}; use the provider-neutral engineering profile for all implementation and verification work.`,
};
...
const invocation = {
  executionId: "SA-REL-DOGFOOD-002",
  workBreakdown,
  workDependency,
  capabilityCatalog,
  specialistCatalog,
