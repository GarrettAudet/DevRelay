...
const RUN_ID = "work-item-verification-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-work-item-verification-v1";
const GRAPH_ID = "devrelay/work-item-verification-contract-change";
...
const architecture = await loadedFile(
  "dogfood/work-item-verification/architecture-design/architecture-baseline.json",
...
const projectArchitectureState = JSON.parse(
  await readFile(new URL("dogfood/work-item-verification/architecture-design/replay-v2/project-architecture-state.json", ROOT), "utf8"),
);
...
const currentContractBaseline = await loadedFile(
  "project/history/contracts/CB-DEVRELAY-003/contract-baseline.json",
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== 34) {
  throw new Error(`expected 34 required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-WORK-ITEM-VERIFICATION-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-WORK-ITEM-VERIFICATION-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-work-item-verification-v1",
);
...
const checkpointFile = await exactJsonFile(
  ...
  "application/json",
  "CG-CHECKPOINT-WORK-ITEM-VERIFICATION-001",
);
  ...
  evidence: [
    {
      ...
      summary: "Core independently validated all 34 native JSON Schema draft 2020-12 artifacts.",
    },
    {
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-WORK-ITEM-VERIFICATION-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-WORK-ITEM-VERIFICATION-CONTRACT-CHANGE-001",
  ...
  "application/json",
  "CGR-WORK-ITEM-VERIFICATION-001",
);
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-WORK-ITEM-VERIFICATION-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
  ...
  "application/json",
  "CGC-WORK-ITEM-VERIFICATION-001",
);
await exactJsonFile(
  ...
  "application/json",
  "CG-RUNTIME-PROOF-WORK-ITEM-VERIFICATION-001",
);
