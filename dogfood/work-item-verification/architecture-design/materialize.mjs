...
import { buildWorkItemVerificationArchitecture } from "./work-item-verification-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/work-item-verification/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/work-item-verification/architecture-design";
...
const requirementsSourcePath = "project/history/1.5.0/requirements-baseline.json";
const projectOverviewSourcePath = "project/history/1.5.0/project-overview-baseline.json";
const requirements = load(requirementsSourcePath);
const projectOverview = load(projectOverviewSourcePath);
const projectContext = load("dogfood/work-item-verification/project-context.json");
const repositorySnapshot = load("dogfood/work-item-verification/repository-snapshot.json");
// The WIV change is always designed against the exact approved WorkExecution
// architecture, never whichever project head a different dogfood test left active.
const architectureBaselineSourcePath =
  "project/history/architecture/architecture-baseline-devrelay-v1-work-execution-001/architecture-baseline.json";
const architectureBaseline = load(architectureBaselineSourcePath);

const requirementsRef = artifactRef(
  requirements.baselineId,
  "https://devrelay.dev/artifacts/requirements-baseline/v1",
  "application/vnd.devrelay.requirements-baseline+json",
  sha256(read(requirementsSourcePath)),
...
const projectOverviewRef = artifactRef(
  ...
  sha256(read(projectOverviewSourcePath)),
  "file:///C:/repos/DevRelay/project/project-overview-baseline.json",
);
const projectContextRef = artifactRef(
  "project-context-devrelay-work-item-verification-v1",
  "https://devrelay.dev/artifacts/project-context/v1",
  "application/vnd.devrelay.project-context+json",
  sha256(read("dogfood/work-item-verification/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/work-item-verification/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  ...
  sha256(read("dogfood/work-item-verification/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/work-item-verification/repository-snapshot.json",
);
const architectureBaselineRef = artifactRef(
  ...
  sha256(read(architectureBaselineSourcePath)),
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-work-item-verification-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-work-item-verification-architecture-v1",
...
const {
  ...
  risks,
  uniq,
} = buildWorkItemVerificationArchitecture({
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay WorkItemVerification")} ${dslQuote("Evidence-bound verification and Gate target architecture")} {`,
    "  !identifiers flat",
...
const designRef = fileRef(
  "openspec-design-work-item-verification-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-work-item-verification-v1",
...
for (const spec of decisionSpecs) {
  decisions.push({
    ...
    consideredOptions: [
      { id: spec.chosen, title: "Adopt the bounded canonical design", pros: ["Preserves executor replaceability and Core authority boundaries"], cons: ["Requires explicit conformance evidence"] },
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/work-item-verification/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-WIV-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
    { section: "technicalDesign", entityIds: [technicalDesign.technicalDesignId], jsonPointers: ["/sections/technicalDesign"] },
    { section: "interfaceIntent", entityIds: interfaces.filter(({ id }) => id.startsWith("IF-WIV-")).map(({ id }) => id), jsonPointers: ["/sections/interfaceIntent"] },
    { section: "architectureConstraints", entityIds: constraints.filter(({ id }) => id.startsWith("CON-WIV-")).map(({ id }) => id), jsonPointers: ["/sections/architectureConstraints"] },
  ]),
  nativeEntry("NA-WIV-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-WIV-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-WIV-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-WIV-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-WIV-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-WIV-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-WIV-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by bounded work item verification.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the bounded work-item-verification target architecture.`,
      compatibilityImpact: "backward-compatible",
      sourceRequirementIds: uniq(
        operation === "remove"
          ? [CG.verify, CG.deterministic]
          : kind === "view"
            ? [CG.verify, CG.deterministic]
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-WIV-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the bounded work-item-verification architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-work-item-verification-v1",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-work-item-verification-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-WIV-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-work-item-verification-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-WIV-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/work-item-verification/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/work-item-verification/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/work-item-verification",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-work-item-verification-v1",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-work-item-verification-v1",
  runId: "work-item-verification-architecture-run-v1",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "work-item-verification", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/work-item-verification/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/work-item-verification/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/work-item-verification/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(requirementsRef, read(requirementsSourcePath));
register(projectOverviewRef, read(projectOverviewSourcePath));
register(projectOverview.renderedDocument.artifact, read("project/history/1.5.0/ProjectOverview.md"));
register(projectContextRef, read("dogfood/work-item-verification/project-context.json"));
register(repositorySnapshotRef, read("dogfood/work-item-verification/repository-snapshot.json"));
register(architectureBaselineRef, read(architectureBaselineSourcePath));
register(architectureBaselineProjectContextRef, read("dogfood/work-execution/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/work-execution/repository-snapshot.json"));
...
if (process.argv.includes("--promote")) {
  const { promoteWorkItemVerificationArchitecture } = await import("./promote.mjs");
  const promotion = await promoteWorkItemVerificationArchitecture({
    root,
    dir,
    read,
    fileRef,
