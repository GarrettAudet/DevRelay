...
export async function promoteWorkItemVerificationArchitecture(context) {
  const {
    root,
    dir,
    read,
  ...
  const ownerApprovalBytes = read(
    "dogfood/work-item-verification/architecture-design/architecture-gate-owner-approval.json",
  );
  ...
  const gateReviewRef = fileRef(
    "architecture-gate-review-work-item-verification-v1",
  ...
  const executionProofRef = fileRef(
    "architecture-dogfood-execution-proof-work-item-verification-v1",
  ...
  const promotedBaseline = {
    ...
    baselineId: "architecture-baseline-devrelay-v1-work-item-verification-001",
  ...
  const promotedProjectState = {
    ...
    stateId: "project-architecture-state-devrelay-work-item-verification-approved-v1",
    state: "baselined",
  ...
  const expectedRequiredInterfaces = [
    ...
    "IF-WE-CANDIDATE-TRACEABILITY",
    "IF-WE-EXECUTION-BINDING",
    "IF-WE-EXECUTOR-INVOCATION",
    "IF-WE-READINESS-PROOF",
    "IF-WE-RESULT-ASSEMBLY",
  ...
  const promotionId = "architecture-promotion-work-item-verification-v1";
  ...
  const replayDir = path.join(dir, "replay-v2");
  ...
  const previousBaselineBytes =
    readIfPresent(path.join(root, historyBaselinePath)) ??
    read("project/architecture-baseline.json");
  ...
  if (!readIfPresent(historyStatePath)) {
    const currentStateBytes = readIfPresent(
      path.join(replayDir, "project-architecture-state.json"),
    );
  ...
  fs.writeFileSync(
    path.join(replayDir, "architecture-gate-owner-approval-work-item-verification-v1.json"),
    ownerApprovalBytes,
  );
  ...
  const promotionProof = {
    ...
    contractGenerationRequired: requiredContractInterfaces.length > 0,
    contractGenerationOperation: "generate-contract-change",
  ...
  fs.writeFileSync(
    path.join(replayDir, "architecture-promotion.commit.json"),
