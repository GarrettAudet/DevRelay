...
const REPOSITORY_REVISION = "4bda7fe707ba102bd22fe0001c83aa13ec03b0c5";
...
async function loadedFile(relativePath, schema, mediaType, logicalPath = relativePath) {
  const url = new URL(relativePath, ROOT);
  const logicalUrl = new URL(logicalPath, ROOT);
  ...
  return {
    ...
    ref: refFor({
      ...
      schema,
      mediaType,
      uri: repositoryArtifactUriFromUrl(ROOT, logicalUrl),
    }),
  };
}
...
const workBreakdown = await loadedFile(
  ...
  "dogfood/work-item-verification/work-breakdown/work-breakdown-baseline.json",
);
...
const contextSliceSetValue = {
  ...
  sliceSetId: "CTXS-WIV-DOGFOOD-001",
  slices: [
    createContextSlice({
      id: "CTX-WIV-REQUIREMENTS",
    ...
    createContextSlice({
      id: "CTX-WIV-REPOSITORY-REVISION",
...
const sources = new Map(
  [requirements, architecture, repository, policyWasm].map((entry) => [
    entry.ref.digest,
    entry,
  ]),
);
...
const execution = await runtime.execute({
  executionId: "WDA-WIV-DOGFOOD-001",
  workBreakdown,
  projectOverview,
  contextSliceSet,
  policyBundle,
...
const gateReview = await exactJsonFile(
  "work-dependency-gate-review.json",
  {
    ...
    reviewId: "WDA-WIV-GATE-REVIEW-001",
...
const gateCandidate = await exactJsonFile(
  "gate-candidate.json",
  {
    ...
    gateCandidateId: "WDA-WIV-GATE-CANDIDATE-001",
    candidate: execution.candidateRef,
    workBreakdownBaseline: workBreakdown.ref,
