...
const verificationRoot = resolve(root, "dogfood/sim-001-simplification/verification-frontier-4");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-SIM-CLI": Object.freeze({ argv: ["--test", "test/operator-cli.test.mjs"], command: "node --test test/operator-cli.test.mjs", tests: 6 }),
  "WI-SIM-PACK-CONFORMANCE": Object.freeze({ argv: ["--test", "test/pack-conformance.test.mjs", "test/v0.11-frontier-two.test.mjs", "test/v0.11-frontier-three.test.mjs"], command: "node --test test/pack-conformance.test.mjs test/v0.11-frontier-two.test.mjs test/v0.11-frontier-three.test.mjs", tests: 20 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/sim-001-simplification/integration-frontier-3-retry/WI-SIM-API-COMPAT/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/sim-001-simplification/execution-frontier-4/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-11ae432",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} SIM-001 fourth-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Sim001FourthFrontierVerificationSummary", results };
