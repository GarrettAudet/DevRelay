...
const outRoot = resolve(root, "dogfood/sim-001-simplification/integration-frontier-2-retry");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/sim-001-integration";
const sourceRef = "refs/heads/codex/sim-001-source-candidate-frontier-2-retry";
const ids = Object.freeze(["WI-SIM-HOST-ISOLATION"]);
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/sim-001-simplification/execution-frontier-2/${workItemId}/change-set-draft.json`);
...
const graphSummary = { resultGraph: json("dogfood/sim-001-simplification/integration-frontier-2/WI-SIM-FACADE/traceability-merge-receipt.json").resultGraph };
let graphSnapshot = json("dogfood/sim-001-simplification/integration-frontier-2/WI-SIM-FACADE/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/sim-001-simplification/verification-frontier-2/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/sim-001-simplification/execution-frontier-2/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Sim001SecondFrontierIntegrationRetrySummary",
  targetRef,
  baseCommit,
