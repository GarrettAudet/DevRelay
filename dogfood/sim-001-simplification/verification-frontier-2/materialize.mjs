...
const verificationRoot = resolve(root, "dogfood/sim-001-simplification/verification-frontier-2");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-SIM-EVIDENCE-ASSETS": Object.freeze({ argv: ["--test", "test/release-evidence-assets.test.mjs"], command: "node --test test/release-evidence-assets.test.mjs", tests: 7 }),
  "WI-SIM-FACADE": Object.freeze({ argv: ["--test", "test/public-facade.test.mjs"], command: "node --test test/public-facade.test.mjs", tests: 8 }),
  "WI-SIM-HOST-ISOLATION": Object.freeze({ argv: ["--test", "test/local-host-isolation.test.mjs"], command: "node --test test/local-host-isolation.test.mjs", tests: 8 }),
});
...
const artifactRef = (artifactId, artifact) => ({
  ...
  uri: `memory://devrelay/sim001/${encodeURIComponent(artifactId)}`,
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/sim-001-simplification/integration-frontier-1/WI-SIM-PROFILES/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/sim-001-simplification/execution-frontier-2/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-b856570",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} SIM-001 first-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Sim001SecondFrontierVerificationSummary", results };
