...
const dogfoodRelative = "dogfood/sim-001-simplification/work-breakdown";
...
const repositoryBytes = read(
  "dogfood/sim-001-simplification/repository-snapshot.json",
);
...
const priorGraphBytes = read(
  "dogfood/v0.11-module-quality/work-breakdown/traceability-graph-snapshot.json",
);
const priorGraphReceipt = load(
  "dogfood/v0.11-module-quality/work-breakdown/module-execution-record.json",
);
const requirementsPromotionBytes = read(
  "dogfood/sim-001-simplification/requirements-gate-promotion-proof.json",
);
const architecturePromotionBytes = read(
  "dogfood/sim-001-simplification/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofBytes = read(
  "dogfood/v0.11-module-quality/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractPromotionBytes = read(
  "dogfood/sim-001-simplification/contract-generation/replay-v1/contract-gate-promotion.json",
);
...
const priorRequirements = load("project/history/1.9.0/requirements-baseline.json");
...
const targetArchitectureIds = architecture.sections.architectureModel.content.elements
  .map(({ id }) => id)
  .filter((id) => id.startsWith("EL-SIM-"))
  .sort();
assert.equal(targetAcceptanceIds.length, 15);
...
const targetContractIds = contractBaseline.contracts
  .filter(({ interfaceIntentId }) => interfaceIntentId.startsWith("IF-SIM-"))
  .map(({ id }) => id)
  .sort();
...
assert.equal(traceabilityRefs.length, 16);
...
const packageReviewRef = artifactRef(
  "approved-change-package-review-sim-001-v1",
  {
...
const projectState = {
  ...
  stateId: "PWBS-SIM-BASELINED-001",
  state: "baselined",
  requirementsBaseline: requirementsRef,
  projectOverviewBaseline: overviewRef,
  architectureBaseline: architectureRef,
...
const workItemSpecs = load("dogfood/sim-001-simplification/work-breakdown/work-items.json");
...
assert.equal(workItems.length, 12);
...
const priorAcceptanceBytes = read("dogfood/v0.11-module-quality/final-acceptance/26-business-acceptance-record.json");
const priorAcceptanceValue = JSON.parse(priorAcceptanceBytes);
const priorAcceptanceRef = artifactRef(priorAcceptanceValue.recordId, { schema: "https://devrelay.dev/artifacts/business-acceptance-record/v1", mediaType: "application/json" }, priorAcceptanceBytes, "file:///C:/repos/DevRelay/dogfood/v0.11-module-quality/final-acceptance/26-business-acceptance-record.json");
...
const coverageDispositions = [
  ...currentBaseline.coverageDispositions.map(({ scopeKind, scopeRef }) => {
    ...
    return {
      ...
      rationale:
        "The prior V0.11 module-quality work is present in the pinned repository snapshot and has an exact approved BusinessAcceptance record; it is historical completion evidence, not new SIM-001 work.",
...
assert.equal(coverageDispositions.length, 296);
...
const changes = [
  ...currentBaseline.workItems.map((item) => ({
    ...
    rationale:
      "The prior V0.11 module-quality deliverable is present in the pinned repository revision and has exact BusinessAcceptance evidence; retain its scope as already satisfied instead of carrying completed work into the SIM-001 plan.",
  })),
...
const resultFor = (invocationId) => ({
  ...
  evidence: [
    ...
    },
    {
      ...
      artifact: workBreakdownChangeSetRef,
      summary:
        "Sixteen prior accepted V0.11 items are retired and their approved scope dispositions are preserved as already satisfied with exact repository and BusinessAcceptance evidence.",
    },
  ],
  diagnostics: [],
});
...
const routeRef = artifactRef(
  "module-route-decision-sim-001-v1",
  ROUTE_CONTRACT,
  routeBytes,
...
const invocation = {
  ...
  invocationId: "work-breakdown-decompose-sim-001-v1",
  runId: "sim-001-simplification-dogfood-run-v1",
  ...
  config: {
    ...
    planningOutputRoot:
      "C:/repos/DevRelay/dogfood/sim-001-simplification/work-breakdown",
    toolName: "OpenSpec",
    toolVersion: "0.1.0-fixture",
    changeName: "sim-001-simplification",
...
const checkpointReplayRef = artifactRef(
  "work-breakdown-checkpoint-replay-sim-001-v1",
  {
...
const activeAdWorkItems = snapshot.nodes.filter(
  ({ kind, state, stableId }) =>
    kind === "work-item" && state === "active" && stableId.startsWith("WI-SIM-"),
);
const retiredCiWorkItems = snapshot.nodes.filter(
  ({ kind, state, stableId }) =>
    kind === "work-item" && state === "retired" && stableId.startsWith("WI-MQ-"),
);
...
const runtimeProofRef = artifactRef(
  "work-breakdown-runtime-proof-sim-001-v1",
  {
...
const gateRef = artifactRef(
  "work-breakdown-gate-sim-001-v1",
  {
...
const gateCandidate = {
  ...
  candidateId: "work-breakdown-gate-candidate-sim-001-v1",
...
assert.equal(
  ...
  "sha256:69b6cb9b6ad3cf958786c1d125068e852af8d872513c7afcc40127647acfc1ba",
);
...
const ownerApprovalSourceRef = artifactRef(
  "work-breakdown-gate-owner-approval-source-sim-001-v1",
  {
...
const ownerApproval = {
  ...
  approvalId: "work-breakdown-gate-owner-approval-sim-001-v1",
  decision: "approved",
...
const promotedProjectState = {
  ...clone(projectState),
  stateId: "PWBS-SIM-001-PROMOTED-001",
...
const promotionProof = {
  ...
  proofId: "work-breakdown-gate-promotion-sim-001-v1",
  status: "promoted",
...
const pending = {
  ...
  transactionId: "work-breakdown-promotion-sim-001-v1",
  status: "prepared",
  candidate: workBreakdownChangeSetRef,
  gateCandidate: gateCandidateRef,
  ownerApproval: ownerApprovalRef,
...
writeJson("work-breakdown-dogfood-proof.json", {
  ...
  limitations: [
    ...
    "WorkBreakdown did not assign, schedule, execute, build, verify, integrate, or claim completion of any SIM-001 item.",
  ],
});
