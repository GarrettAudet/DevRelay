...
const outRoot = resolve(root, "dogfood/sim-001-simplification/integration-frontier-3-retry");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/sim-001-integration";
const sourceRef = "refs/heads/codex/sim-001-source-candidate-frontier-3-retry";
const ids = Object.freeze(["WI-SIM-API-COMPAT"]);
...
const fullRef = (artifactId, digest, schema = "https://devrelay.dev/internal/v1", mediaType = "application/json") => ({ artifactId, schema, mediaType, digest, uri: `memory://sim001/${artifactId}` });
...
const sourceRoot = mkdtempSync(join(tmpdir(), "devrelay-sim001-source-"));
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/sim-001-simplification/execution-frontier-3/${workItemId}/change-set-draft.json`);
    ...
    git(sourceRepo, "commit", "-m", `SIM-001 ${workItemId}`);
...
const graphSummary = { resultGraph: json("dogfood/sim-001-simplification/integration-frontier-3/WI-SIM-HOST-EXECUTION/traceability-merge-receipt.json").resultGraph };
let graphSnapshot = json("dogfood/sim-001-simplification/integration-frontier-3/WI-SIM-HOST-EXECUTION/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/sim-001-simplification/verification-frontier-3/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/sim-001-simplification/execution-frontier-3/${workItemId}/change-set-draft.json`);
  ...
  const targetReference = ref(`REPO-SIM001-INTEGRATION-${currentCommit.slice(0, 12).toUpperCase()}`, api.canonicalJsonDigest(targetSnapshot));
  const integrationPolicy = { kind: "IntegrationPolicy", version: "1.0.0", mode: "verified-fast-forward", targetRef };
  const integrationPolicyBytes = Buffer.from(`DevRelay SIM-001 verified fast-forward policy for ${targetRef}\n`, "utf8");
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-SIM001-${workItemId}`,
  });
...
const summary = {
  apiVersion: API,
  kind: "Sim001ThirdFrontierIntegrationRetrySummary",
  targetRef,
  baseCommit,
