...
const verificationRoot = resolve(root, "dogfood/sim-001-simplification/verification-frontier-5");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-SIM-REGRESSION-GATES": Object.freeze({ argv: ["scripts/verify.mjs"], command: "npm.cmd run verify", tests: 975 }),
  "WI-SIM-VERSION-DOCS": Object.freeze({ argv: ["--test", "test/sim-version-docs.test.mjs", "test/api-tiers.test.mjs", "test/operator-cli.test.mjs"], command: "node --test test/sim-version-docs.test.mjs test/api-tiers.test.mjs test/operator-cli.test.mjs", tests: 13 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/sim-001-simplification/integration-frontier-4/WI-SIM-PACK-CONFORMANCE/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/sim-001-simplification/execution-frontier-5/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} SIM-001 fifth-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Sim001FifthFrontierVerificationSummary", results };
