...
const verificationRoot = resolve(root, "dogfood/sim-001-simplification/verification-frontier-6");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-SIM-WINDOWS-E2E-RELEASE": Object.freeze({ argv: ["--test", "test/sim-windows-desktop-e2e.test.mjs"], command: "node --test test/sim-windows-desktop-e2e.test.mjs", tests: 3 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/sim-001-simplification/integration-frontier-5/WI-SIM-VERSION-DOCS/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/sim-001-simplification/execution-frontier-6/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} SIM-001 sixth-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Sim001SixthFrontierVerificationSummary", results };
