...
const outRoot = resolve(root, "dogfood/sim-001-simplification/integration-frontier-4");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/sim-001-integration";
const sourceRef = "refs/heads/codex/sim-001-source-candidate-frontier-4";
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/sim-001-simplification/execution-frontier-4/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/sim-001-simplification/execution-frontier-4/execution-summary.json");
let graphSnapshot = json("dogfood/sim-001-simplification/execution-frontier-4/WI-SIM-PACK-CONFORMANCE/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/sim-001-simplification/verification-frontier-4/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/sim-001-simplification/execution-frontier-4/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Sim001FourthFrontierIntegrationSummary",
  targetRef,
  baseCommit,
