...
const verificationRoot = resolve(root, "dogfood/sim-001-simplification/verification-frontier-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-SIM-HOST-STORAGE": Object.freeze({
    argv: ["--test", "test/local-host-storage.test.mjs"],
    command: "node --test test/local-host-storage.test.mjs",
    tests: 8,
  }),
  "WI-SIM-PROFILES": Object.freeze({
    argv: ["--test", "test/workflow-profiles.test.mjs"],
    command: "node --test test/workflow-profiles.test.mjs",
    tests: 6,
  }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/sim-001-simplification/repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/sim-001-simplification/execution-frontier-1/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-3374e75",
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Sim001FirstFrontierVerificationSummary", results };
