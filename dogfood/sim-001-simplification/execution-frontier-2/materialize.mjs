...
const completionsValue = readJson(
  "dogfood/sim-001-simplification/frontier-2/integrated-completion-fact-set.json",
);
const completions = canonicalLoaded(
  completionsValue,
  "ICFS-SIM-001-FRONTIER-2",
...
const jobs = [
  {
    workItemId: "WI-SIM-EVIDENCE-ASSETS",
    attemptId: "ATT-SIM-EVIDENCE-ASSETS-001",
    files: ["src/release-evidence-assets.mjs", "test/release-evidence-assets.test.mjs"],
    command: "node --test test/release-evidence-assets.test.mjs",
    tests: 7,
  },
  {
    workItemId: "WI-SIM-FACADE",
    attemptId: "ATT-SIM-FACADE-001",
    files: ["src/public-facade.mjs", "src/index.mjs", "test/public-facade.test.mjs"],
    command: "node --test test/public-facade.test.mjs",
    tests: 8,
  },
  {
    workItemId: "WI-SIM-HOST-ISOLATION",
    attemptId: "ATT-SIM-HOST-ISOLATION-001",
    files: ["src/local-host-isolation.mjs", "test/local-host-isolation.test.mjs"],
    command: "node --test test/local-host-isolation.test.mjs",
    tests: 8,
  },
];
const permissions = [
...
const graphHeadPath =
  "dogfood/sim-001-simplification/integration-frontier-1/WI-SIM-PROFILES/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "Sim001FrontierTwoExecutionSummary",
