...
import { buildSimplificationArchitecture } from "./simplification-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/sim-001-simplification/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/sim-001-simplification/architecture-design";
...
const projectContext = load("dogfood/sim-001-simplification/project-context.json");
const repositorySnapshot = load("dogfood/sim-001-simplification/repository-snapshot.json");
const architectureBaseline = load(
  "project/architecture-baseline.json",
);
const ownerDecisionPath = "docs/specs/sim-001-simplification/architecture-decisions-wave-1-response.json";
...
const projectContextRef = artifactRef(
  ...
  sha256(read("dogfood/sim-001-simplification/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/sim-001-simplification/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  "repository-snapshot-devrelay-3374e75-sim-001-simplification",
  "https://devrelay.dev/artifacts/repository-snapshot/v1",
  "application/vnd.devrelay.repository-snapshot+json",
  sha256(read("dogfood/sim-001-simplification/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/sim-001-simplification/repository-snapshot.json",
);
...
const ownerDecisionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/docs/specs/sim-001-simplification/architecture-decisions-wave-1-response.json",
);
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-sim-001-simplification-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-sim-001-simplification-architecture-v1",
...
const discoveryDecision = {
  ...
  decisionId: "architecture-discovery-bypass-sim-001-simplification-v1",
...
const {
  ...
  risks,
  uniq,
} = buildSimplificationArchitecture({
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay SIM-001 Module Quality")} ${dslQuote("Live provider, evidence, query, Desktop, and optional Godot architecture")} {`,
    "  !identifiers flat",
...
writeText(
  ...
  `# DevRelay architecture\n\nThis software-system-scoped workspace materializes the approved DevRelay architecture baseline and the SIM-001 simplification change. The canonical DevRelay artifact remains authoritative; Structurizr validates hierarchy, relationships, views, documentation, and decisions through a live provider execution.\n`,
);
...
const designRef = fileRef(
  "openspec-design-sim-001-simplification-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-sim-001-simplification-v2",
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/sim-001-simplification/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-SIM-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
    { section: "technicalDesign", entityIds: [technicalDesign.technicalDesignId], jsonPointers: ["/sections/technicalDesign"] },
    { section: "interfaceIntent", entityIds: interfaces.filter(({ id }) => id.startsWith("IF-SIM-")).map(({ id }) => id), jsonPointers: ["/sections/interfaceIntent"] },
    { section: "architectureConstraints", entityIds: constraints.filter(({ id }) => id.startsWith("CON-SIM-")).map(({ id }) => id), jsonPointers: ["/sections/architectureConstraints"] },
  ]),
  nativeEntry("NA-SIM-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-SIM-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-SIM-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-SIM-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-SIM-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-SIM-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-SIM-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by the approved SIM-001 simplification design.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the approved SIM-001 simplification target architecture.`,
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-SIM-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the approved SIM-001 simplification architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-sim-001-simplification-v2",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-sim-001-simplification-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-SIM-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-sim-001-simplification-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-SIM-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/sim-001-simplification/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/sim-001-simplification/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/sim-001-simplification",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-sim-001-simplification-v2",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-sim-001-simplification-v2",
  runId: "sim-001-simplification-architecture-run-v2",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "sim-001-simplification", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/sim-001-simplification/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/sim-001-simplification/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/sim-001-simplification/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(projectContextRef, read("dogfood/sim-001-simplification/project-context.json"));
register(repositorySnapshotRef, read("dogfood/sim-001-simplification/repository-snapshot.json"));
register(architectureBaselineRef, read("project/architecture-baseline.json"));
register(ownerDecisionRef, ownerDecisionBytes);
register(architectureBaselineProjectContextRef, read("dogfood/v0.11-module-quality/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/v0.11-module-quality/repository-snapshot.json"));
...
const traceabilityGraph = createTraceabilityGraphService({
  graphId: "traceability-graph-devrelay-sim-001-simplification",
  projectId: "devrelay",
