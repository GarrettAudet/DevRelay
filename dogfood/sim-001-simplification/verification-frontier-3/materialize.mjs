...
const verificationRoot = resolve(root, "dogfood/sim-001-simplification/verification-frontier-3");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-SIM-API-COMPAT": Object.freeze({ argv: ["--test", "test/api-tiers.test.mjs"], command: "node --test test/api-tiers.test.mjs", tests: 5 }),
  "WI-SIM-HOST-EXECUTION": Object.freeze({ argv: ["--test", "test/desktop-execution-coordinator.test.mjs"], command: "node --test test/desktop-execution-coordinator.test.mjs", tests: 6 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/sim-001-simplification/integration-frontier-2-retry/WI-SIM-HOST-ISOLATION/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/sim-001-simplification/execution-frontier-3/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-ca6d127",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} SIM-001 third-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Sim001ThirdFrontierVerificationSummary", results };
