...
const dogfoodRelative = "dogfood/pm-001-project-memory/work-breakdown";
...
const priorGraphBytes = read(
  "dogfood/pm-001-project-memory/contract-generation/replay-v1/approved-traceability-graph-snapshot.json",
);
const priorGraphReceipt = load(
  "project/contract-gate-promotion.json",
);
const requirementsPromotionBytes = read(
  "dogfood/pm-001-project-memory/requirements-gate-promotion-proof.json",
);
const architecturePromotionBytes = read(
  "dogfood/pm-001-project-memory/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofBytes = read(
  "dogfood/rm-001-roadmap-management/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractPromotionBytes = read(
  "dogfood/pm-001-project-memory/contract-generation/replay-v1/contract-gate-promotion.json",
);
const contractCandidateBytes = read(
  "dogfood/pm-001-project-memory/contract-generation/contract-change-set-draft.json",
);
const contractInvocation = load(
  "dogfood/pm-001-project-memory/contract-generation/contract-generation.invocation.json",
);
const contractResult = load(
  "dogfood/pm-001-project-memory/contract-generation/contract-generation.result.json",
);
const contractStateBytes = read(
  "dogfood/pm-001-project-memory/contract-generation/project-contract-state.json",
);
...
const requirementsPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/requirements-gate-promotion-proof.json",
);
const architecturePromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/rm-001-roadmap-management/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/contract-generation/replay-v1/contract-gate-promotion.json",
);

const priorRequirements = load("project/history/2.1.0/requirements-baseline.json");
...
const targetArchitectureIds = architecture.sections.architectureModel.content.elements
  .map(({ id }) => id)
  .filter((id) => id.startsWith("EL-PM-"))
  .sort();
assert.equal(targetAcceptanceIds.length, 19);
assert.equal(targetArchitectureIds.length, 11);
...
const targetContractIds = contractBaseline.contracts
  .filter(({ interfaceIntentId }) => interfaceIntentId.startsWith("IF-PM-"))
  .map(({ id }) => id)
  .sort();
...
assert.equal(authorizedContractIds.length, priorContractIds.length + 8);
...
const historicalPromotionUpdateDigests = new Set(priorGraph.appliedUpdates.slice(0, 2).map(({ digest }) => digest));
...
const packageReviewRef = artifactRef(
  "approved-change-package-review-pm-001-v1",
  {
...
const workItemSpecs = load("dogfood/pm-001-project-memory/work-breakdown/work-items.json");
...
const priorAcceptanceBytes = read("dogfood/rm-001-roadmap-management/release/final-acceptance/26-business-acceptance-record.json");
const priorAcceptanceValue = JSON.parse(priorAcceptanceBytes);
const priorAcceptanceRef = artifactRef(priorAcceptanceValue.recordId, { schema: "https://devrelay.dev/artifacts/business-acceptance-record/v1", mediaType: "application/json" }, priorAcceptanceBytes, "file:///C:/repos/DevRelay/dogfood/rm-001-roadmap-management/release/final-acceptance/26-business-acceptance-record.json");
...
const coverageDispositions = [
  ...currentBaseline.coverageDispositions.map(({ scopeKind, scopeRef }) => {
    ...
    return {
      ...
      rationale:
        "The prior RM-001 roadmap work is present in the pinned repository snapshot and has an exact approved BusinessAcceptance evidence; it is historical completion evidence, not new PM-001 work.",
...
const changes = [
  ...currentBaseline.workItems.map((item) => ({
    ...
    rationale:
      "The prior RM-001 roadmap deliverable is present in the pinned repository revision and has exact BusinessAcceptance evidence; retain its scope as already satisfied instead of carrying completed work into the PM-001 plan.",
  })),
...
const resultFor = (invocationId) => ({
  ...
  evidence: [
    ...
    },
    {
      ...
      artifact: workBreakdownChangeSetRef,
      summary:
        "Seven prior accepted RM-001 items are retired and their approved scope dispositions are preserved as already satisfied with exact repository and BusinessAcceptance evidence.",
    },
  ],
  diagnostics: [],
});
...
const routeRef = artifactRef(
  "module-route-decision-pm-001-v1",
  ROUTE_CONTRACT,
  routeBytes,
...
const invocation = {
  ...
  invocationId: "work-breakdown-decompose-pm-001-v1",
  runId: "pm-001-project-memory-dogfood-run-v1",
  ...
  config: {
    ...
    planningOutputRoot:
      "C:/repos/DevRelay/dogfood/pm-001-project-memory/work-breakdown",
    toolName: "OpenSpec",
    toolVersion: "0.1.0-fixture",
    changeName: "pm-001-project-memory",
...
const checkpointReplayRef = artifactRef(
  "work-breakdown-checkpoint-replay-pm-001-v1",
  {
...
const activePmWorkItems = snapshot.nodes.filter(
  ({ kind, state, stableId }) =>
    kind === "work-item" && state === "active" && stableId.startsWith("WI-PM-"),
);
const activeRmWorkItems = snapshot.nodes.filter(
  ({ kind, state, stableId }) =>
    kind === "work-item" && state === "active" && stableId.startsWith("WI-RM-"),
);
assert.equal(activePmWorkItems.length, workItems.length);
assert.equal(activeRmWorkItems.length, 0);
const objectivePath = queryTraceabilityGraph(snapshot, {
  start: {
    kind: "business-objective",
    stableId: "BO-DEV-PROJECT-MEMORY-001",
    authority: "approved",
...
const runtimeProofRef = artifactRef(
  "work-breakdown-runtime-proof-pm-001-v1",
  {
...
const gateCandidate = {
  ...
  candidateId: "work-breakdown-gate-candidate-pm-001-v1",
...
const ownerApprovalSourceRef = artifactRef(
  "work-breakdown-gate-owner-approval-source-pm-001-v1",
  {
...
const ownerApproval = {
  ...
  approvalId: "work-breakdown-gate-owner-approval-pm-001-v1",
  decision: "approved",
...
const promotedProjectState = {
  ...clone(projectState),
  stateId: "PWBS-PM-001-PROMOTED-001",
...
const promotionProof = {
  ...
  proofId: "work-breakdown-gate-promotion-pm-001-v1",
  status: "promoted",
...
const pending = {
  ...
  transactionId: "work-breakdown-promotion-pm-001-v1",
  status: "prepared",
  candidate: workBreakdownChangeSetRef,
  gateCandidate: gateCandidateRef,
  ownerApproval: ownerApprovalRef,
...
writeJson("work-breakdown-dogfood-proof.json", {
  ...
  limitations: [
    ...
    "WorkBreakdown did not assign, schedule, execute, build, verify, integrate, or claim completion of any PM-001 item.",
  ],
});
