...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-7-fix-2");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-WINDOWS-E2E": Object.freeze({ executable: process.execPath, argv: ["--test", "test/package-export-verification.test.mjs"], command: "node --test installed-package smoke role regression", tests: 4, passed: 4, skipped: 0 }),
});
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-7-fix-2/${workItemId}`;
  ...
  const candidateWorkspace = {
    ...
    version: "1.0.0",
    workspaceId: `CWS-${workItemId}-003`,
  ...
  const subject = api.bindWorkItemVerificationSubject({ subjectId: `SUB-${workItemId}-003`, workItemId, bindings });
  ...
  const binding = api.validateVerifierBindingSet({
    bindingId: `BIND-${workItemId}-TEST-003`,
    subject,
    obligationSet,
  ...
  const invocation = seal({
    ...
    verificationAttemptId: `VAT-${workItemId}-003`,
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent verifier passed all ${job.tests} focused installed-package smoke-role checks for the mandatory ProjectMemory context bindings for ${workItemId}.`,
  ...
  const controller = api.createWorkItemVerificationCheckpointController({
    verifier: async (exactInvocation) => {
      ...
      const raw = api.adaptTestVerifierResult({
        ...
        nativeBytes,
        nativeResult,
        nativeArtifact: ref(`NATIVE-${workItemId}-TEST-003`, api.sha256Digest(nativeBytes)),
      });
  ...
  const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({
    ...
    normalizedEvidenceId: `NVE-${workItemId}-003`,
  });
  const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
  const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId: `WIVC-${workItemId}-003`, policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation: evaluation });
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001SeventhFrontierFixTwoVerificationSummary", results };
