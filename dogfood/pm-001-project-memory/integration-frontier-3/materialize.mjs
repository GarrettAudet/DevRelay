...
const outRoot = resolve(root, "dogfood/pm-001-project-memory/integration-frontier-3");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/pm-001-integration-frontier-3-r2";
const sourceRef = "refs/heads/codex/pm-001-source-frontier-3-r2";
const ids = Object.freeze([
  "WI-PM-CONTEXT-BOOTSTRAP",
  "WI-PM-MEM0-ADAPTER",
  "WI-PM-TRACEABILITY",
]);
...
const baseCommit = json("dogfood/pm-001-project-memory/integration-frontier-2/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/pm-001-project-memory/execution-frontier-3/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/pm-001-project-memory/execution-frontier-3/execution-summary.json");
let graphSnapshot = json("dogfood/pm-001-project-memory/execution-frontier-3/WI-PM-TRACEABILITY/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/pm-001-project-memory/verification-frontier-3/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/pm-001-project-memory/execution-frontier-3/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Pm001ThirdFrontierIntegrationSummary",
  targetRef,
  baseCommit,
