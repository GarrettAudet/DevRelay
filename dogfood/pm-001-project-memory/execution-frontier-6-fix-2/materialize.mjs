...
const integrationSummary = readJson(
  "dogfood/pm-001-project-memory/integration-frontier-6/integration-summary.json",
);
const previousExecutionSummary = readJson(
  "dogfood/pm-001-project-memory/execution-frontier-6-fix-1/execution-summary.json",
);
...
const jobs = [
  {
    ...
    attemptId: "ATT-PM-REGRESSION-PERFORMANCE-003",
    files: ["scripts/release-tools.mjs", "test/package-export-verification.test.mjs"],
    command: "node --test test/package-export-verification.test.mjs test/public-facade.test.mjs",
    tests: 15,
  },
];
const permissions = [
...
const graphHeadPath =
  "dogfood/pm-001-project-memory/execution-frontier-6-fix-1/WI-PM-REGRESSION-PERFORMANCE/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "Pm001FrontierSixFixTwoExecutionSummary",
...
for (const job of jobs) {
  ...
  const moduleInvocation = {
    ...
    invocationId: `WE-PM-001-${item.id}-ATT-003`,
    module: {
      id: "work-execution",
      version: "0.1.0",
