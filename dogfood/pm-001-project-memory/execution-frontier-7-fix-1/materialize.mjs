...
const integrationSummary = readJson(
  "dogfood/pm-001-project-memory/integration-frontier-7/integration-summary.json",
);
const previousExecutionSummary = readJson(
  "dogfood/pm-001-project-memory/integration-frontier-7/integration-summary.json",
);
const repositoryRevision = execFileSync("git", ["rev-parse", "refs/heads/codex/pm-001-integration-frontier-7"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
...
const jobs = [
  {
    ...
    attemptId: "ATT-PM-WINDOWS-E2E-002",
    files: [
      "dogfood/pm-001-project-memory/system-verification-attempt-1/diagnosis.md",
      "dogfood/pm-001-project-memory/system-verification-attempt-1/failure-summary.json",
      "scripts/bootstrap-structurizr.mjs",
      "test/bootstrap-structurizr.test.mjs"
    ],
    command: "node --test test/project-baseline.test.mjs test/bootstrap-structurizr.test.mjs test/contract-generation-architecture-design-dogfood.test.mjs",
    tests: 6,
  },
];
const permissions = [
  { kind: "filesystem.write", scope: { values: ["dogfood/pm-001-project-memory/system-verification-attempt-1/**", "scripts/**", "test/**"] } },
  { kind: "process.spawn", scope: { values: ["node"] } },
];

const graphHeadPath =
  "dogfood/pm-001-project-memory/integration-frontier-7/WI-PM-WINDOWS-E2E/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "Pm001FrontierSevenFixOneExecutionSummary",
