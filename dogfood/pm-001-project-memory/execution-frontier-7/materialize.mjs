...
const integrationSummary = readJson(
  "dogfood/pm-001-project-memory/integration-frontier-6-fix-2/integration-summary.json",
);
const previousExecutionSummary = readJson(
  "dogfood/pm-001-project-memory/integration-frontier-6-fix-2/integration-summary.json",
);
const repositoryRevision = execFileSync("git", ["rev-parse", "refs/heads/codex/pm-001-release-candidate"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
...
const completionsValue = readJson(
  "dogfood/pm-001-project-memory/frontier-7/integrated-completion-fact-set.json",
);
const completions = canonicalLoaded(
  completionsValue,
  "ICFS-PM-001-FRONTIER-7",
...
const jobs = [
  {
    workItemId: "WI-PM-WINDOWS-E2E",
    attemptId: "ATT-PM-WINDOWS-E2E-001",
    files: [
      "contracts/roadmap-management-artifacts.schema.json",
      "dogfood/pm-001-project-memory/windows-e2e/installed-scenario.mjs",
      "dogfood/pm-001-project-memory/windows-e2e/mem0-toolchain-lock.json",
      "dogfood/pm-001-project-memory/windows-e2e/probe-live-mem0.mjs",
      "dogfood/pm-001-project-memory/windows-e2e/run-installed-package.mjs",
      "src/index.mjs",
      "src/public-facade.mjs",
      "src/session-bootstrap.mjs",
      "test/api-tiers.test.mjs",
      "test/public-facade.test.mjs",
      "test/roadmap-management-performance.test.mjs",
      "test/roadmap-management.test.mjs"
    ],
    command: "node --test test/api-tiers.test.mjs test/public-facade.test.mjs test/roadmap-management.test.mjs test/roadmap-management-performance.test.mjs test/project-memory-context.test.mjs && node dogfood/pm-001-project-memory/windows-e2e/installed-scenario.mjs",
    tests: 30,
  },
];
const permissions = [
  { kind: "filesystem.write", scope: { values: ["contracts/**", "dogfood/pm-001-project-memory/windows-e2e/**", "src/**", "test/**"] } },
  { kind: "process.spawn", scope: { values: ["node"] } },
];

const graphHeadPath =
  "dogfood/pm-001-project-memory/integration-frontier-6-fix-2/WI-PM-REGRESSION-PERFORMANCE/traceability-graph-snapshot.json";
const graphHeadValue = readJson(graphHeadPath);
const graphHeadBytes = Buffer.from(canonicalJson(graphHeadValue), "utf8");
const graphHeadRef = previousExecutionSummary.resultGraph;
...
const summary = {
  apiVersion: API,
  kind: "Pm001FrontierSevenExecutionSummary",
...
for (const job of jobs) {
  ...
  const moduleInvocation = {
    ...
    invocationId: `WE-PM-001-${item.id}-ATT-001`,
    module: {
      id: "work-execution",
      version: "0.1.0",
