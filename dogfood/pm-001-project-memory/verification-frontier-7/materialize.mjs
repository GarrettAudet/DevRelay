...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-7");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-WINDOWS-E2E": Object.freeze({ executable: process.execPath, argv: ["dogfood/pm-001-project-memory/windows-e2e/run-installed-package.mjs"], command: "node dogfood/pm-001-project-memory/windows-e2e/run-installed-package.mjs", tests: 1, passed: 1, skipped: 0, env: { ...process.env, DEVRELAY_MEM0_ENTRY: resolve(root, ".devrelay/tools/mem0/3.1.6/node_modules/mem0ai/dist/oss/index.mjs"), MEM0_TELEMETRY: "false" } }),
});
...
const baselines = {
  ...
  repositoryBase: (() => {
    const revision = execFileSync("git", ["rev-parse", "refs/heads/codex/pm-001-release-candidate"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-7/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent verifier passed the exact installed-package Windows Desktop ProjectMemory scenario with the pinned live Mem0 SDK, zero network attempts, atomic conclude, restart, and zero-call replay for ${workItemId}.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001SeventhFrontierVerificationSummary", results };
