...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-6-fix-2");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-REGRESSION-PERFORMANCE": Object.freeze({ executable: process.execPath, argv: ["--test", "test/package-export-verification.test.mjs", "test/public-facade.test.mjs"], command: "node --test package export and public facade suites", tests: 15, passed: 15, skipped: 0 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/pm-001-project-memory/integration-frontier-6/WI-PM-REGRESSION-PERFORMANCE/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-6-fix-2/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-6534adf",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent verifier passed the complete ${job.tests}-test PM-001 regression and performance gate for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001SixthFrontierFixTwoVerificationSummary", results };
