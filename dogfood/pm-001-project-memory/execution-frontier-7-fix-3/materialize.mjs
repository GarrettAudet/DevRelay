...
const integrationSummary = readJson(
  "dogfood/pm-001-project-memory/integration-frontier-7-fix-1/integration-summary.json",
);
const previousExecutionSummary = readJson(
  "dogfood/pm-001-project-memory/execution-frontier-7-fix-2/execution-summary.json",
);
const repositoryRevision = execFileSync("git", ["rev-parse", "refs/heads/codex/pm-001-integration-frontier-7-fix-1"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
...
const jobs = [
  {
    ...
    attemptId: "ATT-PM-WINDOWS-E2E-004",
    files: [
      "dogfood/pm-001-project-memory/system-verification-attempt-2/failure-summary.json",
      "scripts/release-tools.mjs",
      "test/package-export-verification.test.mjs",
      "dogfood/pm-001-project-memory/integration-frontier-7-fix-2/failure-summary.json",
      "src/change-integration-local-git-adapter.mjs",
      "test/change-integration-local-git-adapter.test.mjs"
    ],
    command: "node --test test/change-integration-local-git-adapter.test.mjs",
    tests: 14,
  },
];
const permissions = [
  { kind: "filesystem.write", scope: { values: ["dogfood/pm-001-project-memory/system-verification-attempt-2/**", "dogfood/pm-001-project-memory/integration-frontier-7-fix-2/**", "scripts/**", "src/**", "test/**"] } },
  { kind: "process.spawn", scope: { values: ["node"] } },
];

const graphHeadPath =
  "dogfood/pm-001-project-memory/execution-frontier-7-fix-2/WI-PM-WINDOWS-E2E/traceability-graph-snapshot.json";
...
const summary = {
  apiVersion: API,
  kind: "Pm001FrontierSevenFixThreeExecutionSummary",
