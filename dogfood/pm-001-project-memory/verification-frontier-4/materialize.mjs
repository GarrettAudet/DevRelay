...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-4");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-CONCLUDE-GATE": Object.freeze({ argv: ["--test", "test/project-memory-conclude.test.mjs"], command: "node --test test/project-memory-conclude.test.mjs", tests: 5 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/pm-001-project-memory/integration-frontier-3/WI-PM-TRACEABILITY/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-4/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} PM-001 conclude/Gate focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001FourthFrontierVerificationSummary", results };
