...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-3");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-CONTEXT-BOOTSTRAP": Object.freeze({ argv: ["--test", "test/project-memory-context.test.mjs"], command: "node --test test/project-memory-context.test.mjs", tests: 5 }),
  "WI-PM-MEM0-ADAPTER": Object.freeze({ argv: ["--test", "test/mem0-project-memory-adapter.test.mjs"], command: "node --test test/mem0-project-memory-adapter.test.mjs", tests: 3 }),
  "WI-PM-TRACEABILITY": Object.freeze({ argv: ["--test", "test/project-memory-traceability.test.mjs"], command: "node --test test/project-memory-traceability.test.mjs", tests: 3 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/pm-001-project-memory/integration-frontier-2/WI-PM-NATIVE-ENGINE/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-3/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-67e1cb3",
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} PM-001 third-frontier focused tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001ThirdFrontierVerificationSummary", results };
