...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-6");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-REGRESSION-PERFORMANCE": Object.freeze({ executable: process.platform === "win32" ? (process.env.ComSpec ?? "C:\\Windows\\System32\\cmd.exe") : "npm", argv: process.platform === "win32" ? ["/d", "/s", "/c", "npm.cmd run verify"] : ["run", "verify"], command: "npm.cmd run verify", tests: 1037, passed: 1035, skipped: 2 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/pm-001-project-memory/integration-frontier-5/WI-PM-DESKTOP-INTEGRATION/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-6/${workItemId}`;
  ...
  const artifactIds = {
    ...
    repositoryBase: "repository-snapshot-devrelay-afba138",
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001SixthFrontierVerificationSummary", results };
