...
const fullRef = (
  ...
  schema,
  mediaType,
  uri: `memory://devrelay/pm-001/${encodeURIComponent(artifactId)}/${digest.slice(7)}.json`,
});
...
function restoreGraph() {
  const graphPath =
    "dogfood/pm-001-project-memory/integration-frontier-7-fix-3/WI-PM-WINDOWS-E2E/traceability-graph-snapshot.json";
  const graphSummary = readJson(
    "dogfood/pm-001-project-memory/integration-frontier-7-fix-3/integration-summary.json",
  );
...
assert.equal(implementationCommit, "61274bee4c1c726952b509017c1af85c1e1bce66");
const canonicalVerification = readJson(
  "dogfood/pm-001-project-memory/system-verification-final/canonical-verification-receipt.json",
);
assert.equal(canonicalVerification.implementationCommit, implementationCommit);
assert.equal(canonicalVerification.outcome, "pass");
assert.equal(canonicalVerification.tests.failed, 0);
const canonicalVerificationBytes = readBytes(
  "dogfood/pm-001-project-memory/system-verification-final/canonical-verification-receipt.json",
);
const windowsDogfood = readJson(
  "dogfood/pm-001-project-memory/windows-e2e/installed-package-verification-receipt.json",
);
const windowsDogfoodBytes = readBytes(
  "dogfood/pm-001-project-memory/windows-e2e/installed-package-verification-receipt.json",
);
assert.equal(windowsDogfood.scenario.provider.maturity, "live-conformant");
assert.equal(windowsDogfood.scenario.restart.zeroCallReplay, true);
assert.equal(windowsDogfood.scenario.conclusion.outcome, "concluded");
const releaseCatalogBytes = readBytes(
  "dogfood/pm-001-project-memory/system-verification-final/verified-release-catalog.json",
);
const releaseEvidenceValue = seal(
  {
    apiVersion: API,
    kind: "PM001ReleaseCandidateEvidenceSet",
    evidenceSetId: "RELEASE-EVIDENCE-DEVRELAY-PM001-PROJECT-MEMORY-001",
    implementationCommit,
    verification: {
      command: canonicalVerification.command,
      total: canonicalVerification.tests.total,
      passed: canonicalVerification.tests.passed,
      failed: canonicalVerification.tests.failed,
      skipped: canonicalVerification.tests.skipped,
      stagedCandidateTree: canonicalVerification.stagedCandidateTree,
      receipt: ref(
        canonicalVerification.receiptId,
        sha256Digest(canonicalVerificationBytes),
      ),
    },
    windowsDesktopDogfood: ref(
      "PM001-WINDOWS-INSTALLED-PACKAGE-VERIFICATION",
      sha256Digest(windowsDogfoodBytes),
    ),
    releaseCatalog: ref(
      "devrelay-release-catalog-0.10.0-rc.3-system-verified",
      sha256Digest(releaseCatalogBytes),
    ),
    package: {
      ...
      installedExports: canonicalVerification.installedPackage.exports,
    },
    projectMemory: {
      provider: windowsDogfood.scenario.provider.id,
      version: windowsDogfood.scenario.provider.version,
      maturity: windowsDogfood.scenario.provider.maturity,
      networkAttempts: windowsDogfood.scenario.provider.networkAttempts,
      concludeOutcome: windowsDogfood.scenario.conclusion.outcome,
      zeroCallReplay: windowsDogfood.scenario.restart.zeroCallReplay,
    },
    releaseBoundary:
...
const integrationSources = [
  { integration: "dogfood/pm-001-project-memory/integration-frontier-1", verification: "dogfood/pm-001-project-memory/verification-frontier-1" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-2", verification: "dogfood/pm-001-project-memory/verification-frontier-2" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-3", verification: "dogfood/pm-001-project-memory/verification-frontier-3" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-4", verification: "dogfood/pm-001-project-memory/verification-frontier-4" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-5", verification: "dogfood/pm-001-project-memory/verification-frontier-5" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-6", verification: "dogfood/pm-001-project-memory/verification-frontier-6" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-6-fix-1", verification: "dogfood/pm-001-project-memory/verification-frontier-6-fix-1" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-6-fix-2", verification: "dogfood/pm-001-project-memory/verification-frontier-6-fix-2" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-7", verification: "dogfood/pm-001-project-memory/verification-frontier-7" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-7-fix-1", verification: "dogfood/pm-001-project-memory/verification-frontier-7-fix-1" },
  { integration: "dogfood/pm-001-project-memory/integration-frontier-7-fix-3", verification: "dogfood/pm-001-project-memory/verification-frontier-7-fix-3" },
];
...
const completionFactSet = seal(
  {
    ...
    factSetId: "ICFS-DEVRELAY-PM001-PROJECT-MEMORY-001",
...
const systemPolicy = seal(
  {
    ...
    policyId: "SV-POLICY-DEVRELAY-PM001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const integratedSubject = seal(
  {
    ...
    subjectId: `DEVRELAY-PM001-DESKTOP-WINDOWS-${releaseEvidence.value.evidenceSetDigest.slice(7, 19).toUpperCase()}`,
...
const reviewObligations = obligations.obligations
  ...
  .map(({ obligationId }) => obligationId);
const invocations = [
  invocationFor("SV-DEVRELAY-PM001-TEST-001", TEST_SYSTEM_VERIFIER, testObligations),
  invocationFor("SV-DEVRELAY-PM001-REVIEW-001", REVIEW_SYSTEM_VERIFIER, reviewObligations),
];
...
const verifier = async (invocation) => {
  ...
  if (invocation.verifier.id === TEST_SYSTEM_VERIFIER.id) {
    ...
    return adaptSystemTestResult({
      invocation,
      nativeBytes: bytes,
      nativeArtifact: ref("NATIVE-DEVRELAY-PM001-TEST-001", sha256Digest(bytes)),
    });
  }
  const native = {
  ...
  return adaptSystemReviewResult({
    invocation,
    nativeBytes: bytes,
    nativeArtifact: ref("NATIVE-DEVRELAY-PM001-REVIEW-001", sha256Digest(bytes)),
  });
};
...
const systemInvocation = {
  invocationId: "SYSTEM-VERIFICATION-DEVRELAY-PM001-001",
...
const businessPolicy = seal(
  {
    ...
    policyId: "BA-POLICY-DEVRELAY-PM001-DESKTOP-WINDOWS-001",
    version: "1.0.0",
...
const businessEvidence = seal(
  {
    ...
    evidenceSetId: "BA-EVIDENCE-DEVRELAY-PM001-DESKTOP-WINDOWS-001",
    requirementsBaseline: ref(requirements.ref.artifactId, requirements.ref.digest),
    projectOverviewBaseline: ref(overview.ref.artifactId, overview.ref.digest),
...
const businessInput = {
  checkpointId: "BA-DEVRELAY-PM001-DESKTOP-WINDOWS-001",
...
const ownerApproval = seal(
  {
    ...
    approvalId: "BA-APPROVAL-DEVRELAY-PM001-DESKTOP-WINDOWS-001",
...
let ownerCalls = 0;
const gateInput = {
  checkpointId: "BA-GATE-DEVRELAY-PM001-DESKTOP-WINDOWS-001",
...
const businessInvocation = {
  invocationId: "BUSINESS-ACCEPTANCE-GATE-DEVRELAY-PM001-001",
  module: {
...
const approvalRequest = seal(
  {
    ...
    requestId: "BA-APPROVAL-REQUEST-DEVRELAY-PM001-DESKTOP-WINDOWS-001",
    candidate: ref(business.candidate.candidateId, business.candidate.candidateDigest),
    candidateRawDigest: sha256Digest(candidateBytes),
    technicalCoverage: ref(
      business.technicalCoverage.coverageId,
...
const finalProof = seal(
  {
    apiVersion: API,
    kind: "DevRelayPM001SourceReleaseAcceptanceProof",
    proofId: "DEVRELAY-PM001-SOURCE-RELEASE-ACCEPTANCE-001",
    sourceBaseCommit,
...
const summary = {
  apiVersion: API,
  kind: "DevRelayPM001FinalAcceptanceSummary",
  releaseEvidence: releaseEvidenceRef,
...
writeText(
  "FINAL_ACCEPTANCE.md",
  `# DevRelay pm-001 final acceptance\n\n` +
