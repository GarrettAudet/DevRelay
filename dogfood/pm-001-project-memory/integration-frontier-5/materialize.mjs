...
const outRoot = resolve(root, "dogfood/pm-001-project-memory/integration-frontier-5");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/pm-001-integration-frontier-5";
const sourceRef = "refs/heads/codex/pm-001-source-frontier-5";
const ids = Object.freeze([
  "WI-PM-DESKTOP-INTEGRATION",
]);
...
const baseCommit = json("dogfood/pm-001-project-memory/integration-frontier-4/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/pm-001-project-memory/execution-frontier-5/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/pm-001-project-memory/execution-frontier-5/execution-summary.json");
let graphSnapshot = json("dogfood/pm-001-project-memory/execution-frontier-5/WI-PM-DESKTOP-INTEGRATION/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/pm-001-project-memory/verification-frontier-5/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/pm-001-project-memory/execution-frontier-5/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Pm001FifthFrontierIntegrationSummary",
  targetRef,
  baseCommit,
