...
const outRoot = resolve(root, "dogfood/pm-001-project-memory/integration-frontier-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/pm-001-integration-frontier-1";
const sourceRef = "refs/heads/codex/pm-001-source-frontier-1";
const ids = Object.freeze([
  "WI-PM-ARTIFACT-CONTRACTS",
]);
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/pm-001-project-memory/execution-frontier-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/pm-001-project-memory/execution-frontier-1/execution-summary.json");
let graphSnapshot = json("dogfood/pm-001-project-memory/execution-frontier-1/WI-PM-ARTIFACT-CONTRACTS/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/pm-001-project-memory/verification-frontier-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/pm-001-project-memory/execution-frontier-1/${workItemId}/change-set-draft.json`);
...
const summary = {
  apiVersion: API,
  kind: "Pm001FirstFrontierIntegrationSummary",
  targetRef,
  baseCommit,
