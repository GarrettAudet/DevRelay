...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-2");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-NATIVE-ENGINE": Object.freeze({ argv: ["--test", "test/project-memory.test.mjs", "test/project-memory-artifacts.test.mjs"], command: "node --test test/project-memory.test.mjs test/project-memory-artifacts.test.mjs", tests: 21 }),
});
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/pm-001-project-memory/integration-frontier-1/WI-PM-ARTIFACT-CONTRACTS/post-repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-2/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} PM-001 native-engine frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001SecondFrontierVerificationSummary", results };
