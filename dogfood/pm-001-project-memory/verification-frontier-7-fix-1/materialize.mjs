...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-7-fix-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-WINDOWS-E2E": Object.freeze({ executable: process.execPath, argv: ["--test", "test/project-baseline.test.mjs", "test/bootstrap-structurizr.test.mjs", "test/contract-generation-architecture-design-dogfood.test.mjs"], command: "node --test PM-001 SystemVerification remediation lane", tests: 6, passed: 6, skipped: 0 }),
});
...
const artifactRef = (artifactId, artifact) => ({
  ...
  uri: `memory://devrelay/pm001/${encodeURIComponent(artifactId)}`,
});
...
const baselines = {
  ...
  repositoryBase: (() => {
    const revision = execFileSync("git", ["rev-parse", "refs/heads/codex/pm-001-integration-frontier-7"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
    const tree = execFileSync("git", ["rev-parse", `${revision}^{tree}`], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
    return { apiVersion: "devrelay.dev/v1alpha1", kind: "RepositorySnapshot", repository: root.replaceAll("\\", "/"), revision, treeDigest: api.canonicalJsonDigest({ commit: revision, tree }), includedPaths: ["contracts/**", "dogfood/pm-001-project-memory/windows-e2e/**", "src/**", "test/**"], excludedPaths: [".git/**", "node_modules/**"] };
  })(),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-7-fix-1/${workItemId}`;
  ...
  const testRun = spawnSync(job.executable, job.argv, {
    cwd: root,
    encoding: null,
    windowsHide: true,
  ...
  const candidateWorkspace = {
    ...
    version: "1.0.0",
    workspaceId: `CWS-${workItemId}-002`,
  ...
  const artifactIds = {
    ...
    repositoryBase: `repository-snapshot-devrelay-${baselines.repositoryBase.revision.slice(0, 7)}`,
  ...
  const subject = api.bindWorkItemVerificationSubject({ subjectId: `SUB-${workItemId}-002`, workItemId, bindings });
  ...
  const binding = api.validateVerifierBindingSet({
    bindingId: `BIND-${workItemId}-TEST-002`,
    subject,
    obligationSet,
  ...
  const invocation = seal({
    ...
    verificationAttemptId: `VAT-${workItemId}-002`,
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent verifier passed all ${job.tests} focused PM-001 SystemVerification remediation checks for the exact approved baseline pair and Windows Structurizr bootstrap bytes for ${workItemId}.`,
  ...
  const controller = api.createWorkItemVerificationCheckpointController({
    verifier: async (exactInvocation) => {
      ...
      const raw = api.adaptTestVerifierResult({
        ...
        nativeBytes,
        nativeResult,
        nativeArtifact: ref(`NATIVE-${workItemId}-TEST-002`, api.sha256Digest(nativeBytes)),
      });
  ...
  const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({
    ...
    normalizedEvidenceId: `NVE-${workItemId}-002`,
  });
  const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
  const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId: `WIVC-${workItemId}-002`, policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation: evaluation });
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001SeventhFrontierFixOneVerificationSummary", results };
