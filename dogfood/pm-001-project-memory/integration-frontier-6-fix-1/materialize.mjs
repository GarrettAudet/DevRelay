...
const outRoot = resolve(root, "dogfood/pm-001-project-memory/integration-frontier-6-fix-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/pm-001-integration-frontier-6-fix-1";
const sourceRef = "refs/heads/codex/pm-001-source-frontier-6-fix-1";
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/pm-001-project-memory/execution-frontier-6-fix-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/pm-001-project-memory/execution-frontier-6-fix-1/execution-summary.json");
let graphSnapshot = json("dogfood/pm-001-project-memory/execution-frontier-6-fix-1/WI-PM-REGRESSION-PERFORMANCE/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/pm-001-project-memory/verification-frontier-6-fix-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/pm-001-project-memory/execution-frontier-6-fix-1/${workItemId}/change-set-draft.json`);
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-PM001-${workItemId}`,
  });
  ...
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-${workItemId}`,
...
const summary = {
  apiVersion: API,
  kind: "Pm001SixthFrontierFixOneIntegrationSummary",
  targetRef,
  baseCommit,
