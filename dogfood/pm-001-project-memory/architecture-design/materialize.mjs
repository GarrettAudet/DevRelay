...
import { buildProjectMemoryArchitecture } from "./project-memory-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/pm-001-project-memory/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/architecture-design";
...
const projectContext = load("dogfood/pm-001-project-memory/project-context.json");
const repositorySnapshot = load("dogfood/pm-001-project-memory/repository-snapshot.json");
const architectureBaseline = load(
  "project/architecture-baseline.json",
);
const ownerDecisionPath = "docs/specs/pm-001-project-memory/owner-architecture-decisions.json";
...
const projectContextRef = artifactRef(
  ...
  sha256(read("dogfood/pm-001-project-memory/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  "repository-snapshot-devrelay-a18e6fb-pm-001-project-memory",
  "https://devrelay.dev/artifacts/repository-snapshot/v1",
  "application/vnd.devrelay.repository-snapshot+json",
  sha256(read("dogfood/pm-001-project-memory/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/pm-001-project-memory/repository-snapshot.json",
);
...
const ownerDecisionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/docs/specs/pm-001-project-memory/owner-architecture-decisions.json",
);
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-pm-001-project-memory-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-pm-001-project-memory-architecture-v1",
...
const discoveryDecision = {
  ...
  decisionId: "architecture-discovery-bypass-pm-001-project-memory-v1",
...
const {
  ...
  risks,
  uniq,
} = buildProjectMemoryArchitecture({
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay PM-001 ProjectMemory")} ${dslQuote("Authoritative project memory, bounded Mem0 retrieval, trace projection, and conclusion control")} {`,
    "  !identifiers flat",
...
writeText(
  ...
  `# DevRelay architecture\n\nThis software-system-scoped workspace materializes the approved DevRelay architecture baseline and the PM-001 ProjectMemory change. The canonical DevRelay artifact remains authoritative; Structurizr validates hierarchy, relationships, views, documentation, and decisions through a live provider execution.\n`,
);
...
const designRef = fileRef(
  "openspec-design-pm-001-project-memory-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-pm-001-project-memory-v2",
...
const views = diagramViewSpecs.map((view) => ({
  ...view,
  renderings: view.viewKey.startsWith("VIEW-PM-")
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/pm-001-project-memory/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-PM-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-PM-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-PM-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-PM-001", entries: nativeEntries } },
};
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      ...
      rationale: operation === "add"
        ? `Add the ${kind} required by the approved PM-001 ProjectMemory design.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the approved PM-001 memory and conclusion target architecture.`,
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    ...
    decisionId: id,
    operation: "add",
    rationale: "Record the approved PM-001 ProjectMemory architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-pm-001-project-memory-v2",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-pm-001-project-memory-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-pm-001-project-memory-v2",
  operation: "design-change",
  projectArchitectureState: stateRef,
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/pm-001-project-memory/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/pm-001-project-memory/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/pm-001-project-memory",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-pm-001-project-memory-v2",
...
register(projectContextRef, read("dogfood/pm-001-project-memory/project-context.json"));
register(repositorySnapshotRef, read("dogfood/pm-001-project-memory/repository-snapshot.json"));
register(architectureBaselineRef, read("project/architecture-baseline.json"));
register(ownerDecisionRef, ownerDecisionBytes);
register(architectureBaselineProjectContextRef, read("dogfood/rm-001-roadmap-management/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/rm-001-roadmap-management/repository-snapshot.json"));
...
const traceabilityGraph = createTraceabilityGraphService({
  graphId: "traceability-graph-devrelay-pm-001-project-memory",
  projectId: "devrelay",
...
if (process.argv.includes("--promote")) {
  const { promoteProjectMemoryArchitecture } = await import("./promote.mjs");
  const promotion = await promoteProjectMemoryArchitecture({
    root,
    dir,
    read,
    fileRef,
