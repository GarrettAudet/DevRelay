...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-7-fix-3");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-WINDOWS-E2E": Object.freeze({ executable: process.execPath, argv: ["--test", "test/change-integration-local-git-adapter.test.mjs"], command: "node --test Windows local Git integration adapter regression", tests: 14, passed: 14, skipped: 0 }),
});
...
const baselines = {
  ...
  repositoryBase: (() => {
    const revision = execFileSync("git", ["rev-parse", "refs/heads/codex/pm-001-integration-frontier-7-fix-1"], { cwd: root, encoding: "utf8", windowsHide: true }).trim();
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-7-fix-3/${workItemId}`;
  ...
  const candidateWorkspace = {
    ...
    version: "1.0.0",
    workspaceId: `CWS-${workItemId}-004`,
  ...
  const subject = api.bindWorkItemVerificationSubject({ subjectId: `SUB-${workItemId}-004`, workItemId, bindings });
  ...
  const binding = api.validateVerifierBindingSet({
    bindingId: `BIND-${workItemId}-TEST-004`,
    subject,
    obligationSet,
  ...
  const invocation = seal({
    ...
    verificationAttemptId: `VAT-${workItemId}-004`,
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent verifier passed all ${job.tests} Windows local Git integration adapter checks, including the exhausted cleanup-lock regression, for ${workItemId}.`,
  ...
  const controller = api.createWorkItemVerificationCheckpointController({
    verifier: async (exactInvocation) => {
      ...
      const raw = api.adaptTestVerifierResult({
        ...
        nativeBytes,
        nativeResult,
        nativeArtifact: ref(`NATIVE-${workItemId}-TEST-004`, api.sha256Digest(nativeBytes)),
      });
  ...
  const normalizedEvidence = api.normalizeWorkItemVerificationEvidence({
    ...
    normalizedEvidenceId: `NVE-${workItemId}-004`,
  });
  const evaluation = api.evaluateWorkItemVerificationPolicy({ policy, subject, obligationSet, binding, normalizedEvidence });
  const gateCandidate = api.assembleWorkItemVerificationGateCandidate({ candidateId: `WIVC-${workItemId}-004`, policy, subject, obligationSet, binding, normalizedEvidence, policyEvaluation: evaluation });
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001SeventhFrontierFixThreeVerificationSummary", results };
