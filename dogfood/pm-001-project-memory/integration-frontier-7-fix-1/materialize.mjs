...
const outRoot = resolve(root, "dogfood/pm-001-project-memory/integration-frontier-7-fix-1");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/pm-001-integration-frontier-7-fix-1";
const sourceRef = "refs/heads/codex/pm-001-source-frontier-7-fix-1";
...
const baseCommit = json("dogfood/pm-001-project-memory/integration-frontier-7/integration-summary.json").finalCommit;
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/pm-001-project-memory/execution-frontier-7-fix-1/${workItemId}/change-set-draft.json`);
...
const graphSummary = json("dogfood/pm-001-project-memory/execution-frontier-7-fix-1/execution-summary.json");
let graphSnapshot = json("dogfood/pm-001-project-memory/execution-frontier-7-fix-1/WI-PM-WINDOWS-E2E/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/pm-001-project-memory/verification-frontier-7-fix-1/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/pm-001-project-memory/execution-frontier-7-fix-1/${workItemId}/change-set-draft.json`);
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-PM001-${workItemId}-FRONTIER-7-FIX-1`,
  });
  ...
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-${workItemId}-FRONTIER-7-FIX-1`,
...
const summary = {
  apiVersion: API,
  kind: "Pm001SeventhFrontierFixOneIntegrationSummary",
  targetRef,
  baseCommit,
