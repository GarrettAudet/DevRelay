...
const outRoot = resolve(root, "dogfood/pm-001-project-memory/integration-frontier-7-fix-3");
mkdirSync(outRoot, { recursive: true });
const API = "devrelay.dev/v1alpha1";
const targetRef = "refs/heads/codex/pm-001-integration-frontier-7-fix-3";
const sourceRef = "refs/heads/codex/pm-001-source-frontier-7-fix-3";
...
const fullRef = (artifactId, digest, schema = "https://devrelay.dev/internal/v1", mediaType = "application/json") => ({ artifactId, schema, mediaType, digest, uri: `memory://pm001/${artifactId}` });
...
const baseCommit = json("dogfood/pm-001-project-memory/integration-frontier-7-fix-1/integration-summary.json").finalCommit;
const sourceRoot = mkdtempSync(join(tmpdir(), "devrelay-pm001-source-"));
...
try {
  ...
  for (const workItemId of ids) {
    const change = json(`dogfood/pm-001-project-memory/execution-frontier-7-fix-3/${workItemId}/change-set-draft.json`);
    ...
    git(sourceRepo, "commit", "-m", `PM-001 ${workItemId}`);
...
const graphSummary = json("dogfood/pm-001-project-memory/execution-frontier-7-fix-3/execution-summary.json");
let graphSnapshot = json("dogfood/pm-001-project-memory/execution-frontier-7-fix-3/WI-PM-WINDOWS-E2E/traceability-graph-snapshot.json");
...
for (const workItemId of ids) {
  ...
  const verificationDir = `dogfood/pm-001-project-memory/verification-frontier-7-fix-3/${workItemId}`;
  ...
  const verifiedChange = json(`dogfood/pm-001-project-memory/execution-frontier-7-fix-3/${workItemId}/change-set-draft.json`);
  ...
  const targetSnapshot = {
    ...
    includedPaths: ["contracts/**", "dogfood/pm-001-project-memory/windows-e2e/**", "src/**", "test/**"],
    excludedPaths: [".git/**", "node_modules/**"],
  };
  const targetReference = ref(`REPO-PM001-INTEGRATION-${currentCommit.slice(0, 12).toUpperCase()}`, api.canonicalJsonDigest(targetSnapshot));
  const integrationPolicy = { kind: "IntegrationPolicy", version: "1.0.0", mode: "verified-fast-forward", targetRef };
  const integrationPolicyBytes = Buffer.from(`DevRelay PM-001 verified fast-forward policy for ${targetRef}\n`, "utf8");
  ...
  const validatedInput = api.bindChangeIntegrationInputs({
    ...
    idempotencyKey: `CI-PM001-${workItemId}-FRONTIER-7-FIX-3`,
  });
  ...
  const coordinated = await api.authorizeChangeIntegrationEffect({
    plan,
    invocationId: `CI-INVOCATION-${workItemId}-FRONTIER-7-FIX-3`,
...
const summary = {
  apiVersion: API,
  kind: "Pm001SeventhFrontierFixThreeIntegrationSummary",
  targetRef,
  baseCommit,
