...
const verificationRoot = resolve(root, "dogfood/pm-001-project-memory/verification-frontier-1");
mkdirSync(verificationRoot, { recursive: true });

const verificationJobs = Object.freeze({
  "WI-PM-ARTIFACT-CONTRACTS": Object.freeze({
    argv: ["--test", "test/project-memory-artifacts.test.mjs"],
...
const baselines = {
  ...
  repositoryBase: readJson("dogfood/pm-001-project-memory/repository-snapshot.json"),
};
...
for (const workItemId of workItemIds) {
  ...
  const executionDir = `dogfood/pm-001-project-memory/execution-frontier-1/${workItemId}`;
  ...
  const nativeResult = {
    ...
    tests: invocation.assignedObligations.map((obligation) => ({
      ...
      status: "pass",
      summary: `The independent focused verifier passed all ${job.tests} PM-001 first-frontier tests for the exact ${workItemId} candidate bytes.`,
...
const summary = { apiVersion: "devrelay.dev/v1alpha1", kind: "Pm001FirstFrontierVerificationSummary", results };
