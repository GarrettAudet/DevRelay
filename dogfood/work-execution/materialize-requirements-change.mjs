...
import {
  buildWorkExecutionRequirements,
  ownerDecisions,
} from "./work-execution-approved-data.mjs";
...
const goalDocument = await loadExisting({
  path: "dogfood/work-execution/goal.json",
  artifactId: "goal-work-execution-module-v1",
  type: "goal",
  uri: `${canonicalArtifactRoot}/goal.json`,
});
const projectContextDocument = await loadExisting({
  path: "dogfood/work-execution/project-context.json",
  artifactId: "project-context-devrelay-work-execution-v1",
  type: "projectContext",
  uri: `${canonicalArtifactRoot}/project-context.json`,
});
const clarificationRequestDocument = await loadExisting({
  path: "dogfood/work-execution/clarification-request.json",
  artifactId: "clarification-request-work-execution-v1",
  type: "clarificationRequest",
  uri: `${canonicalArtifactRoot}/clarification-request.json`,
});
const continuationDocument = await loadExisting({
  path: "dogfood/work-execution/requirements-continuation.json",
  artifactId: "requirements-continuation-work-execution-v1",
  type: "continuation",
...
const clarificationResponseRef = store.addJson(
  ...
  "clarification-response-work-execution-v1",
...
const ownerDecisionRef = store.addText(
  "owner-decisions.md",
  "owner-decisions-work-execution-v1",
  ownerDecisionText,
);
const proposalRef = store.addText(
  "native/openspec/proposal.md",
  "openspec-proposal-work-execution-v1",
  "# WorkExecution proposal\n\nAdd one provider-neutral WorkExecution module that executes one Core-selected runnable work item per immutable isolated attempt through a version-pinned execution binding. It returns proposed changes and raw evidence only; verification and integration remain downstream.\n",
);
const specificationRef = store.addText(
  "native/openspec/specs/work-execution/spec.md",
  "openspec-spec-work-execution-v1",
...
const replacement = buildWorkExecutionRequirements(
...
const requirementsChangeSet = {
  ...
  changeSetId: "requirements-change-set-work-execution-v1",
  ...
  changedSections,
  reason:
    "Define the WorkExecution module, Core readiness and binding authority, isolated attempt boundary, typed proposed-change and raw-evidence outputs, immutable retry semantics, and traceability boundary approved through clarification.",
  ...
  risks: [
    ...
    "An executor can exceed scope or make unauthorized completion claims unless Core and the external host enforce readiness, binding, permissions, output, and retry boundaries.",
  ],
  requiredEvidence: [
...
const candidateMarkdownRef = store.addBytes(
  ...
  "project-overview-markdown-work-execution-v1",
...
const overviewChangeSet = {
  ...
  changeSetId: "project-overview-change-set-work-execution-v1",
...
const nativeSourceBundle = {
  ...
  sources: [
    {
      role: "owner-decision",
      path: "dogfood/work-execution/owner-decisions.md",
      artifact: pointer(ownerDecisionRef),
    },
    {
      role: "proposal",
      path: "dogfood/work-execution/native/openspec/proposal.md",
      artifact: pointer(proposalRef),
    },
    {
      role: "specification",
      path: "dogfood/work-execution/native/openspec/specs/work-execution/spec.md",
...
const nativeSourceBundleRef = store.addJson(
  ...
  "native-source-bundle-work-execution-v1",
  "nativeSourceBundle",
  nativeSourceBundle,
);
...
const invocation = {
  ...
  invocationId: "invocation-requirements-work-execution-openspec-resume-v1",
  runId: "dogfood-work-execution-v1-resume",
...
const sourceCheckpointDocument = JSON.parse(
  await readFile(
    new URL("dogfood/work-execution/requirements-gathering.checkpoint.json", root),
    "utf8",
  ),
);
...
const executionProof = {
  ...
  proofId: "requirements-change-work-execution-v1",
