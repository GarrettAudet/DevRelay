...
import { buildWorkExecutionArchitecture } from "./work-execution-architecture-data.mjs";

const root = process.cwd();
const dir = path.join(root, "dogfood/work-execution/architecture-design");
const logicalRoot = "file:///C:/repos/DevRelay/dogfood/work-execution/architecture-design";
...
const projectContext = load("dogfood/work-execution/project-context.json");
const repositorySnapshot = load("dogfood/work-execution/repository-snapshot.json");
const currentArchitectureBaseline = load("project/architecture-baseline.json");
const architectureBaselineSourcePath =
  currentArchitectureBaseline.approvedDraft?.artifactId ===
  "architecture-change-set-work-execution-v1"
    ? "project/history/architecture/architecture-baseline-devrelay-v1-specialist-assignment-001/architecture-baseline.json"
...
const projectContextRef = artifactRef(
  ...
  sha256(read("dogfood/work-execution/project-context.json")),
  "file:///C:/repos/DevRelay/dogfood/work-execution/project-context.json",
);
const repositorySnapshotRef = artifactRef(
  ...
  sha256(read("dogfood/work-execution/repository-snapshot.json")),
  "file:///C:/repos/DevRelay/dogfood/work-execution/repository-snapshot.json",
);
...
const projectArchitectureState = {
  ...
  stateId: "project-architecture-state-work-execution-v1",
  state: "baselined",
...
const routeRef = fileRef(
  "module-route-decision-work-execution-architecture-v1",
...
const {
  ...
  risks,
  uniq,
} = buildWorkExecutionArchitecture({
...
const buildStructurizrDsl = () => {
  ...
  const lines = [
    `workspace ${dslQuote("DevRelay WorkExecution")} ${dslQuote("Bounded execution and Gate target architecture")} {`,
    "  !identifiers flat",
...
const designRef = fileRef(
  "openspec-design-work-execution-v1",
...
const dslRef = fileRef(
  "structurizr-workspace-work-execution-v1",
...
const diagrams = {
  diagramSetId: "DIAGRAMS-DEVRELAY-WE-005",
...
const nativeEntry = (id, artifact, role, logicalPath, stage, adapterId, toolName, mappings) => ({
  ...
  artifact,
  role,
  logicalPath: `dogfood/work-execution/architecture-design/${logicalPath}`,
  producedBy: {
    stage,
    adapterId,
...
const nativeEntries = [
  nativeEntry("NA-WE-OPENSPEC-DESIGN", designRef, "technical-design-source", "design.md", "designer", "openspec-design", "OpenSpec", [
    { section: "technicalDesign", entityIds: [technicalDesign.technicalDesignId], jsonPointers: ["/sections/technicalDesign"] },
    { section: "interfaceIntent", entityIds: interfaces.filter(({ id }) => id.startsWith("IF-WE-")).map(({ id }) => id), jsonPointers: ["/sections/interfaceIntent"] },
    { section: "architectureConstraints", entityIds: constraints.filter(({ id }) => id.startsWith("CON-WE-")).map(({ id }) => id), jsonPointers: ["/sections/architectureConstraints"] },
  ]),
  nativeEntry("NA-WE-STRUCTURIZR", dslRef, "structurizr-workspace", "workspace.dsl", "modeler", "structurizr", "Structurizr DSL", [
  ...
  ...decisionSpecs.map((spec, index) => nativeEntry(
    `NA-WE-MADR-${String(index + 1).padStart(3, "0")}`,
    spec.nativeRef,
...
const sections = {
  ...
  interfaceIntent: { mode: "embedded", content: { interfaceIntentSetId: "INTERFACES-DEVRELAY-WE-001", interfaces } },
  architectureConstraints: { mode: "embedded", content: { constraintSetId: "CONSTRAINTS-DEVRELAY-WE-001", constraints } },
  decisionRecords: { mode: "embedded", content: { decisionRecordSetId: "DECISIONS-DEVRELAY-WE-001", decisions } },
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-DEVRELAY-WE-001", entries: nativeEntries } },
};
...
for (const [collection, kind, beforeValues, afterValues, idOf] of entityCollections) {
  ...
  for (const id of [...new Set([...before.keys(), ...after.keys()])].sort()) {
    ...
    const change = {
      changeId: `CHG-WE-${kind.toUpperCase()}-${id}`,
      entityKind: kind,
      entityId: id,
      operation,
      rationale: operation === "add"
        ? `Add the ${kind} required by bounded work execution.`
        : operation === "remove"
          ? `Remove the obsolete ${kind} to restore faithful C4 materialization.`
          : `Modify the ${kind} for the bounded work-execution target architecture.`,
      compatibilityImpact: "backward-compatible",
      sourceRequirementIds: uniq(
        operation === "remove"
          ? [CG.execute, CG.deterministic]
          : kind === "view"
            ? [CG.execute, CG.deterministic]
...
for (const id of decisionSpecs.map(({ id }) => id)) {
  ...
  changes.decisionChanges.push({
    changeId: `CHG-WE-DECISION-ADD-${id}`,
    decisionId: id,
    operation: "add",
    rationale: "Record the bounded work-execution architecture choice as a proposed MADR.",
...
const changeSet = {
  ...
  changeSetId: "architecture-change-set-work-execution-v1",
...
const designerWorking = {
  ...
  workingArtifactId: "architecture-designer-working-work-execution-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  baseInputs,
  ...
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-WE-DESIGNER-001", entries: [nativeEntries[0]] } },
  assumptions,
  risks,
  sourceRefs: [],
};
...
const modelerWorking = {
  ...
  workingArtifactId: "architecture-modeler-working-work-execution-v1",
  operation: "design-change",
  projectArchitectureState: stateRef,
  designerWorkingArtifact: designerRef,
  architectureModel: sections.architectureModel,
  diagrams: sections.diagrams,
  nativeArtifacts: { mode: "embedded", content: { nativeArtifactSetId: "NATIVE-WE-MODELER-001", entries: [nativeEntries[1]] } },
  sourceRefs: [],
};
...
const conformanceProof = await verifyStructurizrConformance({
  rootPath: root,
  workspacePath:
    "dogfood/work-execution/architecture-design/workspace.dsl",
  candidatePath:
    "dogfood/work-execution/architecture-design/architecture-change-set-draft.json",
  outputDirectory:
    ".devrelay/conformance/work-execution",
});
...
const conformanceRef = fileRef(
  "structurizr-conformance-proof-work-execution-v1",
...
const invocation = {
  ...
  invocationId: "architecture-design-change-work-execution-v1",
  runId: "work-execution-architecture-run-v1",
  nodeId: "architecture-design",
  module: { id: "architecture-design", version: "0.1.0", operation: "design-change" },
  adapters: [
    { step: "designer", plugin: { id: "openspec-design", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "1.0", changeName: "work-execution", schema: "devrelay-architecture", artifact: "design.md", bridge: "agent-command", toolName: "OpenSpec" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/openspec/changes" }, { kind: "network.connect", scope: "host:implementation-engine" }] },
    { step: "modeler", plugin: { id: "structurizr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", toolVersion: "2026.06.28", workspacePath: "dogfood/work-execution/architecture-design/workspace.dsl", exportFormat: "static", toolName: "Structurizr DSL" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/architecture" }, { kind: "process.spawn", scope: "structurizr-export" }] },
    { step: "decision-recorder", plugin: { id: "madr", version: "0.1.0" }, config: { projectRoot: "C:/repos/DevRelay", templateVersion: "4.0", decisionsPath: "C:/repos/DevRelay/dogfood/work-execution/architecture-design", toolName: "MADR", toolVersion: "4.0" }, grants: [{ kind: "filesystem.read", scope: "C:/repos/DevRelay" }, { kind: "filesystem.write", scope: "C:/repos/DevRelay/dogfood/work-execution/architecture-design" }] },
  ],
  inputs: {
    "project-architecture-state": [stateRef],
...
register(projectContextRef, read("dogfood/work-execution/project-context.json"));
register(repositorySnapshotRef, read("dogfood/work-execution/repository-snapshot.json"));
register(architectureBaselineRef, read(architectureBaselineSourcePath));
register(architectureBaselineProjectContextRef, read("dogfood/specialist-assignment/project-context.json"));
register(architectureBaselineRepositorySnapshotRef, read("dogfood/specialist-assignment/repository-snapshot.json"));
...
if (process.argv.includes("--promote")) {
  const { promoteWorkExecutionArchitecture } = await import("./promote.mjs");
  const promotion = await promoteWorkExecutionArchitecture({
    root,
    dir,
    read,
    fileRef,
