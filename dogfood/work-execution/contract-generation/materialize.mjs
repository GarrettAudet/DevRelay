...
const RUN_ID = "work-execution-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-work-execution-v1";
const GRAPH_ID = "devrelay/work-execution-contract-change";
...
currentContractBaseline.ref = structuredClone(persistedContractState.contractBaseline);
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== 25) {
  throw new Error(`expected 25 required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-WORK-EXECUTION-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-WORK-EXECUTION-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-work-execution-v1",
);
...
const checkpointFile = await exactJsonFile(
  ...
  "application/json",
  "CG-CHECKPOINT-WORK-EXECUTION-001",
);
  ...
  evidence: [
    {
      ...
      summary: "Core independently validated all 25 native JSON Schema draft 2020-12 artifacts.",
    },
    {
      ...
      summary: "Core computed a deterministic five-add contract diff against the exact 20-contract baseline.",
    },
  ],
  diagnostics: [],
};
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-WORK-EXECUTION-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-WORK-EXECUTION-CONTRACT-CHANGE-001",
...
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-WORK-EXECUTION-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
...
await exactJsonFile(
  ...
  "application/json",
  "CG-RUNTIME-PROOF-WORK-EXECUTION-001",
);
