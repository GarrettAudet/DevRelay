...
const projectOverview = read("../baselines/project-overview-baseline.json");
...
const a2aSnapshot = {
  source: {
    ...
    uri: "fixture://devrelay/a2a/documentation-specialist-card.json",
  },
  agentCard,
  toolIds: [],
  grantIds: [],
...
const documentationWorkItem = workBreakdown.workItems.find(({ id }) => id === "WI-SA-DOCUMENTATION");
const assignmentPolicy = {
  apiVersion: "devrelay.dev/v1alpha1",
  kind: "AssignmentPolicy",
  policyId: "AP-SA-DOGFOOD-001",
  version: "1.0.0",
  workItemRules: [],
  profilePriorities: [
  ...
  rationale: `Prefer the A2A documentation profile when it is eligible for ${documentationWorkItem.id}; retain a complete offline fallback.`,
};
...
const bindings = [
  ...
  ["project-overview-baseline", projectOverview.baselineId, canonicalJsonDigest(projectOverview)],
  ["repository-context", "repository-snapshot-devrelay-4bda7fe", canonicalJsonDigest(repositoryContext)],
].map(([role, artifactId, digest]) => ({ role, artifact: { artifactId, digest } }));
