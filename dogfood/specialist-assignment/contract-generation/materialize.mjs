...
import { assertLoadedJsonValueFidelity, normalizeRepositoryArtifactUris, repositoryArtifactUri, repositoryArtifactUriFromUrl } from "../../_support/repository-artifact-uri.mjs";
...
const RUN_ID = "specialist-assignment-contract-change-run-v1";
const INVOCATION_ID = "contract-generation-change-specialist-assignment-v1";
const GRAPH_ID = "devrelay/specialist-assignment-contract-change";
...
const architecture = await loadedFile(
  "dogfood/specialist-assignment/architecture-design/architecture-baseline.json",
...
architecture.ref = {
  ...architecture.ref,
  uri: repositoryArtifactUri("project/architecture-baseline.json"),
};
const projectOverview = await loadedFile(
  "dogfood/specialist-assignment/baselines/project-overview-baseline.json",
...
const requirements = await loadedFile(
  "dogfood/specialist-assignment/baselines/requirements-baseline.json",
...
const upstreamArchitecture = architecture;
...
const currentContractBaseline = await loadedFile(
  "dogfood/contract-generation/contract-generation/replay-v7/contract-baseline.json",
  "https://devrelay.dev/artifacts/contract-baseline/v1",
  "application/vnd.devrelay.contract-baseline+json",
  "baselineId",
);
const persistedContractState = JSON.parse(
  await readFile(new URL("dogfood/contract-generation/contract-generation/replay-v7/project-contract-state-baselined.json", ROOT), "utf8"),
);
currentContractBaseline.ref = normalizeRepositoryArtifactUris(persistedContractState.contractBaseline);
...
const requiredInterfaceIntentIds = architecture.value.sections.interfaceIntent.content.interfaces
  ...
  .map(({ id }) => id)
  .sort();
if (requiredInterfaceIntentIds.length !== 20) {
  throw new Error(`expected 20 required interface intents, found ${requiredInterfaceIntentIds.length}`);
}
const state = await exactJsonFile(
  "project-contract-state.json",
  {
    ...
    stateId: "PCS-DEVRELAY-CONTRACTS-SPECIALIST-ASSIGNMENT-001",
    state: "baselined",
  ...
  "PCS-DEVRELAY-CONTRACTS-SPECIALIST-ASSIGNMENT-001",
);
...
const routeDecision = await exactJsonFile(
  ...
  "module-route-decision-contract-generation-v1",
);
  ...
  evidence: [
    {
      ...
      summary: "Core independently validated all 20 native JSON Schema draft 2020-12 artifacts.",
    },
    {
      ...
      summary: "Core computed a deterministic six-add contract diff against the exact 14-contract baseline.",
    },
  ],
  diagnostics: [],
};
...
const executionRecordFile = await exactJsonFile(
  ...
  "MER-CONTRACT-CHANGE-SPECIALIST-ASSIGNMENT-001",
);
...
const gateReview = await exactJsonFile(
  "contract-gate-review.json",
  {
    ...
    reviewId: "CGR-SPECIALIST-ASSIGNMENT-CONTRACT-CHANGE-001",
...
const gateCandidate = await exactJsonFile(
  "contract-gate-candidate.json",
  {
    ...
    gateCandidateId: "CGC-DEVRELAY-001",
    candidate: execution.candidateRef,
    architectureBaseline: architecture.ref,
    projectOverviewBaseline: projectOverview.ref,
