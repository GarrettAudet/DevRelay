...
const dogfoodRelative = "dogfood/specialist-assignment/work-breakdown";
...
const requirementsBytes = read("dogfood/specialist-assignment/baselines/requirements-baseline.json");
const overviewBytes = read("dogfood/specialist-assignment/baselines/project-overview-baseline.json");
const architectureBytes = read("dogfood/specialist-assignment/architecture-design/architecture-baseline.json");
const repositoryBytes = read(
  "dogfood/specialist-assignment/repository-snapshot.json",
);
const currentBaselineBytes = read(
  "project/history/work-breakdown/1.1.0/work-breakdown-baseline.json",
);
...
const requirementsPromotionBytes = read(
  "dogfood/specialist-assignment/requirements-gate-promotion-proof.json",
);
const architecturePromotionBytes = read(
  "dogfood/specialist-assignment/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofBytes = read(
  "dogfood/work-dependency-analysis/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
const contractDispositionBytes = read("dogfood/specialist-assignment/contract-generation/contract-disposition.json");
const contractBaselineBytes = read("dogfood/specialist-assignment/contract-generation/contract-baseline.json");
const contractPromotionBytes = read(
  "dogfood/specialist-assignment/contract-generation/contract-gate-promotion.json",
);
...
const architectureRef = artifactRef(
  ...
  architectureBytes,
  "file:///C:/repos/DevRelay/dogfood/specialist-assignment/architecture-design/architecture-baseline.json",
);
...
const requirementsPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/specialist-assignment/requirements-gate-promotion-proof.json",
);
const architecturePromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/specialist-assignment/architecture-design/architecture-gate-promotion-proof.json",
);
const priorWorkBreakdownProofRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/work-dependency-analysis/work-breakdown/work-breakdown-gate-promotion-proof.json",
);
...
const contractDispositionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/specialist-assignment/contract-generation/contract-disposition.json",
);
...
const contractPromotionRef = artifactRef(
  ...
  "file:///C:/repos/DevRelay/dogfood/specialist-assignment/contract-generation/contract-gate-promotion.json",
);

const targetAcceptanceIds = requirements.requirements.acceptanceCriteria
  .map(({ id }) => id)
  .filter((id) => id.startsWith("AC-DEV-SA-") || id === "AC-DEV-SPECIALIST-BOUNDARY-001")
  .sort();
const targetArchitectureIds = architecture.sections.architectureModel.content.elements
  .map(({ id }) => id)
  .filter((id) => id.startsWith("EL-SA-"))
  .sort();
assert.equal(targetAcceptanceIds.length, 8);
...
const priorArchitectureIds = currentBaseline.coverageDispositions
  ...
  .map(({ scopeRef }) => scopeRef)
  .sort();
assert.equal(priorAcceptanceIds.length, 26);
assert.equal(priorArchitectureIds.length, 22);
...
const targetContractIds = contractBaseline.contracts
  .filter(({ interfaceIntentId }) => interfaceIntentId.startsWith("IF-SA-"))
  .map(({ id }) => id)
  .sort();
...
assert.equal(traceabilityRefs.length, 11);
...
const packageReviewRef = artifactRef(
  "approved-change-package-review-specialist-assignment-v1",
  {
...
const tasksRef = artifactRef(
  "openspec-tasks-specialist-assignment-v1",
...
const workItems = [
  workItem({
    id: "WI-SA-CONTRACTS",
    objective: "Define the provider-neutral SpecialistAssignment artifacts, module surface, and ranker port.",
    included: ["Define closed invocation, eligibility, candidate, Gate, and traceability contracts.", "Keep concrete runtimes and providers outside the canonical assignment contract."],
    excluded: ["Scheduling, readiness selection, runtime binding, or work execution."],
    deliverables: [["DEL-SA-SCHEMAS", "Closed SpecialistAssignment JSON schemas.", "JsonSchemaSet"], ["DEL-SA-MODULE", "Versioned SpecialistAssignment module definition.", "ModuleDefinition"]],
    workType: "code-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-BOUNDARY-001", "AC-DEV-SA-PLUGIN-001"],
    architectureRefs: ["EL-SA-MODULE", "EL-SA-RANKER-PORT"],
    contractRefs: targetContractIds,
    capabilities: ["CAP-CONTRACT-AUTHORING", "CAP-NODEJS-ENGINEERING"],
    verificationMethod: "Validate positive, negative, unknown-field, and provider-substitution fixtures.",
    successCriteria: "Only closed provider-neutral planning contracts validate and no execution authority crosses the ranker port.",
    evidenceKind: "specialist-assignment/contract-tests",
  }),
  workItem({
    id: "WI-SA-CATALOGS-POLICY",
    objective: "Provide exact versioned specialist, capability, tool, grant, and assignment-policy inputs.",
    included: ["Define closed catalogs and policy fixtures.", "Fail closed for unknown profiles, capabilities, tools, grants, and policy versions."],
    excluded: ["Availability, scheduling, concrete model selection, or runtime credentials."],
    deliverables: [["DEL-SA-CATALOGS", "Versioned specialist and capability catalogs.", "CatalogSet"], ["DEL-SA-POLICY", "Closed assignment policy fixture.", "PolicyBundle"]],
    workType: "configuration-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-ELIGIBILITY-001", "AC-DEV-SA-FAIL-CLOSED-001"],
    architectureRefs: ["EL-SA-INPUT-GUARD", "EL-SA-ELIGIBILITY-EVALUATOR"],
    capabilities: ["CAP-CONTRACT-AUTHORING"],
    dependencyHints: [hint("WI-SA-CONTRACTS", "after", "Catalogs and policy must conform to the canonical input contracts.")],
    verificationMethod: "Exercise valid, stale, missing, and unknown catalog and policy references.",
    successCriteria: "Every accepted catalog and policy input is exact, versioned, content-bound, and fail-closed.",
    evidenceKind: "specialist-assignment/catalog-policy-tests",
  }),
  workItem({
    id: "WI-SA-ELIGIBILITY",
    objective: "Implement Core-owned hard eligibility evaluation for every approved work item.",
    included: ["Evaluate required capabilities, tools, grants, and policy.", "Return deterministic eligible sets and evidence-backed rejection reasons."],
    excluded: ["Ranking eligible profiles or approving an assignment candidate."],
    deliverables: [["DEL-SA-ELIGIBILITY", "Deterministic Core eligibility evaluator.", "RuntimeLibrary"]],
    workType: "code-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-DETERMINISM-001", "AC-DEV-SA-ELIGIBILITY-001", "AC-DEV-SA-FAIL-CLOSED-001"],
    architectureRefs: ["EL-SA-INPUT-GUARD", "EL-SA-ELIGIBILITY-EVALUATOR"],
    contractRefs: ["CT-IF-SA-ELIGIBILITY-EVALUATION"],
    capabilities: ["CAP-NODEJS-ENGINEERING", "CAP-TEST-ENGINEERING"],
    dependencyHints: [hint("WI-SA-CATALOGS-POLICY", "after", "Eligibility consumes exact validated catalogs and policy.")],
    verificationMethod: "Replay shuffled inputs and mutate every hard requirement and policy binding.",
    successCriteria: "Core produces stable eligible sets or exact fail-closed diagnostics without calling a ranker.",
    evidenceKind: "specialist-assignment/eligibility-tests",
  }),
  workItem({
    id: "WI-SA-NATIVE-RANKER",
    objective: "Implement the default native structured ranker over Core-supplied eligible profiles.",
    included: ["Select exactly one eligible profile per work item.", "Return deterministic rationale through the bounded ranker port."],
    excluded: ["Changing eligibility, creating profiles, scheduling work, or binding a concrete runtime."],
    deliverables: [["DEL-SA-NATIVE-RANKER", "Native structured assignment ranker.", "RuntimeLibrary"]],
    workType: "code-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-COMPLETE-001", "AC-DEV-SA-DETERMINISM-001"],
    architectureRefs: ["EL-SA-NATIVE-RANKER", "EL-SA-RANKER-PORT"],
    contractRefs: ["CT-IF-SA-RANKER-INVOCATION"],
    capabilities: ["CAP-NODEJS-ENGINEERING"],
    dependencyHints: [hint("WI-SA-ELIGIBILITY", "after", "The ranker receives only Core-approved eligible sets.")],
    verificationMethod: "Replay equivalent eligible sets in randomized order and reject out-of-set selections.",
    successCriteria: "The native ranker deterministically selects exactly one supplied eligible profile and cannot expand authority.",
    evidenceKind: "specialist-assignment/native-ranker-tests",
  }),
  workItem({
    id: "WI-SA-OPTIONAL-ADAPTERS",
    objective: "Publish optional ranker adapters behind the same provider-neutral port.",
    included: ["Declare bounded adapter capabilities and normalize native output.", "Prove adapter substitution uses identical canonical input and output contracts."],
    excluded: ["Making an optional adapter a Core dependency or eligibility authority."],
    deliverables: [["DEL-SA-OPTIONAL-ADAPTERS", "Optional ranker adapter manifests and fixtures.", "ModulePluginSet"]],
    workType: "configuration-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-BOUNDARY-001", "AC-DEV-SA-PLUGIN-001"],
    architectureRefs: ["EL-SA-OPTIONAL-ADAPTERS", "EL-SA-RANKER-PORT"],
    contractRefs: ["CT-IF-SA-RANKER-INVOCATION"],
    capabilities: ["CAP-CONTRACT-AUTHORING"],
    dependencyHints: [hint("WI-SA-CONTRACTS", "after", "Adapters must conform to the exact ranker port.")],
    verificationMethod: "Normalize optional-adapter fixtures and compare authority and contract behavior with the native ranker.",
    successCriteria: "Adapters are replaceable without Core branches, graph access, or eligibility authority.",
    evidenceKind: "specialist-assignment/plugin-conformance",
  }),
  workItem({
    id: "WI-SA-ASSEMBLY-GATE",
    objective: "Assemble one complete assignment candidate and promote it through SpecialistAssignmentGate.",
    included: ["Require exactly one profile for every approved work item.", "Reject the full candidate if any item is unassignable.", "Promote only exact replay-verified bytes."],
    excluded: ["Partial baselines, scheduling, readiness calculation, execution, or DAG mutation."],
    deliverables: [["DEL-SA-ASSEMBLER", "Complete assignment candidate assembler.", "RuntimeLibrary"], ["DEL-SA-GATE", "SpecialistAssignment Gate and promotion runtime.", "GateRuntime"]],
    workType: "code-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-COMPLETE-001", "AC-DEV-SA-FAIL-CLOSED-001"],
    architectureRefs: ["EL-SA-CANDIDATE-ASSEMBLER", "EL-SA-GATE"],
    contractRefs: ["CT-IF-SA-CANDIDATE-ASSEMBLY", "CT-IF-SA-GATE-CANDIDATE"],
    capabilities: ["CAP-CONTRACT-AUTHORING", "CAP-NODEJS-ENGINEERING"],
    dependencyHints: [hint("WI-SA-NATIVE-RANKER", "after", "Assembly consumes normalized ranker selections."), hint("WI-SA-ELIGIBILITY", "after", "Gate revalidates Core-owned eligibility evidence.")],
    verificationMethod: "Exercise complete, missing, duplicate, ineligible, stale, modified-byte, and replay promotion fixtures.",
    successCriteria: "Only one complete exact assignment set can become the approved baseline; partial assignment never progresses.",
    evidenceKind: "specialist-assignment/gate-tests",
  }),
  workItem({
    id: "WI-SA-TRACEABILITY",
    objective: "Project candidate and approved assignments through trusted forward-only contributors.",
    included: ["Derive assignment relationships from validated domain references.", "Persist update digest, execution identity, graph version, and checkpoint proof."],
    excluded: ["Graph access or arbitrary relationship selection by ranker adapters."],
    deliverables: [["DEL-SA-TRACEABILITY", "Candidate and approved assignment contributors with merge proof.", "TraceabilityContributorSet"]],
    workType: "code-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-TRACEABILITY-001"],
    architectureRefs: ["EL-SA-CANDIDATE-CONTRIBUTOR", "EL-SA-APPROVED-CONTRIBUTOR"],
    contractRefs: ["CT-IF-SA-CANDIDATE-TRACEABILITY", "CT-IF-SA-APPROVED-TRACEABILITY"],
    capabilities: ["CAP-NODEJS-ENGINEERING", "CAP-TEST-ENGINEERING"],
    dependencyHints: [hint("WI-SA-ASSEMBLY-GATE", "after", "Approved projection requires the exact Gate-approved assignment baseline.")],
    verificationMethod: "Project, validate, atomically merge, replay, and query candidate and approved assignment relationships.",
    successCriteria: "Only trusted forward assignment facts enter TraceabilityGraph with reproducible merge proof.",
    evidenceKind: "specialist-assignment/traceability-tests",
  }),
  workItem({
    id: "WI-SA-VERIFICATION",
    objective: "Release-gate SpecialistAssignment across contracts, eligibility, ranking, Gate, traceability, and packaging.",
    included: ["Run focused and full regressions, replay, drift, authority, substitution, and package checks.", "Prove the module assigns profiles without executing work."],
    excluded: ["Runtime provider binding, scheduling, or work execution."],
    deliverables: [["DEL-SA-RELEASE-EVIDENCE", "SpecialistAssignment release verification evidence.", "VerificationEvidenceSet"]],
    ...
    dependencyHints: [hint("WI-SA-ASSEMBLY-GATE", "after", "Gate behavior must be available for end-to-end verification."), hint("WI-SA-TRACEABILITY", "after", "Traceability behavior must be available for end-to-end verification.")],
    verificationMethod: "Run the canonical package gate, module dogfood replay, mutation tests, and independent contract review.",
    successCriteria: "All release gates pass deterministically with complete assignments and no execution authority.",
    evidenceKind: "specialist-assignment/release-gate",
  }),
  workItem({
    id: "WI-SA-DOCUMENTATION",
    objective: "Document SpecialistAssignment configuration, boundaries, artifacts, outcomes, and extension points.",
    included: ["Explain Core eligibility, replaceable ranking, Gate, traceability, and downstream runtime binding.", "State clearly that assignment does not build or execute work."],
    excluded: ["Claiming concrete provider availability or execution completion."],
    deliverables: [["DEL-SA-DOCUMENTATION", "Operator and extension documentation.", "DocumentationSet"]],
    workType: "documentation-change",
    acceptanceCriterionRefs: ["AC-DEV-SA-BOUNDARY-001", "AC-DEV-SPECIALIST-BOUNDARY-001"],
    architectureRefs: ["EL-SA-MODULE"],
    capabilities: ["CAP-TECHNICAL-WRITING"],
    dependencyHints: [hint("WI-SA-VERIFICATION", "related", "Documentation should cite final verified behavior.")],
    verificationMethod: "Review documentation against approved requirements, architecture, contracts, and release evidence.",
    successCriteria: "Operators can configure and audit assignment without confusing profile assignment with execution.",
    evidenceKind: "specialist-assignment/documentation-review",
  }),
...
const coverageDispositions = [
  ...currentBaseline.coverageDispositions.map(({ scopeKind, scopeRef }) => ({
    ...
    rationale:
      "The prior WorkDependencyAnalysis module work is present in the pinned repository snapshot and has an exact passing Gate promotion proof; it is historical completion evidence, not new SpecialistAssignment work.",
...
assert.equal(coverageDispositions.length, 72);
...
const changes = [
  ...currentBaseline.workItems.map((item) => ({
    ...
    rationale:
      "The prior WorkDependencyAnalysis deliverable is present in the pinned repository revision and has exact passing Gate evidence; retain its scope as already satisfied instead of carrying completed work into the SpecialistAssignment plan.",
  })),
...
const resultFor = (invocationId) => ({
  ...
  evidence: [
    ...
    },
    {
      ...
      artifact: workBreakdownChangeSetRef,
      summary:
        "Eleven prior completed dependency-analysis items are retired and their 48 scope dispositions are preserved as already satisfied with exact repository and Gate evidence.",
    },
  ],
  diagnostics: [],
});
...
const routeRef = artifactRef(
  "module-route-decision-specialist-assignment-v1",
  ROUTE_CONTRACT,
  routeBytes,
...
const invocation = {
  ...
  invocationId: "work-breakdown-decompose-specialist-assignment-v1",
  runId: "specialist-assignment-dogfood-run-v1",
  ...
  config: {
    ...
    planningOutputRoot:
      "C:/repos/DevRelay/dogfood/specialist-assignment/work-breakdown",
    toolName: "OpenSpec",
    toolVersion: "0.1.0-fixture",
    changeName: "specialist-assignment",
...
const activeSaWorkItems = snapshot.nodes.filter(
  ({ kind, state, stableId }) =>
...
assert.equal(activeSaWorkItems.length, workItems.length);
...
const gateRef = artifactRef(
  "work-breakdown-gate-specialist-assignment-v1",
  {
...
const promotionProof = {
  ...
  proofId: "work-breakdown-gate-promotion-specialist-assignment-v1",
  status: "promoted",
...
const dogfoodProof = {
  ...
  limitations: [
    ...
    "WorkBreakdown did not assign a specialist, schedule, execute, build, or claim completion of any SpecialistAssignment item.",
  ],
};
