...
- **Human-readable lifecycle run reporting** (`CAP-DEV-RUN-REPORTING-001`): Project standardized execution, Gate, observation, artifact, adapter-maturity, and traceability records into one dynamic human-readable run report.
  ...
  - Users: `USR-DEV-WORKFLOW-AUTHOR-001`
- **Deterministic specialist assignment** (`CAP-DEV-SPECIALIST-ASSIGNMENT-001`): Match every approved work item to one provider-neutral specialist profile through trusted eligibility rules and replaceable ranking.
  - Priority: must
  - Audience: user-facing
  - Business objectives: `BO-DEV-DETERMINISM-001`, `BO-DEV-MODULARITY-001`, `BO-DEV-TRACEABILITY-001`
  - Users: `USR-DEV-WORKFLOW-AUTHOR-001`
...
- **`SCOPE-DEV-SPECIALIST-ASSIGNMENT-DETAIL-001`** One SpecialistAssignment module and Gate covering full-plan profile assignment, Core-owned eligibility, replaceable deterministic ranking, typed clarification, candidate traceability, and exact baseline promotion.
...
- **`CON-DEV-SA-AUTHORITY-001`** [business; project] Core owns hard eligibility and Gate authority; rankers may choose only among the exact eligible profiles supplied by Core.
  - Rationale: An extension cannot safely certify its own authority or eligibility.
  - Acceptance criteria: `AC-DEV-SA-ELIGIBILITY-001`, `AC-DEV-SA-PLUGIN-001`
- **`CON-DEV-SA-EXECUTION-BARRIER-001`** [business; project] WorkExecution cannot progress without an exact approved SpecialistAssignmentBaseline covering the selected work item.
  - Rationale: Concrete executors must be bound against approved profile intent rather than improvised assignment.
  - Acceptance criteria: `AC-DEV-SA-BOUNDARY-001`
- **`CON-DEV-SA-ONE-PROFILE-001`** [business; project] V1 assigns exactly one provider-neutral specialist profile to each approved work item and rejects partial assignment candidates.
  - Rationale: This keeps the handoff deterministic while leaving multi-party execution and collaboration for a later extension.
  - Acceptance criteria: `AC-DEV-SA-COMPLETE-001`, `AC-DEV-SA-FAIL-CLOSED-001`
...
- **`NFR-DEV-SA-DETERMINISM-001`** [reliability; must; project] Eligibility filtering, ranker input ordering, tie handling, candidate construction, diagnostics, and Gate inputs must be deterministic for exact version-pinned inputs.
  ...
  - Acceptance criteria: `AC-DEV-SA-DETERMINISM-001`, `AC-DEV-SA-ELIGIBILITY-001`
...
- **Specialist profile** (`TERM-DEV-SPECIALIST-PROFILE-001`): A provider-neutral, versioned description of capabilities, tools, grants, and execution-policy compatibility required to perform a class of work; it is not a concrete executor or runtime binding.
