...
codex/lifecycle-run-report-completion for increment DGI-LIFECYCLE-RUN-REPORT-2026-08-10.
...
c957db0865ef990b27eb86d6671ed0df2e35aa9e. The last fully green release proof is
31d79faed9a1b926188dc0eea34b0d443fda3a35; it is historical baseline evidence, not proof
that the active branch is green.

Do not start LifecycleRunReport DG-2 yet. Resume by closing the active
portability/evidence blockers:

1. Inspect workflow run 31457082371 and verify whether the six local-Git
   fixture failures were removed by c957db0865ef990b27eb86d6671ed0df2e35aa9e.
2. Fix the LifecycleRunReport independent adversarial verifier's repository-root
   calculation cross-platform without changing its assertions.
3. Preserve the immutable historical 6ddd source evidence. Its original source
...
4. Isolate the host-dependent bytes causing Structurizr conformance and
   ArchitectureGate review digest drift. Make the evidence portable; never
   refresh golden hashes just to match one host.
5. Close PB-005 and PB-006 only after the portable bytes are stable.
6. Perform PB-002 once, transactionally and last, then prove a no-change rerun.
