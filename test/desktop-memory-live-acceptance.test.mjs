...
test("live Desktop proof is concluded into a fresh-task-readable memory baseline", () => {
  ...
  const verification = json("dogfood/do-001-desktop-orchestration/gap-remediation/final-acceptance/01-canonical-verification-receipt.json");
  validateProjectMemoryArtifact(baseline);
  validateProjectMemoryArtifact(proof);
  assert.equal(baseline.version, "1.0.5");
  ...
  assert.deepEqual(verification.testSummary, { tests: 1168, passed: 1166, failed: 0, skipped: 2, durationMs: 959269.8019 });
  ...
  assert.match(output.synopsis, /MEMORY-PROBE-20260906-A/u);
  assert.match(output.synopsis, /supported startup boundary is AGENTS\.md plus the managed task prompt/u);
});
