...
import {
  createStructurizrConformanceProof,
  structurizrJavaArgs,
} from "../scripts/verify-structurizr-conformance.mjs";

test("Structurizr runs with one caller-owned Java temp directory", () => {
  assert.deepEqual(
    structurizrJavaArgs(
      {
        warPath: "C:\\tools\\structurizr.war",
        tempDirectory: "C:\\temp\\devrelay-structurizr-001",
      },
      ["validate", "-workspace", "workspace.dsl"],
    ),
    [
      "-Djava.io.tmpdir=C:\\temp\\devrelay-structurizr-001",
      "-jar",
      "C:\\tools\\structurizr.war",
      "validate",
      "-workspace",
      "workspace.dsl",
    ],
  ...
  assert.throws(
    () => structurizrJavaArgs({ warPath: "structurizr.war" }, []),
    /tempDirectory is required/,
  );
});
