...
test("serial, skipped, failed, resumed, parallel, and sourced stage performance project dynamically",()=>{
 ...
 const snapshot=projectLifecycleRunSnapshot({snapshotId:"s",ledgerCheckpoint:createRunLedgerCheckpoint({runId:"run-1",facts}),traceabilityGraph:graphFixture(),observations:[observation()],stageDetails:{alpha:{operation:"execute",adapterBindings:[{id:"adapter-alpha",version:"1.0.0",configurationDigest:d("a")}],importantArtifacts:[ref("e")],performance:[detailMetric],sourceFacts:[ref("f")]}}});
 assert.deepEqual(snapshot.stages.map(s=>s.componentId),["alpha","beta","gate-x","delta"]);assert.equal(snapshot.stages[0].rework.replayed,true);assert.deepEqual(snapshot.stages[0].performance,[detailMetric]);assert.deepEqual(snapshot.stages[0].adapterBindings,[{id:"adapter-alpha",version:"1.0.0",configurationDigest:d("a")}]);assert.ok(snapshot.stages[0].sourceFacts.some(value=>value.artifactId==="artifact-f"));assert.equal(snapshot.stages[2].status,"skipped");assert.equal(snapshot.stages[3].status,"failed");
});
