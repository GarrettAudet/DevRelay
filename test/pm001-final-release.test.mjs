...
test("the accepted ProjectMemory baseline preserves PM-001 and the latest concluded release",()=>{
  ...
  assert.equal(baseline.records.some(({id,status})=>id==="MEM-DEVRELAY-STATUS-DO001-RELEASE-READY-V2"&&status==="active"),true);
  assert.equal(baseline.records.some(({id,status,statement})=>id==="MEM-DEVRELAY-MEMORY-LIVE-ACCEPTANCE"&&status==="active"&&statement.includes("MEMORY-PROBE-20260906-A")),true);
