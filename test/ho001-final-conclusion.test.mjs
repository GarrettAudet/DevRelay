...
test("HO-001 conclusion promotes exact durable memory and fresh-task replay recovers it", () => {
  ...
  assert.equal(currentBaseline.version, "1.0.10");
  assert.deepEqual(currentBaseline, concludedBaseline);
  assert.equal(currentBaseline.supersedes.digest, api.loadProjectMemoryArtifact(priorBaseline).ref.digest);
  ...
  assert.equal(currentBaseline.records.some(({ id, status }) => id === summary.acceptedMemoryId && status === "active"), true);
  ...
  assert.equal(currentBaseline.records.some(({ id }) => id === "MEM-DEVRELAY-NEXT-AFTER-HO001"), false);
  ...
  assert.match(read("project/CurrentSynopsis.md").toString("utf8"), /MEM-DEVRELAY-STATUS-HO001-RC3-CANDIDATE/u);
