...
test("the canonical project baseline contains the exact approved V1 lifecycle", async () => {
  ...
  assert.equal(requirementsBaseline.version, "2.5.0");
  assert.equal(overviewBaseline.version, "2.5.0");
  assert.equal(
  ...
  assert.equal(
    sha256Digest(
      await readFile(new URL("project/history/2.4.0/requirements-baseline.json", root)),
    ),
    promotion.previous.requirementsBaseline.digest,
  );
  assert.equal(
    sha256Digest(
      await readFile(new URL("project/history/2.4.0/project-overview-baseline.json", root)),
    ),
