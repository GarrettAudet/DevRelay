...
const inputSchema = (operation, required) => ({ type: "object", additionalProperties: false, required, properties: Object.fromEntries(required.map((key) => [key, key === "operation" ? { const: operation } : fieldSchemas[key]])) });

export const CHATGPT_DESKTOP_MCP_TOOLS = Object.freeze(OPERATIONS.map(([name, operation, required, description]) => Object.freeze({
  name,
  description,
  inputSchema: inputSchema(operation, required),
})));
