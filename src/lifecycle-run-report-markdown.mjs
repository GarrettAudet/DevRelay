...
export const LIFECYCLE_RUN_REPORT_RENDERER_VERSION = "1.1.0";
...
export function renderLifecycleRunReportMarkdown(snapshotOrOptions,maybeOptions) {
  const {snapshot,contentPolicy,policy,contentPolicyRef,policyRef,classifications={},defaultClassification="internal",rendererVersion=LIFECYCLE_RUN_REPORT_RENDERER_VERSION,view="full"}=normalizeArguments(snapshotOrOptions,maybeOptions);
  ...
  if(!["summary","full"].includes(view)) fail(`unsupported report view ${view}`);
  ...
  if(view==="summary") return [
    ...
    "---","",`Generated by DevRelay LifecycleRunReport renderer ${rendererVersion}. This concise report is a read-only projection; linked canonical artifacts retain complete authoritative evidence.`,""
  ].join("\n");
  const lines=[
    "# Lifecycle Run Report","","## Executive summary","",
    renderValue(context,"/executiveSummary",summary),"",
