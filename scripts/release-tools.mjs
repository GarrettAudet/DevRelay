...
function installAndImport(tarball, packageDocument, temporaryRoot, exportInventory) {
  ...
  const smokeProgram = [
    ...
    'const { join: checkpointPathJoin } = await import("node:path");',
    'const checkpointStatePath = checkpointPathJoin(process.cwd(), "installed-checkpoint-state");',
    'let checkpointStorage = api.createLocalHostStorage({ rootDirectory: checkpointStatePath });',
    'let durableCheckpoints = api.createLocalHostCheckpointStore({ storage: checkpointStorage, namespace: "installed/core" });',
    'durableCheckpoints.put("step-1", { outcome: "recorded", evidence: ["exact-bytes"] });',
    'checkpointStorage.close();',
    'checkpointStorage = api.createLocalHostStorage({ rootDirectory: checkpointStatePath });',
    'durableCheckpoints = api.createLocalHostCheckpointStore({ storage: checkpointStorage, namespace: "installed/core" });',
    'if (api.canonicalJson(durableCheckpoints.get("step-1")) !== api.canonicalJson({ outcome: "recorded", evidence: ["exact-bytes"] })) throw new Error("installed durable checkpoint restart failed");',
    'if (durableCheckpoints.putIfAbsent("step-1", { wrong: true }).outcome !== "recorded") throw new Error("installed checkpoint replaced its winner");',
    'let checkpointConflictRejected = false; try { durableCheckpoints.put("step-1", { wrong: true }); } catch (error) { checkpointConflictRejected = error.code === "DR4932"; }',
    'if (!checkpointConflictRejected) throw new Error("installed checkpoint failed to reject a conflicting write");',
    'if (api.createLocalHostCheckpointStore({ storage: checkpointStorage, namespace: "installed/other" }).get("step-1") !== undefined) throw new Error("installed checkpoint leaked across namespaces");',
    'checkpointStorage.close();',
    'console.log("Installed durable checkpoint restart/conflict/isolation smoke passed.");',
    'const graphStatePath = checkpointPathJoin(process.cwd(), "installed-graph-state");',
    'let graphStorage = api.createLocalHostStorage({ rootDirectory: graphStatePath });',
    'const connectDurableGraph = () => api.createTraceabilityGraphService({ graphId: "installed-graph", projectId: "installed-project", contributors: [], store: api.createLocalHostTraceabilityStore({ storage: graphStorage, namespace: "installed", graphId: "installed-graph" }) });',
    'let installedGraph = connectDurableGraph();',
    'const graphInvocation = { apiVersion: "devrelay.dev/v1alpha1", kind: "ModuleInvocation", invocationId: "installed-graph-check", runId: "installed", nodeId: "check", module: { id: "installed-fixture", version: "1.0.0", operation: "check" }, inputs: {}, options: {} };',
    'const graphRequest = { invocation: graphInvocation, invocationFingerprint: api.canonicalJsonDigest(graphInvocation), moduleResult: { apiVersion: "devrelay.dev/v1alpha1", kind: "ModuleResult", invocationId: graphInvocation.invocationId, status: "completed", outcome: "checked", outputs: {}, evidence: [], diagnostics: [] }, loadedInputs: {}, loadedOutputs: {} };',
    'const preparedGraph = await installedGraph.prepare({ baseGraph: installedGraph.captureBase(), ...graphRequest });',
    'const mergedGraph = await installedGraph.mergePrepared(preparedGraph);',
    'graphStorage.close(); graphStorage = api.createLocalHostStorage({ rootDirectory: graphStatePath }); installedGraph = connectDurableGraph();',
    ...
    'const rmBindings = ["project-overview", "project-overview-projection", "project-memory-baseline", "current-synopsis", "traceability-context", "lifecycle-status", "roadmap", "roadmap-projection"].map((role) => { const bytes = Buffer.from(`installed:${role}`, "utf8"); const artifact = rmRef(`RM-CONTEXT-${role}`, api.sha256Digest(bytes)); rmContextBytes.set(artifact.digest, bytes); return { role, artifact, artifactVersion: "1.0.0" }; });',
    ...
    'if (typeof api.createHumanOrchestrationView !== "function") throw new Error("missing HumanOrchestration view export");',
    'if (typeof api.createHumanOrchestrationController !== "function") throw new Error("missing HumanOrchestration controller export");',
    'const hoPlan = api.createDesktopOrchestrationPlan({ runId: "RUN-HO-INSTALLED", projectId: "devrelay", horizonDigest: rmDigest, startingRevision: "b".repeat(40), maxConcurrency: 1, workItems: [{ id: "WI-HO-INSTALLED", dependencies: [] }] });',
    'const hoRun = { kind: "LocalHostRunState", version: 0, state: { plan: hoPlan, workState: { "WI-HO-INSTALLED": { status: "pending", receipts: [] } }, blockers: [], recovery: "clean" } };',
    'const hoView = api.createHumanOrchestrationView({ projectId: "devrelay", orchestrationRun: hoRun });',
    'if (!api.verifyHumanOrchestrationView(hoView) || hoView.runId !== "RUN-HO-INSTALLED") throw new Error("installed HumanOrchestration view failed");',
    'const hoRequest = api.createHumanInterventionRequest({ runId: hoView.runId, expectedStateVersion: hoView.stateVersion, snapshotDigest: hoView.viewDigest, requestedBy: "release-smoke", requestedAt: "2026-09-10T00:00:00.000Z", action: "message", target: { kind: "task", id: "TASK-HO-INSTALLED" }, reason: "Prove typed installed control.", payload: { message: "Report status." } });',
    'let hoDispatches = 0;',
    'const hoController = api.createHumanOrchestrationController({ inspectRun: async () => hoRun, inspectView: async () => hoView, handlers: { "desktop-task-adapter": async () => { hoDispatches += 1; return { status: "delivered" }; } } });',
    'const hoFirst = await hoController.execute(hoRequest); const hoReplay = await hoController.execute(hoRequest);',
    'if (hoFirst.receipt.outcome !== "dispatched" || !hoReplay.replayed || hoDispatches !== 1) throw new Error("installed HumanOrchestration control/replay failed");',
